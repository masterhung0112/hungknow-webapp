...
class GridModel {
    ...
    update(childItems?: any[]) {
        ...
        if (this.props.shaders.length) {
            this.apply_shaders(drawContext);
        }
        ...
        overlays.forEach((l) => {
            ...
            const r = l.renderer;
            if (r.pre_draw) {
                r.pre_draw(drawContext);
            }
            r.draw(drawContext);
            if (r.post_draw) {
                r.post_draw(drawContext);
            }
    ...
    grid(stack: any[]) {
        ...
        for (const [x] of this.layout.xs) {
            stack.push(<Line
                key={`x-${x}`}
                points={[x - 0.5, 0, x - 0.5, ymax]}
                stroke={this.props.colors.colorGrid}
            />);
        }
        ...
        for (const [y] of this.layout.ys) {
            stack.push(
                <Line
                    key={`y-${y}`}
                    points={[0, y - 0.5, this.layout.width, y - 0.5]}
                    stroke={this.props.colors.colorGrid}
                />,
            );
        }
        ...
        if (this.props.grid_id) {
            this.upper_border(stack);
        }
    }
    ...
    mousezoom(delta: number, event: any) {
        ...
        if (delta < 0 && this.data.length <= this.MIN_ZOOM) {
            return;
        }
        if (delta > 0 && this.data.length > this.MAX_ZOOM) {
            return;
        }
    ...
    pinchzoom(scale: number) {
        if (scale > 1 && this.data.length <= this.MIN_ZOOM) {
            return;
        }
        if (scale < 1 && this.data.length > this.MAX_ZOOM) {
            return;
        }

        const t = this.pinch.t;
        const nt = (Number(t)) / scale;
    ...
    change_range() {
        ...
        if (!this.range || this.data.length < 2) {
            return;
        }
...
export interface GridProps extends EventEmitterValue {
    ...
    cursor: any;
    colors: any;
    overlays?: Array<React.ComponentType<OverlayProps> & OverlayMeta>;
    width: number;
    height: number;
    data: DataCore[];
    grid_id: number;
...
const GridBase: React.FC<GridProps> = (props) => {
    ...
    const [supportedOverlays, setSupportOverlays] = useState<Array<React.ComponentType<OverlayProps> & OverlayMeta>>([
        ...SupportedOverlays,
        ...overlays,
    ]);
