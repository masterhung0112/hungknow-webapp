...
const emailStartPattern =
  ...
  '.]+\\")@'
const reEmail = XRegExp.cache('(^|[^\\pL\\d])(' + emailStartPattern + '[\\pL\\d.\\-]+[.]\\pL{2,4}(?=$|[^\\p{L}]))', 'g')
...
function autolinkChannelMentions(text: string, tokens: Tokens, channelNamesMap: ChannelNamesMap, team?: Team) {
  function channelMentionExists(c: string) {
    return (
      Boolean(channelNamesMap[c]) && typeof channelNamesMap[c] === 'object' && Boolean(channelNamesMap[c].display_name)
    )
  }
  ...
  function replaceChannelMentionWithToken(fullMatch: string, mention: string, channelName: string) {
    ...
    if (channelMentionExists(channelNameLower)) {
      ...
      return addToken(channelNameLower, teamName, mention, escapeHtml(channelValue.display_name))
    }
...
export function escapeHtml(text: string): string {
  return text.replace(/[&<>"']/g, (match: string) => htmlEntities[match as keyof typeof htmlEntities])
}
...
function highlightCurrentMentions(text: string, tokens: Tokens, mentionKeys: MentionKey[] = []) {
  ...
  function replaceCurrentMentionWithToken(fullMatch: string, prefix: string, mention: string, suffix = '') {
    const index = tokens.size
...
export function highlightSearchTerms(text: string, tokens: Tokens, searchPatterns: SearchPattern[]): string {
