...
describe('TextFormatting.Emails', () => {
  ...
  it('Should be valid, but matching GitHub', () => {
    assert.equal(TextFormatting.formatText('email@123.123.123.123').trim(), '<p>email@123.123.123.123</p>')

    assert.equal(TextFormatting.formatText('email@[123.123.123.123]').trim(), '<p>email@[123.123.123.123]</p>')

    assert.equal(TextFormatting.formatText('"email"@domain.com').trim(), '<p>&quot;email&quot;@domain.com</p>')
  })

  it('Should be valid, but broken due to Markdown parsing happening before email autolinking', () => {
    assert.equal(TextFormatting.formatText('_______@domain.com').trim(), '<p><strong>___</strong>@domain.com</p>')
  })

  it('Not valid emails', () => {
    assert.equal(TextFormatting.formatText('plainaddress').trim(), '<p>plainaddress</p>')

    assert.equal(TextFormatting.formatText('#@%^%#$@#$@#.com').trim(), '<p>#@%^%#$@#$@#.com</p>')

    assert.equal(TextFormatting.formatText('@domain.com').trim(), '<p>@domain.com</p>')
