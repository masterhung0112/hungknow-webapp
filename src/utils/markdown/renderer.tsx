...
export default class Renderer extends marked.Renderer {
  ...
  public code(code: string, language: string) {
    ...
    if (SyntaxHighlighting.canHighlight(usedLanguage)) {
      header = '<span class="post-code__language">' + SyntaxHighlighting.getLanguageName(usedLanguage) + '</span>'
    }

    let lineNumbers = ''
    if (SyntaxHighlighting.canHighlight(usedLanguage)) {
      lineNumbers = '<div class="post-code__line-numbers">' + SyntaxHighlighting.renderLineNumbers(code) + '</div>'
    }
  ...
  public link(href: string, title: string, text: string, isUrl = false) {
    ...
    if (isInternalLink) {
      ...
      const unhandledPaths = ['plugins', 'files']
  ...
  public listitem(text: string, bullet = '') {
    ...
    if (isTaskList) {
      return `<li class="list-item--task-list">${
        '<input type="checkbox" disabled="disabled" ' + (isTaskList[1] === ' ' ? '' : 'checked="checked" ') + '/> '
  ...
  public text(txt: string) {
    return TextFormatting.doFormatText(txt, this.formattingOptions, this.emojiMap)
  }
}
