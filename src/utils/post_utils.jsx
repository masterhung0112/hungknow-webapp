...
import { makeGetDisplayName, getCurrentUserId } from 'hkclient-ts/lib/selectors/entities/users'
import { Permissions, Posts } from 'hkclient-ts/lib/constants'
import * as PostListUtils from 'hkclient-ts/lib/utils/post_list'
import { canEditPost as canEditPostRedux, isPostEphemeral } from 'hkclient-ts/lib/utils/post_utils'
...
export const groupsMentionedInText = (text, groups) => {
  ...
  return (
    (mentions.length > 0 && mentions.map((mention) => groups && groups.get(mention)).filter((trueVal) => trueVal)) || []
  )
}
...
export function isIdNotPost(postId) {
  return PostListUtils.isStartOfNewMessages(postId) || PostListUtils.isDateLine(postId) || PostListRowListIds[postId]
}
