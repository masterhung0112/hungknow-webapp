...
import { samplePlugin1, samplePlugin2 } from 'testlib/helpers/admin_console_plugin_index_sample_pluings'
...
describe('AdminConsoleIndex.generateIndex', () => {
  it('should generate an index where I can search', () => {
    ...
    expect(idx.search('characters')).toEqual(['authentication/password', 'site_config/customization'])
  ...
  it('should generate a index where I can search in other language', () => {
    ...
    expect(idx.search('saml').sort()).toEqual(
      ['authentication/saml', 'environment/session_lengths', 'authentication/email', 'experimental/features'].sort()
    )
    expect(idx.search('nginx')).toEqual(['environment/rate_limiting'])
    expect(idx.search('caracteres').sort()).toEqual(['site_config/customization', 'authentication/password'].sort())
  ...
  it('should generate a index including the plugin settings', () => {
    ...
    const idx = generateIndex(
      AdminDefinition,
      { [samplePlugin1.id]: samplePlugin1, [samplePlugin2.id]: samplePlugin2 },
      intl
    )
