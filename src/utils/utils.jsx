...
import { getPost as getPostAction } from 'hkclient-ts/lib/actions/posts'
import { getTeamByName as getTeamByNameAction } from 'hkclient-ts/lib/actions/teams'
...
import { getTeammateNameDisplaySetting, getBool } from 'hkclient-ts/lib/selectors/entities/preferences'
import { getCurrentUser, getCurrentUserId, getUser } from 'hkclient-ts/lib/selectors/entities/users'
import { blendColors, changeOpacity } from 'hkclient-ts/lib/utils/theme_utils'
...
export function toRgbValues(hexStr) {
  const rgbaStr = `${parseInt(hexStr.substr(1, 2), 16)}, ${parseInt(hexStr.substr(3, 2), 16)}, ${parseInt(
    hexStr.substr(5, 2),
    16
  )}`
  return rgbaStr
...
export function applyTheme(theme) {
  if (theme.sidebarBg) {
    changeCss(
      '.app__body .sidebar--left .sidebar__switcher, .sidebar--left, .app__body .modal .settings-modal .settings-table .settings-links, .app__body .sidebar--menu',
  ...
  if (theme.centerChannelBg) {
    ...
    changeCss(
      '.app__body .dropdown-menu, .app__body .popover, .app__body .tip-overlay',
      'background:' + theme.centerChannelBg
    )
    changeCss(
  ...
  if (theme.centerChannelColor) {
    ...
    changeCss(
      ...
      'box-shadow:' + changeOpacity(theme.centerChannelColor, 0.2) + ' 1px -3px 12px'
    )
    changeCss(
      '.app__body .mentions--top',
    ...
    changeCss(
      '.app__body .file-preview, .app__body .post-image__details, .app__body .markdown__table th, .app__body .markdown__table td, .app__body .modal .settings-modal .settings-table .settings-content .divider-light, .app__body .webhooks__container, .app__body .dropdown-menu, .app__body .modal .modal-header',
      'border-color:' + changeOpacity(theme.centerChannelColor, 0.2)
    )
    ...
    changeCss(
      ...
      'border-bottom-color:' + changeOpacity(theme.centerChannelColor, 0.25)
    )
    ...
    changeCss(
      '.app__body .post.post--comment.other--root.current--user .post-comment, .app__body pre',
      'background:' + changeOpacity(theme.centerChannelColor, 0.05)
    )
    changeCss(
  ...
  if (theme.linkColor) {
    ...
    changeCss(
      '.app__body .channel-header__links .icon:hover, .app__body .channel-header__links > a.active .icon, .app__body .post .post__reply',
      'fill:' + theme.linkColor
    )
    changeCss(
      '.app__body .channel-header__links .icon:hover, .app__body .post .card-icon__container.active svg, .app__body .post .post__reply',
      'fill:' + theme.linkColor
    )
    changeCss(
  ...
  if (theme.errorTextColor) {
    changeCss(
      '.app__body .error-text, .app__body .modal .settings-modal .settings-table .settings-content .has-error, .app__body .modal .input__help.error, .app__body .color--error, .app__body .has-error .help-block, .app__body .has-error .control-label, .app__body .has-error .radio, .app__body .has-error .checkbox, .app__body .has-error .radio-inline, .app__body .has-error .checkbox-inline, .app__body .has-error.radio label, .app__body .has-error.checkbox label, .app__body .has-error.radio-inline label, .app__body .has-error.checkbox-inline label',
...
export function displayFullAndNicknameForUser(user) {
  ...
  if (fullName && user.nickname) {
    displayName = <span className="light">{fullName + ' (' + user.nickname + ')'}</span>
  } else if (fullName) {
    displayName = <span className="light">{fullName}</span>
  } else if (user.nickname) {
    displayName = <span className="light">{'(' + user.nickname + ')'}</span>
  }
        ...
        if (match) {
          ...
          if (team && team.delete_at === 0) {
            ...
            if (match.type === 'channel') {
              ...
              if (!channel) {
                const { data: channelData } = await store.dispatch(
                  getChannelByNameAndTeamName(teamName, channelName, true)
                )
                channel = channelData
              }
            } else {
              // Handle permalink - Get channel data from post
              const { postId } = match
...
function applyLinkMarkdown(e) {
  ...
  if (hasMarkdown) {
    ...
    const cursorAfterWord =
      (selectionEnd !== value.length && spaceAfter && !spaceBefore) || (selectionEnd === value.length && !spaceBefore)
