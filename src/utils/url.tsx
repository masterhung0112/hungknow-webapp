...
export function validateChannelUrl(url: string): React.ReactElement[] {
  ...
  if (cleanedURL !== url || !urlMatched || urlMatched[0] !== url || isDirectMessageFormat) {
    ...
    const endsWithoutLetter =
      url.length > 1 && (url.charAt(url.length - 1) === '-' || url.charAt(url.length - 1) === '_')
    if (startsWithoutLetter && endsWithoutLetter) {
      errors.push(
        formattedError(
          t('change_url.startAndEndWithLetter'),
          'URLs must start and end with a lowercase letter or number.'
        )
      )
