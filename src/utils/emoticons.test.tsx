...
describe('Emoticons', () => {
  describe('handleEmoticons', () => {
    ...
    test('should replace emoticons with tokens', () => {
      expect(Emoticons.handleEmoticons(':goat: :dash:', new Map())).toEqual('$MM_EMOTICON0$ $MM_EMOTICON1$')
    })
    ...
    test('should replace emoticons separated by text', () => {
      expect(Emoticons.handleEmoticons('asdf:goat:asdf:dash:asdf', new Map())).toEqual(
        'asdf$MM_EMOTICON0$asdf$MM_EMOTICON1$asdf'
      )
    })

    test("shouldn't replace invalid emoticons", () => {
      expect(Emoticons.handleEmoticons(':goat : : dash:', new Map())).toEqual(':goat : : dash:')
    })

    test('should allow comma immediately following emoticon :)', () => {
      expect(Emoticons.handleEmoticons(':),', new Map())).toEqual('$MM_EMOTICON0$,')
    })

    test('should allow comma immediately following emoticon :P', () => {
      expect(Emoticons.handleEmoticons(':P,', new Map())).toEqual('$MM_EMOTICON0$,')
    })
  ...
  describe('matchEmoticons', () => {
    ...
    describe('single', () => {
      ...
      test('named emoticons forms', () => {
        expect(Emoticons.matchEmoticons(':thumbs_up:')).toEqual([':thumbs_up:'])
      })
    })

    describe('multiple', () => {
      test('shorthand forms', () => {
        expect(Emoticons.matchEmoticons(':+1: :D')).toEqual([':D', ':+1:'])
      })

      test('named emoticons forms', () => {
        expect(Emoticons.matchEmoticons(':thumbs_up: :smile:')).toEqual([':thumbs_up:', ':smile:'])
      })

      test('mixed', () => {
        expect(Emoticons.matchEmoticons(':thumbs_up: :smile: :+1: :D')).toEqual([
          ':thumbs_up:',
          ':smile:',
          ':D',
          ':+1:',
    ...
    test('inline', () => {
      expect(Emoticons.matchEmoticons('I am feeling pretty :D -- you are: ok?')).toEqual([':D'])
    })
  })
})
