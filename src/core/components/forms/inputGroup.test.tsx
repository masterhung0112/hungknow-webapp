...
describe('<InputGroup>', () => {
  it('supports custom props', () => {
    const renderResult = render(<InputGroup style={{ background: 'yellow' }} tabIndex={4} />)
  ...
  it('would work like a text input', () => {
    ...
    const renderResult = render(<InputGroup value="value" onChange={onChangeMock} data-testid={'input-test'} />)
  ...
  it('supports custom type attribute', () => {
    const emailInput = render(<InputGroup type="email" data-testid={'email'} />).getByTestId('email')
    expect(emailInput).toHaveAttribute('type', 'email')

    const passwordInput = render(<InputGroup type="password" data-testid={'password'} />).getByTestId('password')
  ...
  it('renders left icon before input', () => {
    const renderResult = render(<InputGroup leftIcon="star" data-testid="input-test" />)
  ...
  it(`renders right element inside .${CssClasses.INPUT_ACTION} after input`, () => {
    const renderResult = render(<InputGroup rightElement={<address />} data-testid="input-test" />)
