...
import { AuthorType, MarketplacePlugin, PluginStatusRedux, ReleaseStage } from 'hkclient-ts/lib/types/plugins'
...
describe('components/marketplace/', () => {
  ...
  const sampleInstalledPlugin: MarketplacePlugin = {
    ...
    download_url:
      'https://github.com/mattermost/mattermost-test/releases/download/v1.0.3/com.mattermost.nps-1.0.3.tar.gz',
    author_type: AuthorType.Mattermost,
    release_stage: ReleaseStage.Production,
    enterprise: false,
    manifest: {
  ...
  describe('AllPlugins', () => {
    it('should render with no plugins', () => {
      const wrapper = shallow(<AllPlugins plugins={[]} />)
      expect(wrapper).toMatchSnapshot()
    })

    it('should render with one plugin', () => {
      const wrapper = shallow(<AllPlugins plugins={[samplePlugin]} />)
      expect(wrapper).toMatchSnapshot()
    })

    it('should render with plugins', () => {
      const wrapper = shallow(<AllPlugins plugins={[samplePlugin, sampleInstalledPlugin]} />)
      expect(wrapper).toMatchSnapshot()
    })
  })
  ...
  describe('InstalledPlugins', () => {
    ...
    it('should render with no plugins', () => {
      const wrapper = shallow(<InstalledPlugins {...baseProps} installedPlugins={[]} />)
      expect(wrapper).toMatchSnapshot()
    })

    it('should render with one plugin', () => {
      const wrapper = shallow(<InstalledPlugins {...baseProps} installedPlugins={[sampleInstalledPlugin]} />)
      expect(wrapper).toMatchSnapshot()
    })

    it('should render with multiple plugins', () => {
      const wrapper = shallow(
        <InstalledPlugins {...baseProps} installedPlugins={[sampleInstalledPlugin, sampleInstalledPlugin]} />
      )
      expect(wrapper).toMatchSnapshot()
    })
  })
  ...
  describe('MarketplaceModal', () => {
    ...
    test('should render with plugins installed', () => {
      const props = {
        ...baseProps,
        plugins: [...baseProps.plugins, sampleInstalledPlugin],
        installedPlugins: [sampleInstalledPlugin],
      }

      const wrapper = shallow<MarketplaceModal>(<MarketplaceModal {...props} />)
