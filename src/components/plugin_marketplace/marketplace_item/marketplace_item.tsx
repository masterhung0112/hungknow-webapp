...
import { MarketplaceLabel } from 'hkclient-ts/lib/types/plugins'
...
export const UpdateVersion = ({ version, releaseNotesUrl }: UpdateVersionProps): JSX.Element => {
  ...
  return (
    <a target="_blank" rel="noopener noreferrer" href={releaseNotesUrl}>
      {version}
    </a>
  )
}
  ...
  if (description) {
    label = (
      <OverlayTrigger
        ...
        placement="top"
        overlay={<Tooltip id={'plugin-marketplace_label_' + name.toLowerCase() + '-tooltip'}>{description}</Tooltip>}
      >
        {tag}
      </OverlayTrigger>
    )
...
export const UpdateDetails = ({
  ...
  return (
    <div className={classNames('update')}>
      <FormattedMessage id="marketplace_modal.list.update_available" defaultMessage="Update available:" />{' '}
      <UpdateVersion version={version} releaseNotesUrl={releaseNotesUrl} />
      {' - '}
      <b>
        <a onClick={onUpdate}>
          <FormattedMessage id="marketplace_modal.list.update" defaultMessage="Update" />
        </a>
      </b>
    </div>
  )
...
export const UpdateConfirmationModal = ({
  ...
  return (
    <ConfirmModal
      ...
      message={messages}
      confirmButtonText={
        <FormattedMessage id="marketplace_modal.list.update_confirmation.confirm_button" defaultMessage="Update" />
      }
      onConfirm={onUpdate}
      onCancel={onCancel}
    />
...
export default class MarketplaceItem extends React.PureComponent<MarketplaceItemProps, MarketplaceItemState> {
  ...
  getItemButton(): JSX.Element {
    let actionButton = <FormattedMessage id="marketplace_modal.list.Install" defaultMessage="Install" />
    if (this.props.error) {
      actionButton = <FormattedMessage id="marketplace_modal.list.try_again" defaultMessage="Try Again" />
    }

    let button = (
      <button onClick={this.onInstall} className="btn btn-primary" disabled={this.props.installing}>
        <LoadingWrapper
    ...
    if (this.props.installedVersion !== '' && !this.props.installing && !this.props.error) {
      button = (
        <Link to={'/admin_console/plugins/plugin_' + this.props.id}>
          <button onClick={this.onConfigure} className="btn btn-outline">
            <FormattedMessage id="marketplace_modal.list.configure" defaultMessage="Configure" />
          </button>
        </Link>
      )
    }
  ...
  render(): JSX.Element {
    ...
    if (this.props.labels && this.props.labels.length !== 0) {
      labels = this.props.labels.map((label) => (
        <Label key={label.name} name={label.name} description={label.description} url={label.url} color={label.color} />
      ))
    }
    ...
    if (this.props.homepageUrl) {
      ...
      pluginDetails = (
        <>
          <span aria-label={this.props.name.toLowerCase()} className="style--none">
            {pluginDetailsInner}
          </span>
          {labels}
          <span aria-label="Plugin\'s website" className="style--none">
            {description}
          </span>
        </>
      )
    ...
    return (
      <>
        ...
        >
          ...
          <div className="more-modal__actions">{this.getItemButton()}</div>
