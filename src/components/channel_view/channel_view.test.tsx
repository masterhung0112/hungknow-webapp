...
describe('components/channel_view', () => {
  ...
  it('Should match snapshot if channel roles loading', () => {
    const wrapper = shallow(<ChannelView {...baseProps} channelRolesLoading={true} />)
    expect(wrapper).toMatchSnapshot()
  })

  it('Should match snapshot if channel is archived with roles loading', () => {
    const wrapper = shallow(<ChannelView {...baseProps} channelRolesLoading={true} channelIsArchived={true} />)
    expect(wrapper).toMatchSnapshot()
  })
  ...
  it('Should have focusedPostId state based on props', () => {
    ...
    wrapper.setProps({
      ...
      match: { url: '/team/channel/channelId/postId', params: { postid: 'postid' } },
    })
    expect(wrapper.state('focusedPostId')).toEqual('postid')
    wrapper.setProps({
      channelId: 'newChannelId',
      match: { url: '/team/channel/channelId/postId1', params: { postid: 'postid1' } },
    })
