...
import { PostType } from 'hkclient-ts/lib/types/posts'
...
describe('components/dot_menu/DotMenu', () => {
  ...
  test('should match snapshot, on Center', () => {
    ...
    const wrapper: ShallowWrapper<any, any, DotMenu> = shallow(<DotMenu {...props} />)
  ...
  test('should have divider when plugin menu item exists', () => {
    const wrapper: ShallowWrapper<any, any, DotMenu> = shallow(<DotMenu {...baseProps} />)
    expect(wrapper.find('#divider_post_post_id_1_plugins').exists()).toBe(false)

    wrapper.setProps({
      pluginMenuItems: [{ id: 'test_plugin_menu_item_1', text: 'woof' }],
    })
