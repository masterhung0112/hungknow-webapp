...
export default class DotMenu extends React.PureComponent<Props, State> {
  ...
  tooltip = (
    <Tooltip id="dotmenu-icon-tooltip" className="hidden-xs">
      <FormattedMessage id="post_info.dot_menu.tooltip.more_actions" defaultMessage="More actions" />
    </Tooltip>
  )
  ...
  renderDivider = (suffix: string) => {
    return <li id={`divider_post_${this.props.post.id}_${suffix}`} className="MenuItem__divider" role="menuitem" />
  }
  ...
  render() {
    ...
    return (
      <MenuWrapper onToggle={this.props.handleDropdownOpened}>
        <OverlayTrigger className="hidden-xs" delayShow={500} placement="top" overlay={this.tooltip} rootClose={true}>
          <button
            ref={this.buttonRef}
        ...
        >
          ...
          <Menu.ItemAction
            ...
            isDangerous={true}
          />
          {((typeof pluginItems !== 'undefined' && pluginItems.length > 0) ||
            (this.props.components[PLUGGABLE_COMPONENT] && this.props.components[PLUGGABLE_COMPONENT].length > 0)) &&
            this.renderDivider('plugins')}
          {pluginItems}
          <Pluggable postId={this.props.post.id} pluggableName={PLUGGABLE_COMPONENT} />
        </Menu>
      </MenuWrapper>
    )
  }
