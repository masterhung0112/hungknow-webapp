...
import { filterProfilesStartingWithTerm } from 'hkclient-ts/lib/utils/user_utils'
...
export default class ChannelInviteModal extends React.PureComponent<Props, State> {
  ...
  public render = (): JSX.Element => {
    ...
    let users = filterProfilesStartingWithTerm(this.props.profilesNotInCurrentChannel, this.state.term)
      .filter((user) => {
        return (
          user.delete_at === 0 &&
    ...
    return (
      ...
      >
        ...
        <Modal.Body role="application" className="overflow--visible">
          <div className="channel-switcher__header">{header}</div>
          {inviteError}
          <div className="channel-switcher__content">{content}</div>
        </Modal.Body>
      </Modal>
    )
  }
