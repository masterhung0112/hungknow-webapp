...
export const CrossHair: React.FC<CrossHairProps> = ({ cursor, colors, layout, hidden }) => {
  ...
  return (
    <Group width={layout.width} height={layout.height}>
      {/* H */}
      {(cursor.grid_id === layout.id || true) && (
        <Line key={'cursorh'} points={[0, y, layout.width - 0.5, y]} stroke={colors.colorCross} dash={[5]} />
      )}

      {/* V */}
      <Line key={'cursorv'} points={[x, 0, x, layout.height]} stroke={colors.colorCross} dash={[5]} />
    </Group>
  )
}
