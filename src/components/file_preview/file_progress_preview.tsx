...
export default class FileProgressPreview extends React.PureComponent<Props> {
  ...
  render() {
    ...
    if (fileInfo) {
      ...
      fileNameComponent = (
        <React.Fragment>
          <FilenameOverlay fileInfo={fileInfo} compactDisplay={false} canDownload={false} />
          <span className="post-image__uploadingTxt">
            {percent === 100 ? (
              <FormattedMessage id="create_post.fileProcessing" defaultMessage="Processing..." />
            ) : (
              <React.Fragment>
                <FormattedMessage id="admin.plugin.uploading" defaultMessage="Uploading..." />
      ...
      if (percent) {
        progressBar = <ProgressBar className="post-image__progressBar" now={percent} active={percent === 100} />
      }
    }

    return (
      <div ref={clientId} key={clientId} className="file-preview post-image__column" data-client-id={clientId}>
        <div className="post-image__thumbnail">{previewImage}</div>
        <div className="post-image__details">
          <div className="post-image__detail_wrapper">
            <div className="post-image__detail">{fileNameComponent}</div>
          </div>
          <div>
