...
import Groups from 'hkclient-ts/lib/constants/groups'
...
class ChannelGroupsManageModal extends React.PureComponent<Props> {
  ...
  public onClickRemoveGroup = (item: Group, listModal: any) =>
    this.props.actions
      .unlinkGroupSyncable(item.id, this.props.channel.id, Groups.SYNCABLE_TYPE_CHANNEL)
      .then(async () => {
  ...
  public setChannelMemberStatus = async (item: Group, listModal: any, isChannelAdmin: boolean) => {
    this.props.actions
      .patchGroupSyncable(item.id, this.props.channel.id, Groups.SYNCABLE_TYPE_CHANNEL, {
    ...
    return (
      <ListModal
        titleText={formatMessage(
          { id: 'groups', defaultMessage: '{channel} Groups' },
