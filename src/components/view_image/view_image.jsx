...
import { getFilePreviewUrl, getFileUrl, getFileDownloadUrl } from 'hkclient-ts/lib/utils/file_utils'
  ...
  render() {
    ...
    if (this.state.loaded[this.state.imageIndex]) {
      if (fileType === FileTypes.IMAGE || fileType === FileTypes.SVG) {
        content = <ImagePreview fileInfo={fileInfo} canDownloadFiles={this.props.canDownloadFiles} />
        ...
        content = (
          <div className="pdf">
            <React.Suspense fallback={null}>
              <PDFPreview fileInfo={fileInfo} fileUrl={fileUrl} scale={this.state.scale[this.state.imageIndex]} />
            </React.Suspense>
          </div>
        )
      ...
      content = <LoadingImagePreview loading={loading} progress={progress} />
    }
    ...
    if (this.state.showCloseBtn) {
      closeButton = <div className="modal-close" onClick={this.handleModalClose} />
    }
    ...
    return (
      ...
      >
        <Modal.Body>
          <div className={'modal-image__wrapper'} onClick={this.handleModalClose}>
            <div
              onMouseEnter={this.onMouseEnterImage}
              onMouseLeave={this.onMouseLeaveImage}
              onClick={(e) => e.stopPropagation()}
            >
              <Modal.Title componentClass="h1" id="viewImageModalLabel" className="hide">
                {fileName}
              </Modal.Title>
              {closeButton}
              <div className="modal-image__background">
                <div className={`modal-image__content${modalImageClass}`}>{content}</div>
              </div>
              <PopoverBar
