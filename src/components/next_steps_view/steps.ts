...
import { getCurrentUser, getUsers } from 'hkclient-ts/lib/selectors/entities/users'
...
export const Steps: StepType[] = [
  ...
  },
  {
    ...
    title: localizeMessage('next_steps_view.notificationSetup.setNotifications', 'Set up desktop notifications'),
    roles: ['system_user'],
...
export const nextStepsNotFinished = createSelector(
  ...
  (stepPreferences, currentUser, firstAdmin) => {
    ...
    const checkPref = (step: StepType) =>
      stepPreferences.some((pref) => (pref.name === step.id && pref.value === 'true') || !isStepForUser(step, roles))
