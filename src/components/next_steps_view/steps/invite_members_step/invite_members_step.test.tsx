...
describe('components/next_steps_view/steps/invite_members_step', () => {
  ...
  test('should match snapshot', () => {
    const wrapper = shallow(<InviteMembersStep {...baseProps} />)

    expect(wrapper).toMatchSnapshot()
  })

  test('should set emails based on specified delimiters', () => {
    const wrapper: ShallowWrapper<any, any, any> = shallowWithIntl(<InviteMembersStep {...baseProps} />)
  ...
  test('should not allow more than 10 emails', () => {
    ...
    const emails = Array(11)
      .fill('a')
      .map((a) => `email_${a}@email.com`)
  ...
  test('should have limit errors when remaining_seats are 0 and free tier', () => {
    ...
    const wrapper: ShallowWrapper<any, any, any> = shallowWithIntl(<InviteMembersStep {...props} />)
  ...
  test('do not fire onChange unless it is a removal or a pop', () => {
    ...
    wrapper.instance().onChange(
      emails.map((e) => ({ label: e, value: e, error: false })),
      { action: 'remove-value' }
    )
