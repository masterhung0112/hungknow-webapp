...
import { TeamInviteWithError, Team } from 'hkclient-ts/lib/types/teams'
import { isEmail } from 'hkclient-ts/lib/utils/helpers'

import { SubscriptionStats } from 'hkclient-ts/lib/types/cloud'
...
class InviteMembersStep extends React.PureComponent<Props, State> {
  ...
  onInputChange = (value: string, change: InputActionMeta) => {
    ...
    if (value.indexOf(' ') !== -1 || value.indexOf(',') !== -1) {
      const emails = value
        ...
        .map((email) => ({ label: email, value: email, error: !isEmail(email) }))
  ...
  onBlur = () => {
    if (this.state.emailInput) {
      const emails = this.state.emailInput
        .split(/[\s,]+/)
        .filter((email) => email.length)
        .map((email) => ({ label: email, value: email, error: !isEmail(email) }))
      const newEmails = [...this.state.emails, ...emails]
  ...
  sendEmailInvites = async () => {
    if (this.state.emails.some((email) => email.error)) {
      this.setState({
        emailError: Utils.localizeMessage(
          'next_steps_view.invite_members_step.invalidEmail',
    ...
    trackEvent(getAnalyticsCategory(this.props.isAdmin), 'click_send_invitations', {
      num_invitations: this.state.emails.length,
    })
    ...
    if (error || !data.length || data.some((result) => result.error)) {
      ...
      this.setState({
        emailError: Utils.localizeMessage(
          'next_steps_view.invite_members_step.errorSendingEmails',
  ...
  render(): JSX.Element {
    const linkBtn = this.props.isAdmin ? (
    ...
    return (
      <div className="NextStepsView__stepWrapper">
            ...
            <div className="InviteMembersStep__emailInvitations">
              ...
              <MultiInput
                ...
                placeholder={Utils.localizeMessage(
                  'next_steps_view.invite_members_step.enterEmailAddresses',
              ...
              <div className="InviteMembersStep__send">
                <button
                  ...
                  className={classNames('NextStepsView__button InviteMembersStep__sendButton secondary', {
                    disabled:
                      this.shouldShowLimitError(this.state.emails.length) ||
                      !this.state.emails.length ||
                      Boolean(this.state.emailsSent) ||
                ...
                >
                  ...
                  <FormattedMessage id="next_steps_view.invite_members_step.send" defaultMessage="Send" />
                </button>
                <div className={classNames('InviteMembersStep__invitationResults', { error: this.state.emailError })}>
                  ...
                  {this.state.emailError &&
                    !isNull(this.props.subscriptionStats) &&
                    this.shouldShowLimitError(this.state.emails.length) &&
                    linkBtn}
                </div>
              </div>
            ...
            <div className="InviteMembersStep__shareLinkBlock">
              <input
                ...
                aria-label={Utils.localizeMessage({
                  id: 'next_steps_view.invite_members_step.shareLinkInput',
                  defaultMessage: 'team invite link',
                })}
                data-testid="InviteMembersStep__shareLinkInput"
              />
              <button
                className={classNames('NextStepsView__button InviteMembersStep__shareLinkInputButton secondary', {
                  copied: this.state.copiedLink,
                })}
              ...
              >
                {!this.state.copiedLink && (
                  <>
                    ...
                    <FormattedMessage id="next_steps_view.invite_members_step.copy_button" defaultMessage="Copy Link" />
                  </>
                )}
                {this.state.copiedLink && (
                  <>
                    <i className="icon icon-check" />
                    <FormattedMessage id="next_steps_view.invite_members_step.link_copied" defaultMessage="Copied" />
                  </>
                )}
              </button>
            </div>
        ...
        <div className="NextStepsView__wizardButtons">
          ...
          >
            <FormattedMessage id="next_steps_view.invite_members_step.finish" defaultMessage="Finish" />
          </button>
        </div>
      </div>
    )
