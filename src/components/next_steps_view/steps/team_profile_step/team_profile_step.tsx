...
export default class TeamProfileStep extends React.PureComponent<Props, State> {
  ...
  private handleNameInputChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!event.target.value) {
      teamNameError = Utils.localizeMessage(
        'next_steps_view.team_profile_step.nameCannotBeBlank',
        'Team name cannot be blank'
      )
    } else if (event.target.value.length > MAX_TEAM_NAME_LENGTH) {
      teamNameError = Utils.localizeMessage(
        'next_steps_view.team_profile_step.nameTooBig',
        'Team name must be less than 64 characters'
      )
    }
  ...
  render() {
    ...
    const pictureSrc =
      team.last_team_icon_update && !this.state.removeProfilePicture
        ? Utils.imageURLForTeam(team) || undefined
        : undefined
      ...
      <div className="NextStepsView__stepWrapper">
        <div className="TeamProfileStep">
          <div className="TeamProfileStep__profilePicture">
            <h3>
              <FormattedMessage id="next_steps_view.team_profile_step.teamImage" defaultMessage="Team image" />
            </h3>
            <PictureSelector
              ...
              placeholder={
                ...
                >
                  {this.props.team.display_name
                    ? this.props.team.display_name.replace(/\s/g, '').substring(0, 2)
                    : '??'}
                </div>
              }
            />
          ...
          <div className="TeamProfileStep__textInputs">
            ...
            <Input
              ...
              info={Utils.localizeMessage(
                'next_steps_view.team_profile_step.nameWillBeDisplayed',
                'Your team name will be displayed in your sidebar'
              )}
            />
          </div>
        </div>
        ...
        <div className="NextStepsView__wizardButtons">
          <button
            ...
            disabled={this.isFinishDisabled()}
          >
            <FormattedMessage id="next_steps_view.team_profile_step.saveTeam" defaultMessage="Save team" />
          </button>
        </div>
      </div>
    )
