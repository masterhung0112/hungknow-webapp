...
export default class CompleteProfileStep extends React.PureComponent<Props, State> {
  ...
  private handleInputChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!event.target.value) {
      fullNameError = Utils.localizeMessage(
        'next_steps_view.complete_profile_step.fullNameCannotBeBlank',
        'Your full name cannot be blank'
      )
    } else if (event.target.value.length > MAX_FULL_NAME_LENGTH) {
      fullNameError = Utils.localizeMessage(
        'next_steps_view.complete_profile_step.fullNameTooBig',
        'Your name must be less than 128 character'
      )
    }
  ...
  render() {
    ...
    const pictureSrc = currentUser.last_picture_update
      ? Utils.imageURLForUser(currentUser.id, currentUser.last_picture_update)
      : undefined
      ...
      <div className="NextStepsView__stepWrapper">
        <div className="CompleteProfileStep">
          <div className="CompleteProfileStep__profilePicture">
            <h3>
              <FormattedMessage id="next_steps_view.complete_profile_step.addAPhoto" defaultMessage="Add a photo" />
            </h3>
            <PictureSelector
          ...
          <div className="CompleteProfileStep__fullName">
            ...
            <Input
              ...
              placeholder={Utils.localizeMessage(
                'next_steps_view.complete_profile_step.yourFullName',
                'Your full name'
              )}
              error={this.state.fullNameError}
              info={Utils.localizeMessage(
                'next_steps_view.complete_profile_step.nameWillBeDisplayed',
                'Your name will be displayed with your messages'
              )}
            />
          </div>
        </div>
        ...
        <div className="NextStepsView__wizardButtons">
          <button
            ...
            disabled={this.isFinishDisabled()}
          >
            <FormattedMessage id="next_steps_view.complete_profile_step.saveProfile" defaultMessage="Save profile" />
          </button>
        </div>
      </div>
    )
