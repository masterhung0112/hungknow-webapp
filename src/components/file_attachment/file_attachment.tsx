...
import { getFileThumbnailUrl, getFileUrl } from 'hkclient-ts/lib/utils/file_utils'
import { FileInfo } from 'hkclient-ts/lib/types/files'

import { FileTypes } from 'utils/constants'
import { trimFilename } from 'utils/file_utils'
import { fileSizeToString, getFileType, loadImage, localizeMessage } from 'utils/utils.jsx'
...
export default class FileAttachment extends React.PureComponent<Props, State> {
  ...
  render() {
    ...
    let fileThumbnail
    let fileDetail
    const ariaLabelImage = `${localizeMessage('file_attachment.thumbnail', 'file thumbnail')} ${
      fileInfo.name
    }`.toLowerCase()

    if (!compactDisplay) {
      fileThumbnail = (
        <a aria-label={ariaLabelImage} className="post-image__thumbnail" href="#" onClick={this.onAttachmentClick}>
          {this.state.loaded ? <FileThumbnail fileInfo={fileInfo} /> : <div className="post-image__load" />}
        </a>
      )

      fileDetail = (
        <div className="post-image__detail_wrapper" onClick={this.onAttachmentClick}>
