...
export default class UserListRowWithError extends React.PureComponent<Props, State> {
  ...
  render(): JSX.Element {
    ...
    return (
      <div data-testid="userListRow" key={this.props.user.id} className="more-modal__row">
        <ProfilePicture
        ...
        <div className="more-modal__right">
          <div className="more-modal__top">
            <div className="more-modal__details">
              <div id={userCountID || undefined} className="more-modal__name">
                <Link to={'/admin_console/user_management/user/' + this.props.user.id}>
                  {Utils.displayEntireNameForUser(this.props.user)}
                </Link>
                <BotBadge className="badge-admin" show={Boolean(this.props.user.is_bot)} />
              </div>
              <div id={userCountEmail || undefined} className={emailStyle}>
                {email}
              </div>
