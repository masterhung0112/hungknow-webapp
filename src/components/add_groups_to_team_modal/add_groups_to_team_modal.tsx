...
import { Group, GroupsWithCount, SyncablePatch, SyncableType } from 'hkclient-ts/lib/types/groups'
...
export default class AddGroupsToTeamModal extends React.PureComponent<Props, State> {
  ...
  public handlePageChange = (page: number, prevPage: number): void => {
    if (page > prevPage) {
      this.setGroupsLoadingState(true)
      this.props.actions
        .getGroupsNotAssociatedToTeam(this.props.currentTeamId, this.props.searchTerm, page, GROUPS_PER_PAGE + 1)
        .then(() => {
          this.setGroupsLoadingState(false)
        })
    }
  ...
  public renderOption = (
    ...
    return (
      ...
      >
        <img className="more-modal__image" src={groupsAvatar} alt="group picture" width="32" height="32" />
    ...
    if (this.state.addError) {
      addError = (
        <div className="has-error col-sm-12">
          <label className="control-label font-weight--normal">{this.state.addError}</label>
        </div>
      )
    }
        ...
        <Modal.Header closeButton={true}>
          <Modal.Title componentClass="h1">
            <FormattedMessage
              ...
              values={{
                teamName: <strong>{this.props.currentTeamName}</strong>,
              }}
            />
          </Modal.Title>
        </Modal.Header>
