...
describe('components/AddGroupsToTeamModal', () => {
  ...
  test('should have called onHide when handleExit is called', () => {
    ...
    const wrapper = shallow<AddGroupsToTeamModal>(<AddGroupsToTeamModal {...props} />)
  ...
  test('should match state when handleResponse is called', () => {
    const wrapper = shallow<AddGroupsToTeamModal>(<AddGroupsToTeamModal {...baseProps} />)
  ...
  test('should match state when handleSubmit is called', async () => {
    ...
    expect(actions.linkGroupSyncable).toBeCalledWith('id_1', baseProps.currentTeamId, Groups.SYNCABLE_TYPE_TEAM, {
      auto_add: true,
      scheme_admin: false,
    })
    expect(actions.linkGroupSyncable).toBeCalledWith('id_2', baseProps.currentTeamId, Groups.SYNCABLE_TYPE_TEAM, {
      auto_add: true,
      scheme_admin: false,
    })
  ...
  test('should match when renderValue is called', () => {
    ...
    expect(
      wrapper.instance().renderValue({ data: { id: 'id_1', label: 'label_1', value: 'value_1', display_name: 'foo' } })
    ).toEqual('foo')
  })
})
