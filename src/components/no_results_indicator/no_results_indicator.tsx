...
const NoResultsIndicator = ({
  ...
  style,
  variant,
  iconGraphic = variant ? <div className="no-results__variant-wrapper">{iconMap[variant]}</div> : null,
  titleValues,
  title = variant ? <FormattedMessage {...titleMap[variant]} values={titleValues} /> : null,
  subtitleValues,
  subtitle = variant ? <FormattedMessage {...subtitleMap[variant]} values={subtitleValues} /> : null,
}: Props) => {
  let content = (
    <div className="no-results__wrapper" style={style}>
      {iconGraphic}

      {title ? <div className={classNames('no-results__title', { 'only-title': !subtitle })}>{title}</div> : null}

      {subtitle ? <div className="no-results__subtitle">{subtitle}</div> : null}
    </div>
  )

  if (expanded) {
    content = <div className="no-results__holder">{content}</div>
  }
