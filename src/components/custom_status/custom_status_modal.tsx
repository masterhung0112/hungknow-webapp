...
import { setCustomStatus, unsetCustomStatus, removeRecentCustomStatus } from 'hkclient-ts/lib/actions/users'
import { setCustomStatusInitialisationState } from 'hkclient-ts/lib/actions/preferences'
import { Preferences } from 'hkclient-ts/lib/constants'
import { UserCustomStatus } from 'hkclient-ts/lib/types/users'
import { Emoji } from 'hkclient-ts/lib/types/emojis'
...
const CustomStatusModal: React.FC<Props> = (props: Props) => {
  ...
  const customStatusEmoji =
    emoji || text ? (
      <RenderEmoji emojiName={emoji || 'speech_balloon'} size={20} />
    ) : (
  ...
  const disableSetStatus =
    (currentCustomStatus.text === text && currentCustomStatus.emoji === emoji) || (text === '' && emoji === '')
  ...
  const suggestion = (
    <div className="statusSuggestion">
      <div className="statusSuggestion__content">
        ...
        <div id="statusSuggestion__suggestions">{renderCustomStatusSuggestions()}</div>
      </div>
    </div>
  )
  ...
  return (
    <GenericModal
      enforceFocus={false}
      onHide={props.onHide}
      modalHeaderText={<FormattedMessage id="custom_status.set_status" defaultMessage="Set a status" />}
      confirmButtonText={<FormattedMessage id="custom_status.modal_confirm" defaultMessage="Set Status" />}
      cancelButtonText={<FormattedMessage id="custom_status.modal_cancel" defaultMessage="Clear Status" />}
    ...
    >
      <div className="StatusModal__body">
        <div className="StatusModal__input">
          <div ref={customStatusControlRef} className="StatusModal__emoji-container">
            {showEmojiPicker && (
              <EmojiPickerOverlay
