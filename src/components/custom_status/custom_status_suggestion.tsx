...
const CustomStatusSuggestion: React.FC<Props> = (props: Props) => {
  ...
  const clearButton = handleClear ? (
    <div className="suggestion-clear">
      <OverlayTrigger
        ...
        placement="top"
        overlay={<Tooltip id="clear-recent-custom-status">{'Clear'}</Tooltip>}
      >
        <button className="style--none input-clear-x" onClick={handleRecentCustomStatusClear}>
  ...
  return (
    ...
    >
      ...
      <CustomStatusText text={text} tooltipDirection="top" className="statusSuggestion__text" />
      {show && clearButton}
    </div>
  )
}
