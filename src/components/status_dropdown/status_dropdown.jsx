...
export default class StatusDropdown extends React.PureComponent {
  ...
  renderProfilePicture = () => {
    ...
    return <Avatar size="lg" url={this.props.profilePicture} />
  }

  renderDropdownIcon = () => {
    return (
      <FormattedMessage id="generic_icons.dropdown" defaultMessage="Dropdown Icon">
        {(title) => <i className={'fa fa-caret-down'} aria-label={title} />}
      </FormattedMessage>
    )
  }
  handleClearStatus = (e) => {
  ...
  renderCustomStatus = () => {
    ...
    const customStatusText = isStatusSet
      ? customStatus.text
      : localizeMessage('status_dropdown.set_custom', 'Set a Custom Status')
    const customStatusEmoji = isStatusSet ? (
      <span className="d-flex">
        <CustomStatusEmoji showTooltip={false} emojiStyle={{ marginLeft: 0 }} />
      </span>
    ) : (
      ...
      <OverlayTrigger
        ...
        overlay={
          <Tooltip id="clear-custom-status">
            <FormattedMessage id="status_dropdown.custom_status.tooltip_clear" defaultMessage="Clear" />
          </Tooltip>
        }
      >
        <button className="style--none input-clear-x" id="custom_status__clear" onClick={this.handleClearStatus}>
          <i className="icon icon-close-circle" />
        </button>
      </OverlayTrigger>
    )

    const pulsatingDot = !isStatusSet && this.props.showCustomStatusPulsatingDot && <span className="pulsating_dot" />
    ...
    return (
      <Menu.Group>
        <Menu.ItemToggleModalRedux
          ...
          sibling={clearButton}
        >
          <span className="custom_status__icon">{customStatusEmoji}</span>
          <CustomStatusText text={customStatusText} className="custom_status__text" />
          {pulsatingDot}
        </Menu.ItemToggleModalRedux>
      </Menu.Group>
    )
    ...
    return (
      ...
      >
        <div className="status-wrapper status-selector">
          ...
          >
            <StatusIcon status={this.props.status} button={true} />
          </button>
          <span className={'status status-edit edit'}>{dropdownIcon}</span>
        </div>
        <Menu ariaLabel={localizeMessage('status_dropdown.menuAriaLabel', 'Set a status')} id="statusDropdownMenu">
          {!this.props.isCustomStatusEnabled && (
            <Menu.Header>
              <FormattedMessage id="status_dropdown.set_your_status" defaultMessage="Status" />
            </Menu.Header>
          )}
          <Menu.Group>
            <Menu.ItemAction
