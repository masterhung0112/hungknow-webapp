...
class InvitationModalMembersStep extends React.PureComponent {
  ...
  onChange = (usersAndEmails) => {
    this.setState({ usersAndEmails }, () => {
      if (this.shouldShowPickerError() && this.props.isCloud) {
        trackEvent('cloud_invite_users', 'warning_near_limit', {
          remaining: this.getRemainingUsers() - this.state.usersAndEmails.length,
        })
      }
    })
  ...
  render() {
    ...
    const inviteMembersButtonDisabled =
      this.state.usersAndEmails.length > Constants.MAX_ADD_MEMBERS_BATCH || this.state.usersAndEmails.length === 0
    ...
    const errorProperties = {
      ...
      extraErrorText: this.props.userIsAdmin ? (
    ...
    if (this.state.usersAndEmails.length > Constants.MAX_ADD_MEMBERS_BATCH) {
      ...
      errorProperties.errorMessageId = t('invitation_modal.invite_members.exceeded_max_add_members_batch')
    ...
    return (
      <div className="InvitationModalMembersStep">
        ...
        <div className="share-link" data-testid="shareLink">
          <h2>
            <FormattedMessage id="invitation_modal.members.share_link.title" defaultMessage="Share This Link" />
          </h2>
          <div className="share-link-input-block">
            ...
            />
            <button className="share-link-input-button" onClick={this.copyLink} data-testid="shareLinkInputButton">
              <LinkIcon />
              {!this.state.copiedLink && (
                <FormattedMessage id="invitation_modal.members.share_link.copy_button" defaultMessage="Copy Link" />
              )}
              {this.state.copiedLink && (
                <FormattedMessage id="invitation_modal.members.share_link.link_copied" defaultMessage="Link Copied" />
              )}
            </button>
          </div>
        ...
        <div className="invitation-modal-or">
          <hr />
          <div>
            <FormattedMessage id="invitation_modal.members.or" defaultMessage="OR" />
          </div>
        </div>
        ...
        <div className="invite-members">
          <button
            className={'btn ' + (inviteMembersButtonDisabled ? 'btn-inactive' : 'btn-primary')}
            onClick={this.submit}
            disabled={inviteMembersButtonDisabled}
            id="inviteMembersButton"
          >
            <FormattedMessage id="invitation_modal.members.invite_button" defaultMessage="Invite Members" />
          </button>
        </div>
      </div>
    )
