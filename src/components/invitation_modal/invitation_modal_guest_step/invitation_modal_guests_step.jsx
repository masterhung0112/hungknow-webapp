...
class InvitationModalGuestsStep extends React.PureComponent {
  ...
  onUsersEmailsChange = (usersAndEmails) => {
    ...
    this.props.onEdit(
      usersAndEmails.length > 0 ||
        this.state.channels.length > 0 ||
        this.state.customMessage !== '' ||
        this.state.usersInputValue ||
        this.state.channelsInputValue
    )
  }
  ...
  channelsLoader = async (value) => {
    ...
    return this.props.myInvitableChannels.filter((channel) => {
      return (
        channel.display_name.toLowerCase().startsWith(value.toLowerCase()) ||
  ...
  shouldShowPickerError = () => {
    const { userLimit, userIsAdmin, isCloud, subscriptionStats } = this.props
  ...
  render() {
    let inputPlaceholder = localizeMessage(
      'invitation_modal.guests.search-and-add.placeholder',
      'Add guests or email addresses'
    )
    ...
    if (!this.props.emailInvitationsEnabled) {
      inputPlaceholder = localizeMessage(
        'invitation_modal.guests.search-and-add.placeholder-email-disabled',
        'Add guests'
      )
    ...
    return (
      <div className="InvitationModalGuestsStep">
        ...
        <div className="add-people" data-testid="addPeople">
          <h2>
            <FormattedMessage id="invitation_modal.guests.add_people.title" defaultMessage="Invite People" />
          </h2>
        ...
        <div className="add-channels" data-testid="channelPlaceholder">
          <h2>
            <FormattedMessage
        ...
        <div className="custom-message" data-testid="customMessage">
          ...
          {this.state.customMessageOpen && (
            <React.Fragment>
              <div>
                <FormattedMessage id="invitation_modal.guests.custom-message.title" defaultMessage="Custom message" />
                <CloseCircleIcon onClick={this.closeCustomMessage} />
              </div>
              <textarea ref={this.textareaRef} onChange={this.onMessageChange} value={this.state.customMessage} />
            </React.Fragment>
          )}
          <div className="help-text">
            <FormattedMessage
        ...
        <div className="invite-guests">
          <button
            className={
              'btn ' +
              (this.state.channels.length === 0 || this.state.usersAndEmails.length === 0
                ? 'btn-inactive'
                : 'btn-primary')
            }
          ...
          >
            <FormattedMessage id="invitation_modal.guests.invite_button" defaultMessage="Invite Guests" />
          </button>
        </div>
      </div>
    )
