...
import { getChannelsInCurrentTeam } from 'hkclient-ts/lib/selectors/entities/channels'
import { haveIChannelPermission, haveITeamPermission } from 'hkclient-ts/lib/selectors/entities/roles'
import { getConfig, getLicense, getSubscriptionStats } from 'hkclient-ts/lib/selectors/entities/general'
import { getProfiles, searchProfiles as reduxSearchProfiles } from 'hkclient-ts/lib/actions/users'
import { getCurrentUser } from 'hkclient-ts/lib/selectors/entities/users'
import { searchChannels as reduxSearchChannels } from 'hkclient-ts/lib/actions/channels'
...
export function mapStateToProps(state) {
  ...
  const canInviteGuests =
    ...
    haveITeamPermission(state, { team: currentTeam.id, permission: Permissions.INVITE_GUEST })
  const isCloud = license.Cloud === 'true'
  const isFreeTierWithNoFreeSeats =
    isCloud &&
    !isEmpty(subscriptionStats) &&
    subscriptionStats.is_paid_tier === 'false' &&
    subscriptionStats.remaining_seats <= 0
