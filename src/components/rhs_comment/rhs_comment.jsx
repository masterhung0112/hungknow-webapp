...
import { Posts } from 'hkclient-ts/lib/constants/index'
import {
  isPostEphemeral,
  isPostPendingOrFailed,
  isMeMessage as checkIsMeMessage,
} from 'hkclient-ts/lib/utils/post_utils'
...
class RhsComment extends React.PureComponent {
  ...
  componentDidUpdate(prevProps) {
    ...
    const shortcutReactToLastPostEmittedFromRHS =
      prevProps.shortcutReactToLastPostEmittedFrom !== shortcutReactToLastPostEmittedFrom &&
  ...
  handleShortcutReactToLastPost = (isLastPost) => {
    if (isLastPost) {
      ...
      const isPostHeaderVisibleToUser =
        boundingRectOfPostInfo.top - 110 > 0 && boundingRectOfPostInfo.bottom < window.innerHeight
  ...
  createRemovePostButton = () => {
    return (
      <button className="post__remove theme color--link style--none" type="button" onClick={this.removePost}>
        {'×'}
      </button>
    )
  }
  ...
  updateAriaLabel = () => {
    ...
    this.setState({
      currentAriaLabel: PostUtils.createAriaLabelForPost(post, author, isFlagged, reactions, intl, emojiMap),
    })
  }
  ...
  render() {
    ...
    if (!isConsecutivePost) {
      ...
      if (post.props && post.props.from_webhook) {
        if (post.props.override_username && this.props.enablePostUsernameOverride) {
          ...
          userProfile = (
            <UserProfile userId={post.user_id} channelId={post.channel_id} hideStatus={true} disablePopover={true} />
          )
        }
        ...
        botIndicator = (
          <Badge className="col col__name">
            <FormattedMessage id="post_info.auto_responder" defaultMessage="AUTOMATIC REPLY" />
          </Badge>
        )
      } else if (isSystemMessage && this.props.isBot) {
        userProfile = <UserProfile userId={post.user_id} channelId={post.channel_id} hideStatus={true} />
    ...
    if (!isMobile() && !isEphemeral && !post.failed && !isSystemMessage) {
      flagIcon = <PostFlagIcon location={Locations.RHS_COMMENT} postId={post.id} isFlagged={this.props.isFlagged} />
    }
    ...
    if (isEphemeral) {
      ...
      options = (
        <div ref={this.dotMenuRef} className="col post-menu">
          {dotMenu}
          {postReaction}
          {flagIcon}
        </div>
    ...
    if (post.props && post.props.card) {
      postInfoIcon = (
        <OverlayTrigger
          ...
          overlay={
            <Tooltip>
              <FormattedMessage id="post_info.info.view_additional_info" defaultMessage="View additional info" />
            </Tooltip>
          }
        >
          <button
          ...
          >
            <InfoSmallIcon className="icon icon__info" aria-hidden="true" />
          </button>
        </OverlayTrigger>
      )
    }
    ...
    return (
      ...
      >
        <PostPreHeader isFlagged={this.props.isFlagged} isPinned={post.is_pinned} channelId={post.channel_id} />
        <div role="application" className="post__content">
          ...
          <div>
            ...
            <div className="post__body">
              ...
              {fileAttachment}
              <ReactionList post={post} isReadOnly={isReadOnly || channelIsArchived} />
            </div>
          </div>
        </div>
      </div>
