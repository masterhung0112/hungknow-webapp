...
export default class MoreChannels extends React.PureComponent<Props, State> {
  ...
  search = (term: string) => {
    ...
    const searchTimeoutId = window.setTimeout(async () => {
      try {
        const { data } = (await this.props.actions.searchMoreChannels(term, this.state.shouldShowArchivedChannels)) as {
          data: any
        }
  ...
  setSearchResults = (channels: Channel[]) => {
    this.setState({
      searchedChannels: this.state.shouldShowArchivedChannels
        ? channels.filter((c) => c.delete_at !== 0)
        : channels.filter((c) => c.delete_at === 0),
      searching: false,
    })
  }
  ...
  render() {
    const { channels, archivedChannels, teamId, channelsRequestStarted, bodyOnly } = this.props
    ...
    const createNewChannelButton = (
      <TeamPermissionGate teamId={teamId} permissions={[Permissions.CREATE_PUBLIC_CHANNEL]}>
        <button
          id="createNewChannel"
          type="button"
        ...
        >
          <FormattedMessage id="more_channels.create" defaultMessage="Create Channel" />
        </button>
      </TeamPermissionGate>
    )
    ...
    return (
      ...
      >
        <Modal.Header id="moreChannelsModalHeader" closeButton={true}>
          <Modal.Title componentClass="h1" id="moreChannelsModalLabel">
