...
export default class ChannelMembersDropdown extends React.PureComponent {
  ...
  renderRole(isChannelAdmin, isGuest) {
    if (isChannelAdmin) {
      return <FormattedMessage id="channel_members_dropdown.channel_admin" defaultMessage="Channel Admin" />
    } else if (isGuest) {
      return <FormattedMessage id="channel_members_dropdown.channel_guest" defaultMessage="Channel Guest" />
    }
    return <FormattedMessage id="channel_members_dropdown.channel_member" defaultMessage="Channel Member" />
  }
  ...
  render() {
    ...
    const canRemoveUserFromChannel =
      canRemoveMember && (!channel.group_constrained || user.is_bot) && (!isDefaultChannel || isGuest)

    if (canMakeUserChannelMember || canMakeUserChannelAdmin || canRemoveUserFromChannel) {
      return (
        <MenuWrapper>
          <button className="dropdown-toggle theme color--link style--none" type="button">
