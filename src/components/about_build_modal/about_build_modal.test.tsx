...
describe('components/AboutBuildModal', () => {
  ...
  test('should match snapshot for cloud edition', () => {
    ...
    const wrapper = shallow(
      <AboutBuildModalCloud config={config} license={license} show={true} onHide={jest.fn()} doHide={jest.fn()} />
    )
    expect(wrapper).toMatchSnapshot()
  })
  ...
  test('should call onHide callback when the modal is hidden', () => {
    ...
    const wrapper = mountWithIntl(
      <AboutBuildModal config={config} license={license} webappBuildHash="0a1b2c3d4f" show={true} onHide={onHide} />
    )
  ...
  test('should show default tos and privacy policy links and not the config links', () => {
    const wrapper = mountWithIntl(<AboutBuildModal config={config} license={license} show={true} onHide={jest.fn()} />)
