...
import { ProductNotices, ProductNotice } from 'hkclient-ts/lib/types/product_notices'
...
export default class ProductNoticesModal extends React.PureComponent<Props, State> {
  ...
  private confirmButtonText(presentNoticeInfo: ProductNotice) {
    ...
    if (noOfNotices === 1 && presentNoticeInfo.actionText) {
      return <span>{presentNoticeInfo.actionText}</span>
    } else if (noOfNotices === this.state.presentNoticeIndex + 1) {
      return <FormattedMessage id={'generic.done'} defaultMessage="Done" />
    }
    return (
      <>
  ...
  private renderCicrleIndicators() {
    ...
    for (let i = 0; i < noOfNotices; i++) {
      ...
      indicators.push(<span id={'tutorialIntroCircle' + i} key={'circle' + i} className={className} data-screen={i} />)
    }
    return <span className="tutorial__circles">{indicators}</span>
  }
  ...
  private renderAdminOnlyText() {
    if (this.state.noticesData[this.state.presentNoticeIndex].sysAdminOnly) {
      return (
        <>
          <AdminEyeIcon />
          <FormattedMessage id={'inProduct_notices.adminOnlyMessage'} defaultMessage="Visible to Admins only" />
        </>
      )
    }
    return null
  ...
  private renderImage(image: string | undefined) {
    if (image) {
      return <img className="productNotices__img" src={image} />
    }
    return null
  }
  ...
  render() {
    ...
    return (
      <GenericModal
        ...
        modalHeaderText={<span>{presentNoticeInfo.title}</span>}
      ...
      >
        <span className="productNotices__helpText">
          <Markdown message={presentNoticeInfo.description} />
        </span>
        {this.renderActionButton(presentNoticeInfo)}
        <div className="productNotices__imageDiv">{this.renderImage(presentNoticeInfo.image)}</div>
