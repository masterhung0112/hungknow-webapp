...
import { rudderAnalytics, RudderTelemetryHandler } from 'hkclient-ts/lib/client/rudder'
import { Client4 } from 'hkclient-ts/lib/client'
import { setUrl } from 'hkclient-ts/lib/actions/general'
import { setSystemEmojis } from 'hkclient-ts/lib/actions/emojis'
    ...
    return (
      <IntlProvider>
        <Switch>
          ...
          />
          <Redirect from={'/_redirect/pl/:postid'} to={`/${this.props.permalinkRedirectTeamName}/pl/:postid`} />
          {this.props.plugins?.map((plugin) => (
            <Route
              key={plugin.id}
              path={'/plug/' + plugin.route}
              render={() => <Pluggable pluggableName={'CustomRouteComponent'} pluggableId={plugin.id} />}
            />
          ))}
