...
import { Audit } from 'hkclient-ts/lib/types/audits'
  ...
  render() {
    ...
    for (const [i, audit] of audits.entries()) {
      ...
      accessList[i] = (
        <tr key={audit.id}>
          ...
          {uContent}
          <td className={'word-break--all' + descStyle}>{auditInfo.desc}</td>
          {iContent}
          {sContent}
        </tr>
      )
    ...
    return (
      <table className="table">
        ...
        <tbody data-testid="auditTableBody">{accessList}</tbody>
      </table>
    )
  }
