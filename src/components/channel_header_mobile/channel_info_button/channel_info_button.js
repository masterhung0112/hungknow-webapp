...
export default class NavbarInfoButton extends React.PureComponent {
  ...
  render() {
    ...
    if (channel.header) {
      popoverContent = <Markdown message={channel.header} options={headerMarkdownOptions} />
    } else {
      let addOne
      if (!isReadOnly) {
        const link = (
      ...
      popoverContent = (
        <div>
          <FormattedMessage id="navbar.noHeader" defaultMessage="No channel header yet." />
          {addOne}
        </div>
      )
    }
    ...
    const popover = (
      <Popover popoverStyle="info" placement="bottom" className="navbar__popover" id="header-popover">
        <span onClick={this.handleFormattedTextClick}>{popoverContent}</span>
