...
export default function Describe({
  ...
  if (memberNotifyLevel === NotificationLevels.DEFAULT && globalNotifyLevel) {
    ...
    const notifyLevel = <FormattedMessage id={levelsFormattedMessageId} defaultMessage={globalNotifyLevel} />
    return (
      <FormattedMessage
    ...
    return <FormattedMessage id="channel_notifications.muteChannel.on.title" defaultMessage="On" />
  } else if (
    section === NotificationSections.IGNORE_CHANNEL_MENTIONS &&
    ignoreChannelMentions === IgnoreChannelMentions.ON
  ) {
    return <FormattedMessage id="channel_notifications.ignoreChannelMentions.on.title" defaultMessage="On" />
  } else if (
    section === NotificationSections.IGNORE_CHANNEL_MENTIONS &&
    ignoreChannelMentions === IgnoreChannelMentions.OFF
  ) {
    return <FormattedMessage id="channel_notifications.ignoreChannelMentions.off.title" defaultMessage="Off" />
  } else if (memberNotifyLevel === NotificationLevels.MENTION) {
    return <FormattedMessage id="channel_notifications.onlyMentions" defaultMessage="Only for mentions" />
  } else if (
    (section === NotificationSections.DESKTOP || section === NotificationSections.PUSH) &&
    memberNotifyLevel === NotificationLevels.ALL
  ) {
    return <FormattedMessage id="channel_notifications.allActivity" defaultMessage="For all activity" />
  } else if (section === NotificationSections.MARK_UNREAD && memberNotifyLevel === NotificationLevels.ALL) {
    return <FormattedMessage id="channel_notifications.muteChannel.off.title" defaultMessage="Off" />
  }

  return <FormattedMessage id="channel_notifications.never" defaultMessage="Never" />
}
