...
describe('components/activity_log_modal/ActivityLog', () => {
  ...
  test('should match when mobileSessionInfo is called', () => {
    ...
    const appleText = (
      <FormattedMessage defaultMessage="iPhone Native Classic App" id="activity_log_modal.iphoneNativeClassicApp" />
    )
    const apple = {
      devicePicture: 'fa fa-apple',
      deviceTitle: localizeMessage('device_icons.apple', 'Apple Icon'),
      devicePlatform: appleText,
    }
    expect(mobileSessionInfo({ device_id: 'apple' })).toEqual(apple)

    const androidText = (
      <FormattedMessage defaultMessage="Android Native Classic App" id="activity_log_modal.androidNativeClassicApp" />
    )
    const android = {
      devicePicture: 'fa fa-android',
      deviceTitle: localizeMessage('device_icons.android', 'Android Icon'),
      devicePlatform: androidText,
    }
    expect(mobileSessionInfo({ device_id: 'android' })).toEqual(android)

    const appleRNText = <FormattedMessage defaultMessage="iPhone Native App" id="activity_log_modal.iphoneNativeApp" />
    const appleRN = {
      devicePicture: 'fa fa-apple',
      deviceTitle: localizeMessage('device_icons.apple', 'Apple Icon'),
      devicePlatform: appleRNText,
    }
    expect(mobileSessionInfo({ device_id: 'apple_rn' })).toEqual(appleRN)

    const androidRNText = (
      <FormattedMessage defaultMessage="Android Native App" id="activity_log_modal.androidNativeApp" />
    )
    const androidRN = {
      devicePicture: 'fa fa-android',
      deviceTitle: localizeMessage('device_icons.android', 'Android Icon'),
      devicePlatform: androidRNText,
    }
