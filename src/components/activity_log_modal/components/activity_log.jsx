...
export default class ActivityLog extends React.PureComponent {
  ...
  render() {
    ...
    if (currentSession.props.browser.indexOf('Desktop App') !== -1) {
      devicePlatform = <FormattedMessage id="activity_log_modal.desktop" defaultMessage="Native Desktop App" />
    }

    return (
      <div key={'activityLogEntryKey' + index} className="activity-log__table">
          ...
          <div className="report__info">
            <div>
              <FormattedMessage
                ...
                values={{
                  date: (
                    <FormattedDate value={lastAccessTime} day="2-digit" month={getMonthLong(locale)} year="numeric" />
                  ),
        ...
        <div className="activity-log__action">
          <button onClick={this.submitRevoke} className="btn btn-primary">
            <FormattedMessage id="activity_log.logout" defaultMessage="Log Out" />
          </button>
        </div>
      </div>
    )
