...
import { getEmailInterval } from 'hkclient-ts/lib/utils/notify_props'
...
export default class EmailNotificationSetting extends React.PureComponent<Props, State> {
  constructor(props: Props) {
    ...
    const { emailInterval, enableEmail, enableEmailBatching, sendEmailNotifications, activeSection } = props
    ...
    this.state = {
      ...
      newInterval: getEmailInterval(
        JSON.parse(enableEmail) && sendEmailNotifications,
        enableEmailBatching,
        emailInterval
      ),
    }
  ...
  static getDerivedStateFromProps(nextProps: Props, prevState: State) {
    const { emailInterval, enableEmail, enableEmailBatching, sendEmailNotifications, activeSection } = nextProps
    ...
    if (!sendEmailNotifications) {
      ...
      switch (newInterval) {
        case Preferences.INTERVAL_IMMEDIATE:
          description = (
            <FormattedMessage id="user.settings.notifications.email.immediately" defaultMessage="Immediately" />
          )
          break
        case Preferences.INTERVAL_HOUR:
          description = (
            <FormattedMessage id="user.settings.notifications.email.everyHour" defaultMessage="Every hour" />
          )
          break
        ...
        inputs={[
          <fieldset key="userNotificationEmailOptions">
            <legend className="form-legend">
              <FormattedMessage id="user.settings.notifications.email.send" defaultMessage="Send email notifications" />
            </legend>
            <div className="radio">
              <label>
                <input
