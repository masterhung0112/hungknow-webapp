...
export class UserSettingsGeneralTab extends React.Component<Props, State> {
    ...
    return (
      <span className="resend-verification-wrapper">
        ...
        >
          ...
          >
            <FormattedMessage id="user.settings.general.sendAgain" defaultMessage="Send again" />
          </a>
        </LoadingWrapper>
      </span>
    )
  ...
  submitPicture = () => {
    ...
    this.props.actions.uploadProfileImage(this.props.user.id, file).then(({ data, error: err }) => {
      if (data) {
        this.updateSection('')
        this.submitActive = false
      } else if (err) {
      ...
      if (this.props.user.auth_service === '') {
          ...
          <div key="currentEmailSetting">
            <div className="form-group">
              <label className="col-sm-5 control-label">
                <FormattedMessage id="user.settings.general.currentEmail" defaultMessage="Current Email" />
              </label>
        ...
        inputs.push(
          <div key="confirmEmailSetting">
            <div className="form-group">
              <label className="col-sm-5 control-label">
                <FormattedMessage id="user.settings.general.confirmEmail" defaultMessage="Confirm Email" />
              </label>
              <div className="col-sm-7">
                <input
                  id="confirmEmail"
        ...
        inputs.push(
          <div key="currentPassword">
            <div className="form-group">
              <label className="col-sm-5 control-label">
                <FormattedMessage id="user.settings.general.currentPassword" defaultMessage="Current Password" />
              </label>
              <div className="col-sm-7">
                <input
                  id="currentPassword"
  ...
  render() {
    ...
    if (this.props.activeSection === 'name') {
      ...
      if (user.first_name && user.last_name) {
        ...
        describe = (
          <FormattedMessage id="user.settings.general.emptyName" defaultMessage="Click 'Edit' to add your full name" />
        )
        if (Utils.isMobile()) {
          describe = (
            <FormattedMessage
    ...
    if (this.props.activeSection === 'nickname') {
      ...
      let submit = null
      if (
        (this.props.user.auth_service === 'ldap' && this.props.ldapNicknameAttributeSet) ||
        ...
        let nicknameLabel: JSX.Element | string = (
          <FormattedMessage id="user.settings.general.nickname" defaultMessage="Nickname" />
        )
        if (Utils.isMobile()) {
          nicknameLabel = ''
        }
      ...
      if (user.nickname) {
        ...
        describe = (
          <FormattedMessage id="user.settings.general.emptyNickname" defaultMessage="Click 'Edit' to add a nickname" />
        )
        if (Utils.isMobile()) {
          describe = (
            <FormattedMessage
    ...
    if (this.props.activeSection === 'username') {
      ...
      if (this.props.user.auth_service === '') {
        let usernameLabel: JSX.Element | string = (
          <FormattedMessage id="user.settings.general.username" defaultMessage="Username" />
        )
        if (Utils.isMobile()) {
          usernameLabel = ''
        }
    ...
    if (this.props.activeSection === 'position') {
      ...
      let submit = null
      if (
        (this.props.user.auth_service === Constants.LDAP_SERVICE && this.props.ldapPositionAttributeSet) ||
        (this.props.user.auth_service === Constants.SAML_SERVICE && this.props.samlPositionAttributeSet)
      ) {
        extraInfo = (
          <span>
            <FormattedMessage
        ...
        let positionLabel: JSX.Element | string = (
          <FormattedMessage id="user.settings.general.position" defaultMessage="Position" />
        )
        if (Utils.isMobile()) {
          positionLabel = ''
        }
      ...
      if (user.last_picture_update) {
        minMessage = (
          <FormattedMessage
            ...
            values={{
              date: (
                <FormattedDate value={new Date(user.last_picture_update)} day="2-digit" month="short" year="numeric" />
              ),
            }}
          />
        )
          ...
          <h4 className="modal-title" ref="title">
            <div className="modal-back">
              <FormattedMessage id="generic_icons.collapse" defaultMessage="Collapse Icon">
                {(title?: string) => (
                  <i className="fa fa-angle-left" title={title} onClick={this.props.collapseModal} />
                )}
              </FormattedMessage>
            </div>
