...
import { getTimezoneRegion } from 'hkclient-ts/lib/utils/timezone_utils'
import { PreferenceType } from 'hkclient-ts/lib/types/preferences'
import { UserProfile, UserTimezone } from 'hkclient-ts/lib/types/users'
...
export default class UserSettingsDisplay extends React.PureComponent<Props, State> {
  ...
  createSection(props: SectionProps) {
    const { section, display, value, title, firstOption, secondOption, thirdOption, description, disabled } = props
    let extraInfo = null
    let submit: (() => Promise<void>) | null = this.handleSubmit

    const firstMessage = (
      <FormattedMessage id={firstOption.radionButtonText.id} defaultMessage={firstOption.radionButtonText.message} />
    )
    ...
    const secondMessage = (
      <FormattedMessage id={secondOption.radionButtonText.id} defaultMessage={secondOption.radionButtonText.message} />
    )
    ...
    if (thirdOption) {
      thirdMessage = (
        <FormattedMessage id={thirdOption.radionButtonText.id} defaultMessage={thirdOption.radionButtonText.message} />
      )
    }

    const messageTitle = <FormattedMessage id={title.id} defaultMessage={title.message} />

    const messageDesc = <FormattedMessage id={description.id} defaultMessage={description.message} />
    ...
    if (this.props.activeSection === section) {
      ...
      let inputs = [
        <fieldset key={key}>
          <legend className="form-legend hidden-label">{messageTitle}</legend>
          <div className="radio">
            <label>
              <input
                id={name + 'A'}
    ...
    return (
      <div>
        <SettingItemMin title={messageTitle} describe={describe} section={section} updateSection={this.updateSection} />
        <div className="divider-dark" />
      </div>
    )
  }
  ...
  render() {
    ...
    if (this.props.enableTimezone && !this.props.shouldAutoUpdateTimezone) {
      ...
      if (this.props.activeSection === 'timezone') {
        ...
        timezoneSelection = (
          <div>
            <SettingItemMin
              title={<FormattedMessage id="user.settings.display.timezone" defaultMessage="Timezone" />}
    ...
    if (this.props.activeSection === 'languages') {
      ...
      languagesSection = (
        <div>
          <ManageLanguages user={this.props.user} locale={userLocale} updateSection={this.updateSection} />
          <div className="divider-dark" />
        </div>
      )
    } else {
