...
export default class MfaSection extends React.PureComponent<Props, State> {
  ...
  private renderTitle = () => {
    return <FormattedMessage id="user.settings.mfa.title" defaultMessage="Multi-factor Authentication" />
  }

  private renderDescription = () => {
    if (this.props.mfaActive) {
      return <FormattedMessage id="user.settings.security.active" defaultMessage="Active" />
    }

    return <FormattedMessage id="user.settings.security.inactive" defaultMessage="Inactive" />
  }
  ...
  private renderContent = () => {
    ...
    if (this.props.mfaActive) {
      ...
      if (this.props.mfaEnforced) {
        buttonText = <FormattedMessage id="user.settings.mfa.reset" defaultMessage="Reset MFA on Account" />
      } else {
        buttonText = <FormattedMessage id="user.settings.mfa.remove" defaultMessage="Remove MFA from Account" />
      }
      ...
      content = (
        <a className="btn btn-primary" href="#" onClick={this.setupMfa}>
          <FormattedMessage id="user.settings.mfa.add" defaultMessage="Add MFA to Account" />
        </a>
      )
    }
