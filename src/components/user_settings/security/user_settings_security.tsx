...
export default class SecurityTab extends React.PureComponent<Props, State> {
      ...
      if (this.props.user.auth_service === '') {
        ...
        inputs.push(
          <div key="currentPasswordUpdateForm" className="form-group">
            <label className="col-sm-5 control-label">
              <FormattedMessage id="user.settings.security.currentPassword" defaultMessage="Current Password" />
            </label>
        ...
        inputs.push(
          <div key="retypeNewPasswordUpdateForm" className="form-group">
            <label className="col-sm-5 control-label">
              <FormattedMessage id="user.settings.security.retypePassword" defaultMessage="Retype New Password" />
            </label>
            <div className="col-sm-7">
              <input
                id="confirmPassword"
      ...
      describe = (
        <FormattedMessage
          ...
          values={{
            ...
            time: <FormattedTime value={d} hour12={!this.props.militaryTime} hour="2-digit" minute="2-digit" />,
          }}
        />
      )
    } else if (this.props.user.auth_service === Constants.GITLAB_SERVICE) {
      describe = <FormattedMessage id="user.settings.security.loginGitlab" defaultMessage="Login done through GitLab" />
    } else if (this.props.user.auth_service === Constants.LDAP_SERVICE) {
      describe = <FormattedMessage id="user.settings.security.loginLdap" defaultMessage="Login done through AD/LDAP" />
    } else if (this.props.user.auth_service === Constants.SAML_SERVICE) {
      describe = <FormattedMessage id="user.settings.security.loginSaml" defaultMessage="Login done through SAML" />
    } else if (this.props.user.auth_service === Constants.GOOGLE_SERVICE) {
      describe = (
        <FormattedMessage id="user.settings.security.loginGoogle" defaultMessage="Login done through Google Apps" />
      )
    } else if (this.props.user.auth_service === Constants.OFFICE365_SERVICE) {
      describe = (
        <FormattedMessage id="user.settings.security.loginOffice365" defaultMessage="Login done through Office 365" />
      )
    }
  ...
  createSignInSection = () => {
    ...
    if (this.props.activeSection === SECTION_SIGNIN) {
      ...
      if (user.auth_service === '') {
        ...
        if (this.props.enableLdap) {
          ldapOption = (
            <div className="pb-3">
              <Link className="btn btn-primary" to={'/claim/email_to_ldap?email=' + encodeURIComponent(user.email)}>
                <FormattedMessage id="user.settings.security.switchLdap" defaultMessage="Switch to Using AD/LDAP" />
              </Link>
              <br />
            </div>
          )
        ...
        if (this.props.enableSaml) {
          samlOption = (
            <div className="pb-3">
              ...
              >
                <FormattedMessage id="user.settings.security.switchSaml" defaultMessage="Switch to Using SAML SSO" />
              </Link>
              <br />
            </div>
          )
    ...
    if (this.props.user.auth_service === Constants.GITLAB_SERVICE) {
      ...
      describe = <FormattedMessage id="user.settings.security.openid" defaultMessage="OpenID" />
    } else if (this.props.user.auth_service === Constants.LDAP_SERVICE) {
              ...
              <div className="col-sm-10">
                ...
                <div className="authorized-app__deauthorize">
                  <a href="#" data-app={app.id} onClick={this.deauthorizeApp}>
                    <FormattedMessage id="user.settings.security.deauthorize" defaultMessage="Deauthorize" />
                  </a>
                </div>
              </div>
    ...
    return (
      <div>
        <div className="modal-header">
          <FormattedMessage id="user.settings.security.close" defaultMessage="Close">
            {(ariaLabel: string) => (
              <button
                type="button"
                className="close"
          ...
          <h4 className="modal-title" ref="title">
            <div className="modal-back">
              <FormattedMessage id="generic_icons.collapse" defaultMessage="Collapse Icon">
                {(title: string) => <i className="fa fa-angle-left" title={title} onClick={this.props.collapseModal} />}
              </FormattedMessage>
            </div>
        ...
        <div className="user-settings">
          ...
          <MfaSection active={this.props.activeSection === SECTION_MFA} updateSection={this.handleUpdateSection} />
          <div className="divider-light" />
          {oauthSection}
          <div className="divider-light" />
          {tokensSection}
          ...
          >
            <FormattedMessage id="user.settings.security.viewHistory.icon" defaultMessage="Access History Icon">
              {(title: string) => <i className="fa fa-clock-o" title={title} />}
            </FormattedMessage>
            <FormattedMessage id="user.settings.security.viewHistory" defaultMessage="View Access History" />
          </ToggleModalButton>
          <ToggleModalButton
