...
export default class EmojiPickerOverlay extends React.PureComponent {
  ...
  static emojiPickerPosition(props) {
    ...
    if (emojiTrigger) {
      rightOffset =
        window.innerWidth - emojiTrigger.getBoundingClientRect().left - Constants.DEFAULT_EMOJI_PICKER_LEFT_OFFSET
  ...
  static getPlacement(props) {
    ...
    if (target) {
      ...
      return popOverOverlayPosition(
        targetBounds,
        window.innerHeight,
        props.spaceRequiredAbove,
        props.spaceRequiredBelow,
        props.defaultHorizontalPosition
      )
    }
