...
export default class EmojiPicker extends React.PureComponent {
  ...
  constructor(props) {
    ...
    this.handleScrollThrottle = throttle(this.handleScroll, EMOJI_LAZY_LOAD_SCROLL_THROTTLE, {
      leading: false,
      trailing: true,
    })
    ...
    const categories = props.recentEmojis.length
      ? { ...recentEmojiCategory, ...peopleEmojiCategory }
      : peopleEmojiCategory
    this.state = {
      allEmojis: {},
      categories,
  ...
  updateEmojisToShow(divTopOffset) {
    ...
    if (this.lastVisibleEmoji) {
      const difference =
        this.lastVisibleEmoji.offsetTop - (divTopOffset + EMOJI_CONTAINER_HEIGHT + EMOJI_LAZY_LOAD_BUFFER)
      if (difference <= 0) {
  ...
  componentDidUpdate(prevProps) {
    ...
    const pixelsFromBottom =
      this.emojiPickerContainer.scrollHeight - this.state.divTopOffset - this.emojiPickerContainer.clientHeight
  ...
  handleKeyDown = (e) => {
    switch (e.key) {
      case 'ArrowRight':
        if (
          this.state.cursor[0] !== -1 ||
          this.state.cursor[1] !== -1 ||
          e.target.selectionStart + 1 > this.props.filter.length
        ) {
          e.preventDefault()
          this.selectNextEmoji()
        }
  ...
  emojiCategories() {
    ...
    const emojiPickerCategories = categoryKeys.map((categoryName) => {
      ...
      return (
        <EmojiPickerCategory
          ...
          icon={
            <FormattedMessage id={category.id} defaultMessage={category.message}>
              {(title) => <i className={category.className} title={title} />}
            </FormattedMessage>
          }
    ...
    return (
      <div id="emojiPickerCategories" className="emoji-picker__categories" onKeyDown={this.handleCategoryKeyDown}>
  ...
  emojiSearch() {
    return (
      <div className="emoji-picker__search-container">
        ...
        <FormattedMessage id="emoji_picker.search_emoji" defaultMessage="Search for an emoji">
          {(ariaLabel) => (
            <LocalizedInput
  ...
  emojiCurrentResults() {
    ...
    return (
      ...
      >
        <div className="emoji-picker__container">{categoryComponents}</div>
      </div>
    )
  }
  emojiCurrentResultsItems = (categoryIndex, emojis, currentEmojiLoadedCount) => {
    ...
    return emojis.map((emoji, emojiIndex) => {
      ...
      if (numEmojisLoaded >= emojisToShow) {
        return (
          <div key={numEmojisLoaded} className="emoji-picker__item" ref={ref}>
            <img alt={'emoji image'} src={imgTrans} className="emojisprite" />
          </div>
        )
      }
      return (
