...
export default class EmailToLDAP extends React.PureComponent {
  ...
  render() {
    ...
    if (this.state.ldapError) {
      ldapError = (
        <div className="form-group has-error">
          <label className="control-label">{this.state.ldapError}</label>
        </div>
      )
      ldapClass += ' has-error'
    }
    ...
    if (this.state.showMfa) {
      ...
      content = (
        <form onSubmit={this.preSubmit} className={formClass}>
          ...
          <input type="text" style={style.usernameInput} name="fakeusernameremembered" />
          ...
          <button type="submit" className="btn btn-primary">
            <FormattedMessage id="claim.email_to_ldap.switchTo" defaultMessage="Switch Account to AD/LDAP" />
          </button>
          {serverError}
        </form>
      )
    ...
    return (
      <div>
        <h3>
          <FormattedMessage id="claim.email_to_ldap.title" defaultMessage="Switch Email/Password Account to AD/LDAP" />
        </h3>
        {content}
      </div>
    )
