...
export default class LDAPToEmail extends React.PureComponent {
  ...
  submit = (loginId, password, token, ldapPassword) => {
    this.props
      .switchLdapToEmail(ldapPassword || this.state.ldapPassword, this.props.email, password, token)
      .then(({ data, error: err }) => {
  ...
  render() {
    ...
    if (this.state.passwordError) {
      passwordError = (
        <div className="form-group has-error">
          <label className="control-label">{this.state.passwordError}</label>
        </div>
      )
    ...
    if (this.state.ldapPasswordError) {
      ldapPasswordError = (
        <div className="form-group has-error">
          <label className="control-label">{this.state.ldapPasswordError}</label>
        </div>
      )
    ...
    if (this.state.confirmError) {
      confirmError = (
        <div className="form-group has-error">
          <label className="control-label">{this.state.confirmError}</label>
        </div>
      )
    ...
    if (this.state.showMfa) {
      ...
      content = (
        <form onSubmit={this.preSubmit} className={formClass}>
          ...
          {ldapPasswordError}
          <p>
            <FormattedMessage id="claim.ldap_to_email.enterPwd" defaultMessage="New email login password:" />
          </p>
          ...
          <button type="submit" className="btn btn-primary">
            <FormattedMessage id="claim.ldap_to_email.switchTo" defaultMessage="Switch account to email/password" />
          </button>
          {serverError}
        </form>
      )
    ...
    return (
      <div>
        <h3>
          <FormattedMessage id="claim.ldap_to_email.title" defaultMessage="Switch AD/LDAP Account to Email/Password" />
        </h3>
        {content}
      </div>
    )
