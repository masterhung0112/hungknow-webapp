...
class FaviconTitleHandler extends React.PureComponent<Props> {
  ...
  updateTitle = () => {
    const { siteName, currentChannel, currentTeam, currentTeammate, unreads } = this.props
    ...
    if (currentChannel && currentTeam && currentChannel.id) {
      ...
      document.title =
        mentionTitle + unreadTitle + currentChannelName + ' - ' + currentTeam.display_name + ' ' + currentSiteName
    } else {
