...
describe('components/integrations/bots/Bot', () => {
  ...
  it('regular bot', () => {
    ...
    const wrapper = shallow(
      <Bot bot={bot} user={user} owner={undefined} accessTokens={{}} team={team} actions={actions} />
    )
    ...
    expect(
      wrapper.contains(<FormattedMessage id="bot.manage.create_token" defaultMessage="Create New Token" />)
    ).toEqual(false)
    expect(wrapper.contains(<FormattedMessage id="bots.manage.edit" defaultMessage="Edit" />)).toEqual(false)
    expect(wrapper.contains(<FormattedMessage id="bot.manage.disable" defaultMessage="Disable" />)).toEqual(false)
    expect(wrapper.contains(<FormattedMessage id="bot.manage.enable" defaultMessage="Enable" />)).toEqual(false)
  })
  ...
  it('disabled bot', () => {
    ...
    expect(wrapper.contains(<FormattedMessage id="bots.manage.edit" defaultMessage="Edit" />)).toEqual(false)
    expect(wrapper.contains(<FormattedMessage id="bot.manage.disable" defaultMessage="Disable" />)).toEqual(false)
    expect(wrapper.contains(<FormattedMessage id="bot.manage.enable" defaultMessage="Enable" />)).toEqual(true)
  })
  ...
  it('bot with owner', () => {
    ...
    const user = UtilsTestHelper.getUserMock({ id: bot.user_id })
    const wrapper = shallow(<Bot bot={bot} owner={owner} user={user} accessTokens={{}} team={team} actions={actions} />)
    ...
    expect(wrapper.contains(<FormattedMessage id="bots.manage.edit" defaultMessage="Edit" />)).toEqual(true)
    expect(wrapper.contains(<FormattedMessage id="bot.manage.disable" defaultMessage="Disable" />)).toEqual(true)
  })
  ...
  it('bot with access tokens', () => {
    ...
    const wrapper = shallow(
      <Bot bot={bot} owner={undefined} user={user} accessTokens={accessTokens} team={team} actions={actions} />
    )

    expect(wrapper.contains(tokenId)).toEqual(true)
    expect(
      wrapper.contains(<FormattedMessage id="user.settings.tokens.deactivate" defaultMessage="Disable" />)
    ).toEqual(true)
    expect(wrapper.contains(<FormattedMessage id="user.settings.tokens.activate" defaultMessage="Enable" />)).toEqual(
      false
    )
  })
