...
describe('components/integrations/bots/Bots', () => {
  ...
  it('bots', () => {
    ...
    const wrapperFull = shallow(
      <Bots bots={bots} team={team} accessTokens={{}} owners={{}} users={users} actions={actions} />
    )
  ...
  it('bot owner tokens', () => {
    ...
    const wrapperFull = shallow(
      <Bots bots={bots} team={team} accessTokens={tokens} owners={owners} users={users} actions={actions} />
    )
    wrapperFull.instance().setState({ loading: false })
    const wrapper = shallow(<div>{(wrapperFull.instance() as Bots).bots()[0]}</div>)
