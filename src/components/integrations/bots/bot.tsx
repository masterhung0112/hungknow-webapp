...
import { Bot as BotType } from 'hkclient-ts/lib/types/bots'
import { UserProfile, UserAccessToken } from 'hkclient-ts/lib/types/users'
...
export default class Bot extends React.PureComponent<Props, State> {
  ...
  handleCreateToken = async (e: SyntheticEvent): Promise<void> => {
    ...
    if (this.state.token.description === '') {
      this.setState({
        error: <FormattedMessage id="bot.token.error.description" defaultMessage="Please enter a description." />,
      })
      return
    }
  ...
  public render(): JSX.Element | null {
    ...
    Object.values(this.props.accessTokens).forEach((token) => {
      ...
      if (token.is_active) {
        activeLink = (
          ...
          >
            <FormattedMessage id="user.settings.tokens.deactivate" defaultMessage="Disable" />
          </a>
        )
      } else {
        disableClass = 'light'
        disabledText = (
          <span className="mr-2 light">
            <FormattedMessage id="user.settings.tokens.deactivatedWarning" defaultMessage="(Disabled)" />
          </span>
        )
        activeLink = (
          <a
          ...
          >
            <FormattedMessage id="user.settings.tokens.activate" defaultMessage="Enable" />
          </a>
        )
      }
          ...
          <div className="item-details__row d-flex justify-content-between">
            <div className={disableClass}>
              <div className="whitespace--nowrap overflow--ellipsis">
                <b>
                  <FormattedMessage id="user.settings.tokens.tokenDesc" defaultMessage="Token Description: " />
                </b>
                {token.description}
              </div>
    ...
    if (ownerUsername !== 'plugin') {
      options = (
        <div className="item-actions">
          <button id="createToken" className="style--none color--link" onClick={this.openCreateToken}>
            <FormattedMessage id="bot.manage.create_token" defaultMessage="Create New Token" />
          </button>
          {' - '}
          ...
          {' - '}
          <button className="style--none color--link" onClick={this.disableBot}>
            <FormattedMessage id="bot.manage.disable" defaultMessage="Disable" />
          </button>
        </div>
      )
    }
    if (this.props.bot.delete_at !== 0) {
      options = (
        <div className="item-actions">
          <button className="style--none color--link" onClick={this.enableBot}>
            <FormattedMessage id="bot.manage.enable" defaultMessage="Enable" />
          </button>
        </div>
      )
    }
    ...
    if (this.state.creatingTokenState === 'OPEN') {
      tokenList.push(
        <div key={'create'} className="bot-list__item">
          <div key={'create'}>
            <form className="form-horizontal" onSubmit={this.handleCreateToken}>
              <div className="row">
              ...
              <div>
                ...
                <label id="clientError" className="has-error is-empty">
                  {this.state.error}
                </label>
                <div className="mt-2">
                  <SaveButton
                    btnClass="btn-sm btn-primary"
                    savingMessage={<FormattedMessage id="user.settings.tokens.save" defaultMessage="Save" />}
                    saving={false}
                  />
                  <button className="btn btn-sm btn-link" onClick={this.closeCreateToken}>
      ...
      tokenList.push(
        <div key={'created'} className="bot-list__item alert alert-warning">
          <div className="mb-2">
          ...
          <div className="mt-2">
            <button className="btn btn-sm btn-primary" onClick={this.closeCreateToken}>
              <FormattedMessage id="bot.create_token.close" defaultMessage="Close" />
            </button>
          </div>
        </div>
      )
    ...
    return (
      <div className="backstage-list__item">
        <div className={'bot-list-img-container'}>
          <img className={'bot-list-img'} alt={'bot image'} src={imageURL} />
        </div>
        <div className="item-details">
          <div className="item-details__row d-flex flex-column flex-md-row justify-content-between">
            <strong className="item-details__name">{displayName + ' (@' + username + ')'}</strong>
            {options}
          </div>
          ...
          <div className="light small">
            <FormattedMessage id="bots.managed_by" defaultMessage="Managed by " />
            {ownerUsername}
          </div>
          <div className="bot-list is-empty">{tokenList}</div>
        </div>
        <ConfirmModal
          title={<FormattedMessage id="bots.token.delete" defaultMessage="Delete Token" />}
          message={
            <FormattedMessage
          ...
          confirmButtonText={<FormattedMessage id="bots.token.confirm" defaultMessage="Delete" />}
