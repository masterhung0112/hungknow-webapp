...
export default class Bots extends React.PureComponent<Props, State> {
  ...
  public componentDidMount(): void {
    this.props.actions
      .loadBots(Constants.Integrations.START_PAGE_NUM, parseInt(Constants.Integrations.PAGE_SIZE, 10))
      .then((result) => {
        if (result.data) {
          const promises = []
    ...
    return (
      <React.Fragment>
        <div className="bot-disabled">
          <FormattedMessage id="bots.disabled" defaultMessage="Disabled" />
        </div>
  ...
  bots = (filter?: string): Array<boolean | JSX.Element> => {
    ...
    const sections = (
      <div key="sections">
        ...
        <this.DisabledSection hasDisabled={disabledBots.length > 0} disabledBots={disabledBots} />
      </div>
    )
  ...
  public render(): JSX.Element {
    return (
      <BackstageList
        header={<FormattedMessage id="bots.manage.header" defaultMessage="Bot Accounts" />}
        addText={this.props.createBots && <FormattedMessage id="bots.manage.add" defaultMessage="Add Bot Account" />}
        addLink={'/' + this.props.team.name + '/integrations/bots/add'}
        addButtonId="addBotAccount"
        emptyText={<FormattedMessage id="bots.manage.empty" defaultMessage="No bot accounts found" />}
        emptyTextSearch={
        ...
        helpText={
          <React.Fragment>
            <FormattedMessage
              ...
              values={{
                botAccounts: (
                  <a target="_blank" rel="noopener noreferrer" href="https://mattermost.com/pl/default-bot-accounts">
                    <FormattedMessage id="bots.manage.bot_accounts" defaultMessage="Bot Accounts" />
                  </a>
                ),
              }}
            />
            <FormattedMarkdownMessage
              ...
              defaultMessage={
                'Enable bot account creation in the [System Console]({siteURL}/admin_console/integrations/bot_accounts).'
              }
              values={{
                siteURL: getSiteURL(),
              }}
