...
describe('components/integrations/AbstractCommand', () => {
  ...
  test('should match state when handleSubmit is called', () => {
    ...
    handleSubmit(evt)
    expect(wrapper.state('clientError')).toEqual(
      <FormattedMessage defaultMessage="A trigger word is required" id="add_command.triggerRequired" />
    )
    expect(newAction).toHaveBeenCalledTimes(1)
    ...
    handleSubmit(evt)
    expect(wrapper.state('clientError')).toEqual(
      <FormattedMessage defaultMessage="A trigger word cannot begin with a /" id="add_command.triggerInvalidSlash" />
    )
    expect(newAction).toHaveBeenCalledTimes(1)
    ...
    handleSubmit(evt)
    expect(wrapper.state('clientError')).toEqual(
      <FormattedMessage defaultMessage="A trigger word must not contain spaces" id="add_command.triggerInvalidSpace" />
    )
    expect(newAction).toHaveBeenCalledTimes(1)
    ...
    handleSubmit(evt)
    expect(wrapper.state('clientError')).toEqual(
      <FormattedMessage defaultMessage="A request URL is required" id="add_command.urlRequired" />
    )
    expect(newAction).toHaveBeenCalledTimes(4)
  })
})
