...
export function matchesFilter(outgoingWebhook, channel, filter) {
  ...
  const { display_name: displayName, description, trigger_words: triggerWords } = outgoingWebhook
...
export default class InstalledOutgoingWebhook extends React.PureComponent {
  ...
  makeDisplayName(outgoingWebhook, channel) {
    ...
    return <FormattedMessage id="installed_outgoing_webhooks.unknown_channel" defaultMessage="A Private Webhook" />
  }
    ...
    if (outgoingWebhook.description) {
      description = (
        <div className="item-details__row">
          <span className="item-details__description">{outgoingWebhook.description}</span>
        </div>
      )
    }
    ...
    if (this.props.canChange) {
      actions = (
        <div className="item-actions">
          <button className="style--none color--link" onClick={this.handleRegenToken}>
            <FormattedMessage id="installed_integrations.regenToken" defaultMessage="Regen Token" />
          </button>
          {' - '}
          ...
          {' - '}
          <DeleteIntegration messageId={t('installed_outgoing_webhooks.delete.confirm')} onDelete={this.handleDelete} />
        </div>
      )
    }
      ...
      <div className="backstage-list__item">
        <div className="item-details">
          <div className="item-details__row d-flex flex-column flex-md-row justify-content-between">
            <strong className="item-details__name">{displayName}</strong>
            {actions}
          </div>
          {description}
          <div className="item-details__row">
