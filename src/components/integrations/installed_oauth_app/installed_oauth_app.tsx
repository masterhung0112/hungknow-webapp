...
export default class InstalledOAuthApp extends React.PureComponent<InstalledOAuthAppProps, InstalleOAuthAppState> {
  ...
  handleRegenerate = (e: React.MouseEvent): void => {
    e.preventDefault()
    this.props.onRegenerateSecret(this.props.oauthApp.id).then(({ error }) => {
      if (error) {
  ...
  render(): React.ReactNode {
    ...
    if (oauthApp.description) {
      description = (
        <div className="item-details__row">
          <span className="item-details__description">{oauthApp.description}</span>
        </div>
      )
    }
    ...
    if (this.state.clientSecret === FAKE_SECRET) {
      showHide = (
        <button id="showSecretButton" className="style--none color--link" onClick={this.handleShowClientSecret}>
          <FormattedMessage id="installed_integrations.showSecret" defaultMessage="Show Secret" />
        </button>
      )
      clientSecret = (
      ...
      showHide = (
        <button id="hideSecretButton" className="style--none color--link" onClick={this.handleHideClientSecret}>
          <FormattedMessage id="installed_integrations.hideSecret" defaultMessage="Hide Secret" />
        </button>
      )
      clientSecret = (
        <span className="item-details__token">
    ...
    const regen = (
      <button id="regenerateSecretButton" className="style--none color--link" onClick={this.handleRegenerate}>
        <FormattedMessage id="installed_integrations.regenSecret" defaultMessage="Regenerate Secret" />
      </button>
    )
      ...
      <div className="backstage-list__item">
        ...
        <div className="item-details">
          <div className="item-details__row d-flex flex-column flex-md-row justify-content-between">
            <strong className="item-details__name">{name}</strong>
            <div className="item-actions">
              ...
              {' - '}
              <DeleteIntegration messageId={t('installed_oauth_apps.delete.confirm')} onDelete={this.handleDelete} />
            </div>
          </div>
          {error}
          {description}
          ...
          <div className="item-details__row">
            <span className="item-details__token">
              ...
              />
              <CopyText idMessage="integrations.copy_client_id" defaultMessage="Copy Client Id" value={oauthApp.id} />
            </span>
          </div>
