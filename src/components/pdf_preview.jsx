...
export default class PDFPreview extends React.PureComponent {
  ...
  render() {
    ...
    const pdfCanvases = []
    for (let i = 0; i < this.state.numPages; i++) {
      pdfCanvases.push(<canvas ref={this[`pdfCanvasRef-${i}`]} key={'previewpdfcanvas' + i} />)

      if (i < this.state.numPages - 1 && this.state.numPages > 1) {
        pdfCanvases.push(<div key={'previewpdfspacer' + i} className="pdf-preview-spacer" />)
      }
    }

    return (
      <div ref={this.container} className="post-code">
        {pdfCanvases}
      </div>
    )
  }
