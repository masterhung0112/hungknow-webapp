...
import { Constants, ModalIdentifiers, NotificationLevels, RHSStates } from 'utils/constants'
...
class ChannelHeader extends React.PureComponent {
  ...
  constructor(props) {
    ...
    this.getHeaderMarkdownOptions = memoizeResult((channelNamesMap) => ({ ...headerMarkdownOptions, channelNamesMap }))
  ...
  render() {
    ...
    if (hasGuests) {
      hasGuestsText = (
        <span className="has-guest-header">
          <FormattedMessage id="channel_header.channelHasGuests" defaultMessage="This channel has guests" />
        </span>
      )
    }
    ...
    const pinnedIcon = this.props.pinnedPostsCount ? (
      <React.Fragment>
        <i aria-hidden="true" className="icon icon-pin-outline channel-header__pin" />
    ...
    if (headerText) {
      const popoverContent = (
        ...
        >
          <span onClick={this.handleFormattedTextClick}>
            <Markdown message={headerText} options={this.getPopoverMarkdownOptions(channelNamesMap)} />
          </span>
        </Popover>
      )

      headerTextContainer = (
        <div id="channelHeaderDescription" className="channel-header__description" dir="auto">
          {dmHeaderIconStatus}
          {dmHeaderTextStatus}
          {popoverListMembers}
          <HeaderIconWrapper
          ...
          />
          {hasGuestsText}
          <div className="header-popover-text-measurer" ref={this.headerPopoverTextMeasurerRef}>
            <Markdown
          ...
          >
            ...
            <Markdown message={headerText} options={this.getHeaderMarkdownOptions(channelNamesMap)} />
          </span>
        </div>
      )
    } else {
      ...
      if (!isReadOnly && !channelIsArchived) {
        if (isDirect || isGroup) {
          if (!isDirect || !dmUser.is_bot) {
            editMessage = (
              <button className="header-placeholder style--none" onClick={this.showEditChannelHeaderModal}>
                <FormattedMessage id="channel_header.addChannelHeader" defaultMessage="Add a channel description" />
                <FormattedMessage id="channel_header.editLink" defaultMessage="Edit">
                  {(message) => <i aria-label={message} className="icon icon-pencil-outline edit-icon" />}
                </FormattedMessage>
              </button>
            )
          }
      ...
      headerTextContainer = (
        <div id="channelHeaderDescription" className="channel-header__description light">
          {dmHeaderIconStatus}
          {dmHeaderTextStatus}
          {popoverListMembers}
          <HeaderIconWrapper
      ...
      toggleFavorite = (
        ...
        >
          <button
            ...
            className={
              'style--none color--link channel-header__favorites ' + (this.props.isFavorite ? 'active' : 'inactive')
            }
    ...
    const channelMutedTooltip = (
      <Tooltip id="channelMutedTooltip">
        <FormattedMessage id="channelHeader.unmute" defaultMessage="Unmute" />
      </Tooltip>
    )
    ...
    if (channelMuted) {
      muteTrigger = (
        <OverlayTrigger delayShow={Constants.OVERLAY_TIME_DELAY} placement="bottom" overlay={channelMutedTooltip}>
          <button
            id="toggleMute"
            onClick={this.unmute}
      ...
      <React.Fragment>
        <MenuWrapper onToggle={this.setTitleMenuOpen}>
          <div id="channelHeaderDropdownButton" className="channel-header__top">
            ...
            >
              <strong role="heading" aria-level="2" id="channelHeaderTitle" className="heading">
                <span>
                  {archivedIcon}
                  {channelTitle}
                </span>
    ...
    if (isDirect && dmUser.is_bot) {
      title = (
        <div id="channelHeaderDropdownButton" className="channel-header__top channel-header__bot">
          <strong role="heading" aria-level="2" id="channelHeaderTitle" className="heading">
            <span>
              {archivedIcon}
              {channelTitle}
    ...
    return (
      ...
      >
        <div className="flex-parent">
          ...
          <ChannelHeaderPlug channel={channel} channelMember={channelMember} />
          <RHSSearchNav />
        </div>
      </div>
    )
