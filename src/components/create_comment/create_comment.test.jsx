...
describe('components/CreateComment', () => {
  ...
  test('should correctly update draft when handleEmojiClick is called', () => {
    ...
    wrapper.instance().textboxRef.current = {
    ...
    // Empty message case
    expect(onUpdateCommentDraft.mock.calls[0][0]).toEqual(expect.objectContaining({ message: ':smile: ' }))
    ...
    expect(onUpdateCommentDraft.mock.calls[1][0]).toEqual(expect.objectContaining({ message: 'test :smile:  ' }))
    ...
    expect(onUpdateCommentDraft.mock.calls[2][0]).toEqual(expect.objectContaining({ message: 'test  :smile:  ' }))
    expect(wrapper.state().draft.message).toBe('test  :smile:  ')
  ...
  it('check for uploadsProgressPercent state on handleUploadProgress callback', () => {
    ...
    expect(wrapper.find(FilePreview).prop('uploadsProgressPercent')).toEqual({
  ...
  test('handleChange should update comment draft correctly', () => {
    ...
    expect(baseProps.onUpdateCommentDraft.mock.calls[0][0]).toEqual(expect.objectContaining({ message: testMessage }))
  ...
  test('removePreview should remove file info and upload in progress with corresponding id', () => {
    ...
    expect(onUpdateCommentDraft.mock.calls[1][0]).toEqual(expect.objectContaining({ uploadsInProgress: [4, 6] }))
  ...
  test('should the RHS thread scroll to bottom when state.draft.uploadsInProgress increases but not when it decreases', () => {
    ...
    const wrapper = shallowWithIntl(<CreateComment {...baseProps} draft={draft} />)
