...
import { sortFileInfos } from 'hkclient-ts/lib/utils/file_utils'
...
class CreateComment extends React.PureComponent {
  ...
  static getDerivedStateFromProps(props, state) {
    ...
    if (rootChanged || messageInHistoryChanged) {
      updatedState = {
        ...updatedState,
        draft: { ...props.draft, uploadsInProgress: rootChanged ? [] : props.draft.uploadsInProgress },
      }
    }

    if (
      props.createPostErrorId === 'api.post.create_post.root_id.app_error' &&
      props.createPostErrorId !== state.createPostErrorId
    ) {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (
      ...
      prevProps.selectedPostFocussedAt !== this.props.selectedPostFocussedAt
    ) {
  ...
  handleSubmit = async (e) => {
    ...
    if (enableConfirmNotificationsToChannel && notContainsAtChannel && useGroupMentions) {
      ...
      if (mentions.length > 0) {
        mentions = mentions.map((group) => {
          ...
          if (
            mappedValue &&
            mappedValue.channel_member_count > Constants.NOTIFY_ALL_MEMBERS &&
    ...
    if (notificationsToChannel && channelMembersCount > Constants.NOTIFY_ALL_MEMBERS && !notContainsAtChannel) {
  ...
  handleKeyDown = (e) => {
    ...
    if (ctrlKeyCombo) {
      ...
      } else if (Utils.isKeyPressed(e, Constants.KeyCodes.B) || Utils.isKeyPressed(e, Constants.KeyCodes.I)) {
      ...
      uploadsInProgressText = (
        <span className="post-right-comments-upload-in-progress">
          {draft.uploadsInProgress.length === 1 ? (
            <FormattedMessage id="create_comment.file" defaultMessage="File uploading" />
          ) : (
            <FormattedMessage id="create_comment.files" defaultMessage="Files uploading" />
          )}
        </span>
      )
    }
        ...
        >
          <div id={this.props.rootId} className="post-create-body comment-create-body">
            <div className="post-body__cell">
              ...
              />
              <span ref={this.createCommentControlsRef} className="post-body__actions">
                {fileUpload}
                {emojiPicker}
              </span>
            </div>
          ...
          <div className="post-create-footer">
            <div className="d-flex justify-content-between">
              <div className="col">
                <MsgTyping channelId={this.props.channelId} postId={this.props.rootId} />
                {postError}
              </div>
