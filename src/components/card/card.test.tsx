...
describe('components/card/card', () => {
  ...
  const headerProps = {
    title: (
      <FormattedMessage id="admin.data_retention.customPolicies.title" defaultMessage="Custom retention policies" />
    ),
    subtitle: (
      <FormattedMessage
  ...
  test('should match snapshot when using header content and a button', () => {
    ...
    const buttonProps = {
      buttonText: <FormattedMessage id="admin.data_retention.customPolicies.addPolicy" defaultMessage="Add policy" />,
      onClick: () => {},
    }
    ...
    const wrapper = shallow(
      <Card {...props}>
        <Card.Header>
          <TitleAndButtonCardHeader {...headerProps} {...buttonProps} />
        </Card.Header>
        <Card.Body>{'Body Test'}</Card.Body>
      </Card>
    )
