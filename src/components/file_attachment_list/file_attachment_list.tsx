...
export default class FileAttachmentList extends React.PureComponent<Props, State> {
  ...
  render() {
    const { compactDisplay, enableSVGs, fileInfos, fileCount, locale } = this.props
    ...
    if (sortedFileInfos && sortedFileInfos.length > 0) {
      ...
      for (let i = 0; i < fileCount; i++) {
        ...
        postFiles.push(<div key={`fileCount-${i}`} className="post-image__column post-image__column--placeholder" />)
      }
    }

    return (
      <>
        <div data-testid="fileAttachmentList" className="post-image__columns clearfix">
          {postFiles}
        </div>
        <ViewImageModal
