...
describe('components/emoji/components/AddEmoji', () => {
  const baseProps: AddEmojiProps = {
    emojiMap: new EmojiMap(
      new Map<string, unknown>([['mycustomemoji', {}]])
    ),
    team: {
      id: 'team-id',
    } as Team,
  ...
  test('should submit the new added emoji', () => {
    const wrapper = shallow<AddEmoji>(<AddEmoji {...baseProps} />, { context })
  ...
  test.each(['hyphens-are-allowed', 'underscores_are_allowed', 'numb3rsar3all0w3d'])(
    '%s should be a valid emoji name',
    (emojiName) => {
      const wrapper = shallow<AddEmoji>(<AddEmoji {...baseProps} />, { context })
  ...
  test.each([
    ...
    expect(wrapper.state().saving).toEqual(true)
    expect(baseProps.actions.createCustomEmoji).toHaveBeenCalledWith(
      { creator_id: baseProps.user.id, name: expectedName },
      file
    )
    expect(wrapper.state().error).toBeNull()
  })
  ...
  test('should show generic error when action response cannot be parsed', async () => {
    ...
    const wrapper = shallow<AddEmoji>(<AddEmoji {...props} />, { context })
  ...
  test('should show response error message when action response is error', async () => {
    ...
    props.actions = {
      createCustomEmoji: jest
        .fn()
        .mockImplementation(async (): Promise<unknown> => ({ error: { message: serverError } })),
    }
