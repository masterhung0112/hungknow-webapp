...
import { checkDialogElementForError, checkIfErrorsMatchElements } from 'hkclient-ts/lib/utils/integration_utils'
...
export default class InteractiveDialog extends React.PureComponent {
  ...
  handleSubmit = async (e) => {
    ...
    if (data) {
      ...
      if (data.errors && Object.keys(data.errors).length >= 0 && checkIfErrorsMatchElements(data.errors, elements)) {
        hasErrors = true
  ...
  render() {
    ...
    let submitText = <FormattedMessage id="interactive_dialog.submit" defaultMessage="Submit" />
    if (submitLabel) {
    ...
    return (
      ...
      >
        <form onSubmit={this.handleSubmit}>
          <Modal.Header closeButton={true} style={{ borderBottom: elements == null && '0px' }}>
            <Modal.Title componentClass="h1" id="interactiveDialogModalLabel">
              {icon}
              {title}
            </Modal.Title>
          </Modal.Header>
          ...
          <Modal.Footer>
            ...
            <button
              ...
              onClick={this.onHide}
            >
              <FormattedMessage id="interactive_dialog.cancel" defaultMessage="Cancel" />
            </button>
            <SpinnerButton
