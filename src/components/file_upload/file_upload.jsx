...
class FileUpload extends PureComponent {
  ...
  uploadFiles = (sortedFiles) => {
    ...
    for (let i = 0; i < sortedFiles.length && numUploads < uploadsRemaining; i++) {
      ...
      const request = this.props.actions.uploadFile(sortedFiles[i], sortedFiles[i].name, channelId, rootId, clientId)
    ...
    if (tooLargeFiles.length > 1) {
      ...
      errors.push(
        formatMessage(holders.filesAbove, { max: this.props.maxFileSize / 1048576, filenames: tooLargeFilenames })
      )
    } else if (tooLargeFiles.length > 0) {
      errors.push(
        formatMessage(holders.fileAbove, { max: this.props.maxFileSize / 1048576, filename: tooLargeFiles[0].name })
      )
    }
  ...
  handleDrop = (e) => {
    ...
    if (files.length === 0) {
      this.props.onUploadError(
        localizeMessage('file_upload.drag_folder', 'Folders cannot be uploaded. Please drag all files separately.')
      )
      return
    }
  ...
  render() {
    ...
    if (this.props.pluginFileUploadMethods.length === 0) {
      ...
      bodyAction = (
        <div>
          ...
          />
          <MenuWrapper>
            <button type="button" aria-label={ariaLabel} className="style--none post-action">
            ...
            >
              <li>
                <a href="#" onClick={this.simulateInputClick} onTouchEnd={this.simulateInputClick}>
                  <span className="mr-2">
    ...
    return <div className={uploadsRemaining <= 0 ? ' style--none btn-file__disabled' : 'style--none'}>{bodyAction}</div>
  }
}
