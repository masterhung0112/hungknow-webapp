...
export type Props<T extends Value> = {
  ...
  optionRenderer: (option: T, isSelected: boolean, add: (value: T) => void, select: (value: T) => void) => void
...
export default class MultiSelect<T extends Value> extends React.PureComponent<Props<T>, State> {
  ...
  private onAdd = (value: T) => {
    ...
    if (this.reactSelectRef.current) {
      this.reactSelectRef.current.select.handleInputChange({ currentTarget: { value: '' } } as React.KeyboardEvent<
        HTMLInputElement
      >)
  ...
  formatOptionLabel = (user: any) => {
    ...
    return (
      <React.Fragment>
        ...
        <div className="react-select__value__name">{getDisplayName(user)}</div>
      </React.Fragment>
    )
  }
      ...
      noteTextContainer = (
        <div className="multi-select__note">
          <div className="note__icon">
            <FormattedMessage id="generic_icons.info" defaultMessage="Info Icon">
              {(title) => <span className="fa fa-info" title={title as string} />}
            </FormattedMessage>
          </div>
    ...
    if (options && options.length > this.props.perPage) {
      ...
      if (!this.props.loading) {
        if (options.length > pageEnd) {
          nextButton = (
            <button className="btn btn-link filter-control filter-control__next" onClick={this.nextPage}>
        ...
        if (this.state.page > 0) {
          previousButton = (
            <button className="btn btn-link filter-control filter-control__prev" onClick={this.prevPage}>
      ...
      <React.Fragment>
        <div className="filtered-user-list">
          <div className="filter-row filter-row--full">
              ...
              <ReactSelect
                ...
                components={{
                  ...
                  MultiValueLabel: this.props.valueWithImage
                    ...
                    : paddedComponent(this.props.valueRenderer),
            ...
            <div id="multiSelectHelpMemberInfo" className="multi-select__help">
              {numRemainingText}
              {memberCount}
            </div>
          </div>
          {multiSelectList}
          <div id="multiSelectMessageNote" className="multi-select__help">
            {noteTextContainer}
          </div>
          {this.props.saveButtonPosition === 'top' && (
