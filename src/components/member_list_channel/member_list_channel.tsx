...
import { Channel, ChannelStats, ChannelMembership } from 'hkclient-ts/lib/types/channels'
...
export default class MemberListChannel extends React.PureComponent<Props, State> {
  ...
  componentDidUpdate(prevProps: Props) {
    if (prevProps.searchTerm !== this.props.searchTerm) {
      ...
      const searchTimeoutId = window.setTimeout(async () => {
        const { data } = await prevProps.actions.searchProfiles(searchTerm, {
        ...
        this.props.actions
          .loadTeamMembersAndChannelMembersForProfilesList(data, this.props.currentTeamId, this.props.currentChannelId)
  ...
  nextPage = (page: number) => {
    this.props.actions.loadProfilesAndTeamMembersAndChannelMembers(page + 1, USERS_PER_PAGE, undefined, undefined, {
      active: true,
    })
  }
