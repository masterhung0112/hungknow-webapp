...
class RHSSearchNav extends React.PureComponent<Props, State> {
  ...
  searchMentions = (e: KeyboardEvent) => {
    ...
    const {
      rhsState,
      actions: { closeRightHandSide, showMentions },
    } = this.props
  ...
  render() {
    const { rhsOpen, rhsState } = this.props

    return (
      <>
        {this.state.showSearchBar ? (
          <div id="searchbarContainer" className="flex-child search-bar__container">
            <Search isFocus={Utils.isMobile() || (rhsOpen && Boolean(rhsState))} />
          </div>
        ) : (
          <HeaderIconWrapper
            iconComponent={<SearchIcon className="icon icon--standard" aria-hidden="true" />}
            ariaLabel={true}
