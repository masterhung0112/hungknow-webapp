...
import { getChannelByNameAndTeamName, getChannelMember, joinChannel } from 'hkclient-ts/lib/actions/channels'
import { getUserByEmail } from 'hkclient-ts/lib/actions/users'
...
jest.mock('hkclient-ts/lib/actions/channels', () => ({
  joinChannel: jest.fn(() => ({
    type: '',
    data: { channel: { id: 'channel_id3', name: 'achannel3', team_id: 'team_id1', type: 'O' } },
  })),
  getChannelByNameAndTeamName: jest.fn(() => ({
    type: '',
    data: { id: 'channel_id3', name: 'achannel3', team_id: 'team_id1', type: 'O' },
  })),
...
describe('Actions', () => {
    ...
    entities: {
      channels: {
        ...
        myMembers: {
          ...
          channel_id2: { channel_id: 'channel_id2', user_id: 'current_user_id' },
        },
  ...
  describe('getPathFromIdentifier', () => {
    ...
    test('Should return direct channel path if identifier is in the format userid__userid2', () => {
      const path = getPathFromIdentifier(
        initialState as any,
        'channels',
        '3y8ujrgtbfn78ja5nfms3qm5jw__3y8ujrgtbfn78ja5nfms3qm5jw'
      )
  ...
  describe('goToChannelByChannelId', () => {
    test('switch to public channel we have locally but need to join', async () => {
      ...
      await testStore.dispatch(
        goToChannelByChannelId(
          { params: { team: 'team1', identifier: 'channel_id3', path: '/' }, url: '' },
          history as any
        ) as any
      )
  ...
  describe('goToChannelByChannelName', () => {
    test('switch to channel on different team with same name', async () => {
      ...
      await testStore.dispatch(
        goToChannelByChannelName(
          { params: { team: 'team2', identifier: 'achannel', path: '/' }, url: '' },
          {} as any
        ) as any
      )
    ...
    test('switch to public channel we have locally but need to join', async () => {
      ...
      await testStore.dispatch(
        goToChannelByChannelName(
          { params: { team: 'team1', identifier: 'achannel3', path: '/' }, url: '' },
          {} as any
        ) as any
      )
    ...
    test("switch to private channel we don't have locally and get prompted if super user and then join", async () => {
      ...
      await testStore.dispatch(
        goToChannelByChannelName(
          { params: { team: 'team1', identifier: channel.name, path: '/' }, url: '' },
          {} as any
        ) as any
      )
  ...
  describe('goToDirectChannelByUserId', () => {
    test('switch to a direct channel by user id on the same team', async () => {
      ...
      await testStore.dispatch(
        goToDirectChannelByUserId(
          { params: { team: 'team1', identifier: 'channel', path: '/' }, url: '' },
          history as any,
          'user_id2'
        ) as any
      )
    ...
    test('switch to a direct channel by user id on different team', async () => {
      ...
      await testStore.dispatch(
        goToDirectChannelByUserId(
          { params: { team: 'team2', identifier: 'channel', path: '/' }, url: '' },
          history as any,
          'user_id2'
        ) as any
      )
  ...
  describe('goToDirectChannelByUserIds', () => {
    test('switch to a direct channel by name on the same team', async () => {
      ...
      await testStore.dispatch(
        goToDirectChannelByUserIds(
          { params: { team: 'team1', identifier: 'current_user_id__user_id2', path: '/' }, url: '' },
          history as any
        ) as any
      )
      expect(history.replace).toHaveBeenCalledWith('/team1/messages/@user2')
    ...
    test('switch to a direct channel by name on different team', async () => {
      ...
      await testStore.dispatch(
        goToDirectChannelByUserIds(
          { params: { team: 'team2', identifier: 'current_user_id__user_id2', path: '/' }, url: '' },
          history as any
        ) as any
      )
      expect(history.replace).toHaveBeenCalledWith('/team2/messages/@user2')
  ...
  describe('goToDirectChannelByEmail', () => {
    test('switch to a direct channel by email with user already existing locally', async () => {
      ...
      await testStore.dispatch(
        goToDirectChannelByEmail(
          { params: { team: 'team1', identifier: 'user2@bladekick.com', path: '/' }, url: '' },
          history as any
        ) as any
      )
    ...
    test('switch to a direct channel by email with user not existing locally', async () => {
      ...
      await testStore.dispatch(
        goToDirectChannelByEmail(
          { params: { team: 'team1', identifier: 'user3@bladekick.com', path: '/' }, url: '' },
          history as any
        ) as any
      )
