...
import { getUser, getUserByUsername, getUserByEmail } from 'hkclient-ts/lib/actions/users'
import { getTeamByName } from 'hkclient-ts/lib/selectors/entities/teams'
import {
  getCurrentUser,
  getCurrentUserId,
...
import { GetStateFunc, DispatchFunc, ActionFunc } from 'hkclient-ts/lib/types/actions'
...
export function goToChannelByChannelId(match: Match, history: History): ActionFunc {
  return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
    ...
    if (channel.type === Constants.DM_CHANNEL) {
      dispatch(
        goToDirectChannelByUserId(match, history, Utils.getUserIdFromChannelId(channel.name, getCurrentUserId(state)))
      )
...
export function goToChannelByChannelName(match: Match, history: History): ActionFunc {
  return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
    ...
    if (!channel || !member) {
      ...
      const joinChannelDispatchResult = await dispatch(
        joinChannel(getCurrentUserId(state), teamObj!.id, '', channelName)
      )
