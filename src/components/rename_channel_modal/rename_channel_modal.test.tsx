...
describe('components/RenameChannelModal', () => {
  ...
  test('should match snapshot', () => {
    const wrapper = shallowWithIntl(<RenameChannelModal {...baseProps} />)
  ...
  test('should submit form', () => {
    ...
    const wrapper = shallowWithIntl(<RenameChannelModal {...props} />)

    wrapper.find('#display_name').simulate('change', { preventDefault: jest.fn(), target: { value: 'valid name' } })
  ...
  describe('should validate channel url (name)', () => {
    const testCases: Array<[{ name: string; value: string }, boolean]> = [
      ...
      [
        {
          name: 'can not resemble direct message channel url',
          value: 'uzsfmtmniifsjgesce4u7yznyh__uzsfmtmniifsjgesce4u7yznyh',
        },
        false,
      ],
  ...
  test('should not call patchChannel as channel.name.length > Constants.MAX_CHANNELNAME_LENGTH (64)', () => {
    ...
    wrapper.find('#display_name').simulate('change', {
      ...
      target: {
        value: 'string-above-sixtyfour-characters-to-test-the-channel-maxlength-limit-properly-in-the-component',
      },
    })
  ...
  test('should change state when display_name is edited', () => {
    ...
    wrapper
      .find('#display_name')
      .simulate('change', { preventDefault: jest.fn(), target: { value: 'New Fake Channel' } })
  ...
  test('should call handleSubmit function', async () => {
    const patchChannel = jest.fn().mockResolvedValueOnce({ error: true }).mockResolvedValue({ data: true })

    const wrapper = shallowWithIntl(<RenameChannelModal {...baseProps} actions={{ patchChannel }} />)
