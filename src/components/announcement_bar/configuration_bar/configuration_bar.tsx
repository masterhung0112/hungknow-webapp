...
import { ClientConfig, WarnMetricStatus } from 'hkclient-ts/lib/types/config'
...
const ConfigurationAnnouncementBar: React.FC<Props> = (props: Props) => {
  ...
  if (props.config?.SendEmailNotifications !== 'true' && props.config?.EnablePreviewModeBanner === 'true') {
    const emailMessage = formatMessage({
      id: AnnouncementBarMessages.PREVIEW_MODE,
      defaultMessage: 'Preview Mode: Email notifications have not been configured',
    })

    return <TextDismissableBar allowDismissal={true} text={emailMessage} type={AnnouncementBarTypes.SUCCESS} />
  }
  ...
  if (props.canViewSystemErrors && props.config?.SiteURL === '') {
    ...
    if (props.config?.EnableSignUpWithGitLab === 'true') {
      ...
      defaultMessage =
        "Please configure your [site URL](https://docs.mattermost.com/administration/config-settings.html#site-url) either on the [System Console](/admin_console/environment/web_server) or, if you're using GitLab Mattermost, in gitlab.rb."
    } else {
      id = t('announcement_bar.error.site_url.full')
      defaultMessage =
        'Please configure your [site URL](https://docs.mattermost.com/administration/config-settings.html#site-url) on the [System Console](/admin_console/environment/web_server).'
    }

    const values = { siteURL: props.siteURL }
    const siteURLMessage = formatMessage({ id, defaultMessage }, values)

    return <TextDismissableBar allowDismissal={true} text={siteURLMessage} type={AnnouncementBarTypes.ANNOUNCEMENT} />
  }
