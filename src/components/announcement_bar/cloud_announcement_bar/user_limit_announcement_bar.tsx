...
import { Subscription } from 'hkclient-ts/lib/types/cloud'
...
class UserLimitAnnouncementBar extends React.PureComponent<Props> {
  async componentDidMount() {
    ...
    if (!isEmpty(this.props.subscription) && !isEmpty(this.props.analytics) && this.shouldShowBanner()) {
      if (this.isDismissable()) {
        trackEvent(TELEMETRY_CATEGORIES.CLOUD_ADMIN, 'bannerview_user_limit_reached')
      } else {
        trackEvent(TELEMETRY_CATEGORIES.CLOUD_ADMIN, 'bannerview_user_limit_exceeded')
      }
    }
  }
  ...
  handleClose = async () => {
    trackEvent(TELEMETRY_CATEGORIES.CLOUD_ADMIN, 'click_close_banner_user_limit_reached')
  ...
  showModal = () => {
    if (this.isDismissable()) {
      trackEvent(TELEMETRY_CATEGORIES.CLOUD_ADMIN, 'click_upgrade_banner_user_limit_reached')
    } else {
      trackEvent(TELEMETRY_CATEGORIES.CLOUD_ADMIN, 'click_upgrade_banner_user_limit_exceeded')
    }
  ...
  render() {
    ...
    if (
      userLimit === analytics!.TOTAL_USERS &&
