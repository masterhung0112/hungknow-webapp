...
export default class AdminSidebarCategory extends React.PureComponent {
  ...
  render() {
    ...
    if (this.props.name) {
      link += '/' + name
      title = (
        <NavLink to={link} className="category-title" activeClassName="category-title category-title--active">
          {title}
        </NavLink>
      )
    }
    ...
    return (
      <li className="sidebar-category" data-testid={this.props.definitionKey}>
        {title}
        {clonedChildren}
      </li>
    )
