...
export default class SchemaAdminSettings extends React.PureComponent {
  ...
  renderTitle = () => {
    ...
    if (this.props.schema.translate === false) {
      return <AdminHeader>{this.props.schema.name || this.props.schema.id}</AdminHeader>
    }
    return (
  ...
  renderBanner = (setting) => {
    ...
    if (typeof setting.label === 'string') {
      ...
      return (
        <FormattedMessage id={setting.label} defaultMessage={setting.label_default} values={setting.label_values} />
      )
    }
  ...
  isDisabled = (setting) => {
    ...
    if (typeof setting.isDisabled === 'function') {
      return setting.isDisabled(
        ...
        this.props.license,
        enterpriseReady,
        this.props.consoleAccess,
        this.props.cloud,
        this.props.isCurrentUserSystemAdmin
      )
    }
        ...
        settings.forEach((tsetting) => {
          if (tsetting.key && tsetting.setFromMetadataField) {
            ...
            if (tsetting.type === Constants.SettingsTypes.TYPE_TEXT) {
              ...
              if (
                this.buildSettingFunctions[tsetting.type] &&
                this.buildSettingFunctions[tsetting.type](tsetting).props.onSetData
              ) {
  ...
  buildDropdownSetting = (setting) => {
    ...
    const values = options.map((o) => ({
      value: o.value,
      text: Utils.localizeMessage(o.display_name, o.display_name_default),
    }))
  ...
  buildLanguageSetting = (setting) => {
    ...
    if (setting.multiple) {
      const noResultText = <FormattedMessage id={setting.no_result} defaultMessage={setting.no_result_default} />
  ...
  buildBannerSetting = (setting) => {
    ...
    return (
      <div className={'banner ' + setting.banner_type} key={this.props.schema.id + '_bool_' + setting.key}>
  ...
  buildUsernameSetting = (setting) => {
    return (
      <UserAutocompleteSetting
        ...
        helpText={this.renderHelpText(setting)}
        placeholder={
          Utils.localizeMessage(setting.placeholder, setting.placeholder_default) ||
  ...
  buildJobsTableSetting = (setting) => {
    return (
      <JobsTable
        ...
        disabled={this.isDisabled(setting)}
        createJobButtonText={<FormattedMessage id={setting.label} defaultMessage={setting.label_default} />}
        createJobHelpText={
          <FormattedMarkdownMessage id={setting.help_text} defaultMessage={setting.help_text_default} />
        }
      />
    )
  }
    ...
    if (this.state[setting.key]) {
      ...
      return (
        <RemoveFileSetting
          ...
          label={this.renderLabel(setting)}
          helpText={
            <FormattedMessage id={setting.remove_help_text} defaultMessage={setting.remove_help_text_default} />
          }
  ...
  buildCustomSetting = (setting) => {
    ...
    if (setting.showTitle) {
      return (
        <Setting label={setting.label} inputId={setting.key} helpText={setting.helpText}>
          {componentInstance}
        </Setting>
      )
    }
  ...
  renderSettings = () => {
    ...
    if (schema.header) {
      header = (
        <div className="banner">
          <SchemaText text={schema.header} isMarkdown={true} isTranslated={this.props.schema.translate} />
        </div>
      )
    }
    ...
    if (schema.footer) {
      footer = (
        <div className="banner">
          <SchemaText text={schema.footer} isMarkdown={true} isTranslated={this.props.schema.translate} />
        </div>
      )
    }
  ...
  doSubmit = async (getStateFromConfig) => {
    ...
    const hasSaveActionError = await Promise.all(results).then((values) =>
      values.some((value) => value.error && value.error.message)
    )
  ...
  hybridSchemaAndComponent = () => {
    ...
    if (schema && schema.component && schema.settings) {
      ...
      return <CustomComponent {...this.props} disabled={this.props.isDisabled} />
    }
    return null
  }
    ...
    if (!schema) {
      return (
        <div className={'wrapper--fixed'}>
          <AdminHeader>
            <FormattedMessage id="error.plugin_not_found.title" defaultMessage="Plugin Not Found" />
          </AdminHeader>
          <div className="admin-console__wrapper">
            <div className="admin-console__content">
              ...
              <Link to={'plugin_management'}>
                <FormattedMessage id="admin.plugin.backToPlugins" defaultMessage="Go back to the Plugins" />
              </Link>
            </div>
          </div>
        </div>
    ...
    return (
      <div className={'wrapper--fixed ' + this.state.customComponentWrapperClass}>
        ...
        <div className="admin-console-save">
          ...
          >
            <Tooltip id="error-tooltip">{this.state.serverError}</Tooltip>
          </Overlay>
        </div>
      </div>
    )
