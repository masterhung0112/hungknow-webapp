...
export default class SystemRolePermissionDropdown extends React.PureComponent<Props> {
  ...
  renderOption = (label: JSX.Element, description: JSX.Element) => {
    return (
      <div className="PermissionSectionDropdownOptions">
        <div className="PermissionSectionDropdownOptions_label">{label}</div>
        <div className="PermissionSectionDropdownOptions_description">{description}</div>
      </div>
    )
  }
  ...
  render() {
    ...
    const canWriteLabel = (
      <FormattedMessage id={'admin.permissions.system_role_permissions.write.title'} defaultMessage="Can edit" />
    )
    ...
    const canReadLabel = (
      <FormattedMessage id={'admin.permissions.system_role_permissions.read.title'} defaultMessage="Read only" />
    )
    ...
    const noAccessLabel = (
      <FormattedMessage id={'admin.permissions.system_role_permissions.no_access.title'} defaultMessage="No access" />
    )
    ...
    const ariaLabel = Utils.localizeMessage(
      ...
      'Change role access on a system console section'
    )
    return (
      <MenuWrapper isDisabled={isDisabled}>
        <button
          ...
          aria-expanded="true"
        >
          <div className="PermissionSectionDropdownButton_text">{currentAccess}</div>
