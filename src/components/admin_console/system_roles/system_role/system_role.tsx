...
export default class SystemRole extends React.PureComponent<Props, State> {
  ...
  getSaveStateNeeded = (nextState: Partial<State>): boolean => {
    ...
    if (Object.keys(permissionsToUpdate).length > 0) {
      saveNeeded =
        saveNeeded ||
        difference(updatedRolePermissions, role.permissions).length > 0 ||
    ...
    return (
      <div className="wrapper--fixed">
        <div className="admin-console__header with-back">
          <div>
            <BlockableLink to="/admin_console/user_management/system_roles" className="fa fa-angle-left back" />
            <FormattedMessage id={`admin.permissions.roles.${role.name}.name`} defaultMessage={defaultName} />
          </div>
        </div>
