...
const getSectionsListForRole = memoizeResult(
  (sections: SystemSection[], roleName: string, sectionsByRole: Record<string, Record<string, boolean>>) => {
...
export default class SystemRolePermissions extends React.PureComponent<Props, State> {
  ...
  getRows = (
    ...
    return getSectionsListForRole(sectionsList, this.props.role.name, editedSectionsByRole).map(
