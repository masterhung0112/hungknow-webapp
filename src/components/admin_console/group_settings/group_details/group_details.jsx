...
import { Groups } from 'hkclient-ts/lib/constants'
...
export default class GroupDetails extends React.PureComponent {
    ...
    if (prevProps.groupChannels !== this.props.groupChannels) {
      ...
      if (this.state.saveNeeded) {
        ...
        gcs = this.props.groupChannels
          ...
          .concat(this.state.groupChannels)
      } else {
        gcs = this.props.groupChannels
      }
  ...
  handlePatchGroup = async () => {
    ...
    if (!groupMentionName && allowReference) {
      serverError = (
        <FormattedMessage id="admin.group_settings.need_groupname" defaultMessage="You must specify a group mention." />
      )
      ...
      const result = await this.props.actions.patchGroup(this.props.groupID, {
      ...
      if (result.error) {
        if (result.error.server_error_id === 'store.sql_group.unique_constraint') {
          ...
          result.error.server_error_id === 'api.ldap_groups.existing_reserved_name_error' ||
  ...
  handleRolesToUpdate = async () => {
    ...
    const promises = Object.entries(rolesToChange).map(([key, value]) => {
      ...
      return this.props.actions.patchGroupSyncable(
        this.props.groupID,
        syncableID,
  ...
  handleAddedTeamsAndChannels = async () => {
    ...
    if (teamsToAdd.length) {
      teamsToAdd.forEach((groupTeam) => {
        ...
        promises.push(
          this.props.actions.link(this.props.groupID, groupTeam.team_id, Groups.SYNCABLE_TYPE_TEAM, {
            auto_add: true,
            scheme_admin: groupTeam.scheme_admin,
          })
        )
      })
    }
    if (channelsToAdd.length) {
      channelsToAdd.forEach((groupChannel) => {
        ...
        delete rolesToChange[roleChangeKey] // delete the key because it won't need a patch, it's being handled by the link request.
        promises.push(
          this.props.actions.link(this.props.groupID, groupChannel.channel_id, Groups.SYNCABLE_TYPE_CHANNEL, {
            auto_add: true,
    ...
    return (
      <div className="wrapper--fixed">
        <div className="admin-console__header with-back">
          <div>
            <BlockableLink to="/admin_console/user_management/groups" className="fa fa-angle-left back" />
            <FormattedMessage
              ...
              button={
                <div className="group-profile-add-menu">
                  <MenuWrapper isDisabled={isDisabled}>
                    <button type="button" id="add_team_or_channel" className="btn btn-primary">
                      <FormattedMessage
        ...
        <SaveChangesPanel
          ...
          serverError={
            serverError && (
              <FormError iconClassName={'fa-exclamation-triangle'} textClassName={'has-error'} error={serverError} />
            )
          }
        />
      </div>
