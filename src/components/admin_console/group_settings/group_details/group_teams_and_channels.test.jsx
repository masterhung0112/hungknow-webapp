...
describe('components/admin_console/group_settings/group_details/GroupTeamsAndChannels', () => {
  ...
  test('should match snapshot, without teams, without channels and loading', () => {
    const wrapper = shallow(<GroupTeamsAndChannels {...defaultProps} teams={[]} channels={[]} loading={true} />)
    expect(wrapper).toMatchSnapshot()
  })

  test('should match snapshot, without teams, without channels and loaded', () => {
    const wrapper = shallow(<GroupTeamsAndChannels {...defaultProps} teams={[]} channels={[]} loading={false} />)
    expect(wrapper).toMatchSnapshot()
  })
  ...
  test('should invoke the onRemoveItem callback', async () => {
    ...
    const wrapper = shallow(
      <GroupTeamsAndChannels {...defaultProps} onChangeRoles={jest.fn()} onRemoveItem={onRemoveItem} />
    )
    const instance = wrapper.instance()
  ...
  test('should invoke the onChangeRoles callback', async () => {
    ...
    const wrapper = shallow(
      <GroupTeamsAndChannels {...defaultProps} onChangeRoles={onChangeRoles} onRemoveItem={jest.fn()} />
    )
    const instance = wrapper.instance()
