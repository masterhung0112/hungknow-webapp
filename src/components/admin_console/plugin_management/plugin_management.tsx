...
import PluginState from 'hkclient-ts/lib/constants/plugins'
...
const PluginItemState = ({ state }: { state: number }) => {
  switch (state) {
    case PluginState.PLUGIN_STATE_NOT_RUNNING:
      return <FormattedMessage id="admin.plugin.state.not_running" defaultMessage="Not running" />
    case PluginState.PLUGIN_STATE_STARTING:
      return <FormattedMessage id="admin.plugin.state.starting" defaultMessage="Starting" />
    case PluginState.PLUGIN_STATE_RUNNING:
      return <FormattedMessage id="admin.plugin.state.running" defaultMessage="Running" />
    case PluginState.PLUGIN_STATE_FAILED_TO_START:
      return <FormattedMessage id="admin.plugin.state.failed_to_start" defaultMessage="Failed to start" />
    case PluginState.PLUGIN_STATE_FAILED_TO_STAY_RUNNING:
      return <FormattedMessage id="admin.plugin.state.failed_to_stay_running" defaultMessage="Crashing" />
    case PluginState.PLUGIN_STATE_STOPPING:
      return <FormattedMessage id="admin.plugin.state.stopping" defaultMessage="Stopping" />
    default:
      return <FormattedMessage id="admin.plugin.state.unknown" defaultMessage="Unknown" />
  }
}
  ...
  switch (state) {
    ...
    case PluginState.PLUGIN_STATE_STARTING:
      return (
        <div className="alert alert-success">
          ...
          <FormattedMessage id="admin.plugin.state.starting.description" defaultMessage="This plugin is starting." />
        </div>
      )
    case PluginState.PLUGIN_STATE_RUNNING:
      return (
        <div className="alert alert-success">
          ...
          <FormattedMessage id="admin.plugin.state.running.description" defaultMessage="This plugin is running." />
        </div>
      )
    case PluginState.PLUGIN_STATE_FAILED_TO_START:
      return (
    ...
    case PluginState.PLUGIN_STATE_STOPPING:
      return (
        <div className="alert alert-info">
          <i className="fa fa-info" />
          <FormattedMessage id="admin.plugin.state.stopping.description" defaultMessage="This plugin is stopping." />
        </div>
      )
    default:
      return null
...
const PluginItem = ({
  ...
  if (pluginStatus.active) {
    activateButton = (
      ...
      >
        {deactivating ? (
          <FormattedMessage id="admin.plugin.disabling" defaultMessage="Disabling..." />
        ) : (
          <FormattedMessage id="admin.plugin.disable" defaultMessage="Disable" />
        )}
      </a>
    )
  } else {
    activateButton = (
      <a data-plugin-id={pluginStatus.id} className={activating || isDisabled ? 'disabled' : ''} onClick={handleEnable}>
        {activating ? (
          <FormattedMessage id="admin.plugin.enabling" defaultMessage="Enabling..." />
        ) : (
          <FormattedMessage id="admin.plugin.enable" defaultMessage="Enable" />
        )}
      </a>
    )
  }
    ...
    settingsButton = (
      <span>
        ...
        <Link to={'/admin_console/plugins/plugin_' + pluginStatus.id}>
          <FormattedMessage id="admin.plugin.settingsButton" defaultMessage="Settings" />
        </Link>
      </span>
    )
  }
  ...
  const removeButton = (
    <span>
      {' - '}
      <a data-plugin-id={pluginStatus.id} className={removing || isDisabled ? 'disabled' : ''} onClick={handleRemove}>
        {removeButtonText}
      </a>
    </span>
  )
  ...
  notices.push(<PluginItemStateDescription key="state-description" state={pluginStatus.state} />)
      ...
      <div className="pt-3 pb-3">
        <div className="row">
          <div className="col-md-6">
            <strong>
              <FormattedMessage id="admin.plugin.cluster_instance" defaultMessage="Cluster Instance" />
            </strong>
          </div>
          <div className="col-md-3">
            <strong>
              <FormattedMessage id="admin.plugin.version_title" defaultMessage="Version" />
            </strong>
          </div>
          <div className="col-md-3">
            <strong>
              <FormattedMessage id="admin.plugin.state" defaultMessage="State" />
            </strong>
          </div>
        </div>
...
export default class PluginManagement extends AdminSettings<Props, State> {
  ...
  componentDidMount() {
    if (this.state.enable) {
      this.props.actions.getPluginStatuses().then(() => this.setState({ loading: false }))
    }
  }
  ...
  renderTitle() {
    return <FormattedMessage id="admin.plugin.management.title" defaultMessage="Management" />
  }
  ...
  renderOverwritePluginModal = ({
    ...
    const title = (
      <FormattedMessage id="admin.plugin.upload.overwrite_modal.title" defaultMessage="Overwrite existing plugin?" />
    )
    ...
    const overwriteButton = (
      <FormattedMessage id="admin.plugin.upload.overwrite_modal.overwrite" defaultMessage="Overwrite" />
    )
  ...
  renderRemovePluginModal = (
    ...
    const title = <FormattedMessage id="admin.plugin.remove_modal.title" defaultMessage="Remove plugin?" />
    ...
    const removeButton = <FormattedMessage id="admin.plugin.remove_modal.overwrite" defaultMessage="Remove" />
  ...
  renderEnablePluginsSetting = () => {
    const hideEnablePlugins =
      this.props.config.ExperimentalSettings && this.props.config.ExperimentalSettings.RestrictSystemAdmin
    if (!hideEnablePlugins) {
      return (
        <BooleanSetting
          id="enable"
          label={<FormattedMessage id="admin.plugins.settings.enable" defaultMessage="Enable Plugins: " />}
          helpText={
            <FormattedMarkdownMessage
  ...
  renderSettings = () => {
    ...
    const enableUploadButton =
      enableUploads &&
      enable &&
      !(this.props.config.PluginSettings && this.props.config.PluginSettings.RequirePluginSignature)
    ...
    if (this.state.lastMessage) {
      lastMessage = (
        <div className="col-sm-12">
          <div className="form-group half">{this.state.lastMessage}</div>
        </div>
      )
    }
    ...
    if (this.state.loading) {
      ...
      pluginsListContainer = <FormattedMessage id="admin.plugin.no_plugins" defaultMessage="No installed plugins." />
    } else {
        ...
        const hasSettings = Boolean(
          p &&
            ...
            (p.settings_schema.header ||
              ...
              (p.settings_schema.settings && p.settings_schema.settings.length > 0))
        )
        return (
          <PluginItem
            key={pluginStatus.id}
      ...
      pluginsListContainer = <div className="alert alert-transparent">{pluginsList}</div>
    }
      ...
      pluginsContainer = (
        <div className="form-group">
          <label className="control-label col-sm-4">
            <FormattedMessage id="admin.plugin.installedTitle" defaultMessage="Installed Plugins: " />
          </label>
    ...
    const removePluginModal =
      ...
      this.renderRemovePluginModal(this.state.showRemoveModal, this.handleRemovePlugin, this.handleRemovePluginCancel)
            ...
            {!this.props.config.ExperimentalSettings?.RestrictSystemAdmin && (
              <>
                ...
                <TextSetting
                  ...
                  type={'input'}
                  label={
                    <FormattedMessage id="admin.plugins.settings.marketplaceUrl" defaultMessage="Marketplace URL:" />
                  }
