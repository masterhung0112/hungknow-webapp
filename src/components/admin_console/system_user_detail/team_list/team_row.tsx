...
export default class TeamRow extends React.PureComponent<Props> {
  private renderTeamType = (team: { [x: string]: string }) => {
    if (team.group_constrained) {
      return (
        <FormattedMessage id={'admin.systemUserDetail.teamList.teamType.groupSync'} defaultMessage={'Group sync'} />
      )
    }
    ...
    return (
      <FormattedMessage id={'admin.systemUserDetail.teamList.teamType.inviteOnly'} defaultMessage={'Invite only'} />
    )
  }
  private renderTeamRole = (team: { [x: string]: string }) => {
    if (team.scheme_guest) {
      return <FormattedMessage id={'admin.systemUserDetail.teamList.teamRole.guest'} defaultMessage={'Guest'} />
    }
    if (team.scheme_admin && !team.scheme_guest) {
      return <FormattedMessage id={'admin.systemUserDetail.teamList.teamRole.admin'} defaultMessage={'Team Admin'} />
    }
    if (team.scheme_user && !team.scheme_guest && !team.scheme_admin) {
      return <FormattedMessage id={'admin.systemUserDetail.teamList.teamRole.member'} defaultMessage={'Team Member'} />
    }
    return null
  }
        ...
        <div className="TeamRow__row">
          <div className="TeamRow__team-name">
            <div className="col-sm-auto">
              <TeamIcon size="sm" url={teamIconUrl} content={team.display_name} />
            </div>
            <div className="col-md-auto">
              <b>{team.display_name}</b>
              {team.description && (
                <div className="overflow--ellipsis text-nowrap TeamRow__team-description">{team.description}</div>
              )}
            </div>
          </div>

          <span className="TeamRow__description">{this.renderTeamType(team)}</span>

          <span className="TeamRow__description">{this.renderTeamRole(team)}</span>
