...
type Props = {
  ...
  actions: {
    getTeamsData: (
      userId: string
    ) => (ActionFunc & Partial<{ then: (func: () => void) => void }>) | Promise<Record<string, any>>
    removeGroup?: () => void
  }
}
      ...
      return (
        <div className="AbstractList__header">
          {this.props.headerLabels.map((headerLabel, id) => (
            <div key={id} className="AbstractList__header-label" style={headerLabel.style}>
              {headerLabel.default}
            </div>
          ))}
        </div>
    ...
    if (this.props.data.length === 0) {
      return (
        <div className="AbstractList__empty">
          <FormattedMessage id={this.props.emptyListTextId} defaultMessage={this.props.emptyListTextDefaultMessage} />
        </div>
      )
    }
  ...
  public render = (): JSX.Element => {
    ...
    return (
      <div className="AbstractList">
        ...
        <div className="AbstractList__body">{this.renderRows()}</div>
        {total > 0 && (
