...
import { Team, TeamSearchOpts } from 'hkclient-ts/lib/types/teams'
...
class TeamFilterDropdown extends React.PureComponent<Props, State> {
  ...
  generateButtonText = () => {
    ...
    if (selectedTeamIds.length === 0) {
      return {
        buttonText: <FormattedMessage id="admin.filter.all_teams" defaultMessage="All Teams" />,
        buttonMore: 0,
      }
    }
  ...
  render() {
    ...
    if (this.state.searchTerm.length === 0) {
      visibleTeams = this.props.teams
        ...
        .filter((team) => !this.state.savedSelectedTeams.some((selectedTeam) => selectedTeam.id === team.id))
    ...
    return (
      <div className="FilterList FilterList__full">
        ...
        <div className="TeamFilterDropdown" ref={this.ref}>
          <button type="button" className="TeamFilterDropdownButton" onClick={this.togglePopover}>
            <div className="TeamFilterDropdownButton_text">{buttonText}</div>
            ...
            <div className="TeamFilterDropdownButton_icon">{chevron}</div>
          </button>
          ...
          >
            ...
            {selectedTeamIds.length > 0 && (
              <a className="TeamFilterDropdown_reset" onClick={this.resetTeams}>
                <FormattedMessage id="admin.filter.reset_teams" defaultMessage="Reset to all teams" />
              </a>
            )}

            {selectedTeamIds.length === 0 && (
              <div className="TeamFilterDropdown_allTeams">
                <FormattedMessage id="admin.filter.showing_all_teams" defaultMessage="Showing all teams" />
              </div>
            )}

            <div className="TeamFilterDropdownOptions_list" ref={this.listRef} onScroll={this.setScrollPosition}>
              {selectedTeams}
              ...
              {this.state.loading && (
                <div className="TeamFilterDropdown_loading">
                  <LoadingSpinner />
                  <FormattedMessage id="admin.data_grid.loading" defaultMessage="Loading" />
                </div>
              )}

              {teamsToDisplay.length === 0 && selectedTeams.length === 0 && !this.state.loading && (
                <div className="TeamFilterDropdown_empty">
                  <FormattedMessage id="admin.filter.no_results" defaultMessage="No items match" />
                </div>
              )}
            </div>
          </div>
