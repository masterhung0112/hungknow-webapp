...
import { AdminConfig, ClientLicense } from 'hkclient-ts/lib/types/config'
import { TermsOfService } from 'hkclient-ts/lib/types/terms_of_service'
...
export default class CustomTermsOfServiceSettings extends AdminSettings<Props, State> {
  ...
  getStateFromConfig(config: Props['config']) {
    return {
      ...
      reAcceptancePeriod: this.parseIntNonZero(
        String(config.SupportSettings?.CustomTermsOfServiceReAcceptancePeriod),
  ...
  getConfigFromState = (config: Props['config']) => {
    if (config && config.SupportSettings) {
      ...
      config.SupportSettings.CustomTermsOfServiceReAcceptancePeriod = this.parseIntNonZero(
        String(this.state.reAcceptancePeriod),
        Constants.DEFAULT_TERMS_OF_SERVICE_RE_ACCEPTANCE_PERIOD
      )
    }
    return config
  ...
  renderTitle() {
    return <FormattedMessage id="admin.support.termsOfServiceTitle" defaultMessage="Custom Terms of Service (Beta)" />
  }
      ...
      <SettingsGroup>
        <BooleanSetting
          ...
          disabled={
            ...
            !(this.props.license.IsLicensed && this.props.license.CustomTermsOfService === 'true')
          }
        />
        <TextSetting
