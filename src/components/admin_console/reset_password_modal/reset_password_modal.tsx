...
export default class ResetPasswordModal extends React.PureComponent<Props, State> {
  ...
  public render(): JSX.Element {
    ...
    if (user.auth_service) {
      title = (
        <FormattedMessage id="admin.reset_password.titleSwitch" defaultMessage="Switch Account to Email/Password" />
      )
    } else {
      title = <FormattedMessage id="admin.reset_password.titleReset" defaultMessage="Reset Password" />
    }
    ...
    if (this.props.currentUserId === user.id) {
      ...
      if (this.state.serverErrorCurrentPass) {
        ...
        serverErrorCurrentPass = (
          <div className="has-error">
            <p className="input__help error">{this.state.serverErrorCurrentPass}</p>
          </div>
        )
      }
      currentPassword = (
        <div className="col-sm-10 password__group-addon-space">
          <div className={urlClassCurrentPass}>
            <span data-toggle="tooltip" title="Current Password" className="input-group-addon password__group-addon">
              <FormattedMessage id="admin.reset_password.curentPassword" defaultMessage="Current Password" />
            </span>
            <input type="password" ref={this.currentPasswordRef} className="form-control" autoFocus={true} />
          </div>
        </div>
      )
    }
    ...
    return (
      ...
      >
        ...
        <form role="form" className="form-horizontal">
            ...
            <div className="form-group">
              ...
              <div className="col-sm-10">
                <div className={urlClass}>
                  <span data-toggle="tooltip" title="New Password" className="input-group-addon password__group-addon">
                    <FormattedMessage id="admin.reset_password.newPassword" defaultMessage="New Password" />
                  </span>
                  <input type="password" ref={this.passwordRef} className="form-control" autoFocus={newPasswordFocus} />
                </div>
                {serverErrorNewPass}
                {serverErrorCurrentPass}
              </div>
          ...
          <Modal.Footer>
            <button type="button" className="btn btn-link" onClick={this.doCancel}>
              <FormattedMessage id="admin.reset_password.cancel" defaultMessage="Cancel" />
            </button>
            <button onClick={this.doSubmit} type="submit" className="btn btn-primary">
              <FormattedMessage id="admin.reset_password.reset" defaultMessage="Reset" />
            </button>
          </Modal.Footer>
        </form>
      </Modal>
