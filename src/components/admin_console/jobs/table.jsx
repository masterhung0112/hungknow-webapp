...
class JobTable extends React.PureComponent {
  ...
  getDownloadLink = (job) => {
    if (
      ...
      return (
        <a key={job.id} href={`${Client4.getJobsRoute()}/${job.id}/download`} target="_blank" rel="noopener noreferrer">
          <FormattedMessage id="admin.jobTable.downloadLink" defaultMessage="Download" />
        </a>
      )
    }
  ...
  getStatus = (job) => {
    ...
    if (job.status === JobStatuses.PENDING) {
      ...
      return (
        <span
          ...
          title={formatMessage({ id: 'admin.jobTable.jobId', defaultMessage: 'Job ID: ' }) + job.id}
        >
          <FormattedMessage id="admin.jobTable.statusWarning" defaultMessage="Warning" />
        </span>
      )
    ...
    return (
      <span title={formatMessage({ id: 'admin.jobTable.jobId', defaultMessage: 'Job ID: ' }) + job.id}>
        {job.status}
      </span>
    )
  }
    ...
    var items = this.props.jobs.map((job) => {
      return (
        <tr key={job.id}>
          ...
          {showFilesColumn && <td className="whitespace--nowrap">{this.getDownloadLink(job)}</td>}
          ...
          <table className="table" data-testid="jobTable">
            <thead>
              <tr>
                ...
                {showFilesColumn && (
                  <th>
                    <FormattedMessage id="admin.jobTable.headerFiles" defaultMessage="Files" />
                  </th>
                )}
                <th>
                ...
                <th colSpan="3">
                  <FormattedMessage id="admin.jobTable.headerExtraInfo" defaultMessage="Details" />
                </th>
              </tr>
            </thead>
            <tbody>{items}</tbody>
