...
export default class LicenseSettings extends React.PureComponent {
  ...
  render() {
    ...
    if (!this.props.enterpriseReady) {
      ...
      edition = (
        <div>
          <p>
            {
              'Mattermost Team Edition. Upgrade to Mattermost Enterprise Edition to add the ability to unlock enterprise features.'
            }
          </p>
          {this.state.upgradingPercentage !== 100 && (
            <div>
              <p>
                <button type="button" onClick={this.handleUpgrade} className="btn btn-primary">
                  <LoadingWrapper
          ...
          {this.state.upgradingPercentage === 100 && (
            <div>
              ...
              <p>
                <button type="button" onClick={this.handleRestart} className="btn btn-primary">
                  <LoadingWrapper
                    loading={this.state.restarting}
                    text={Utils.localizeMessage('admin.license.enterprise.restarting', 'Restarting')}
                  >
                    <FormattedMessage id="admin.license.enterprise.restart" defaultMessage="Restart Server" />
                  </LoadingWrapper>
      ...
      licenseType = (
        <div>
          ...
          <p>
            {
              'When using Mattermost Enterprise Edition, the software is offered under a commercial license. See below for “Enterprise Edition License” for details.'
            }
          </p>
      ...
      const sku = license.SkuShortName ? (
        <React.Fragment>
          {`Edition: Mattermost Enterprise Edition ${license.SkuShortName}`}
          <br />
        </React.Fragment>
      ) : null
      edition =
        'Mattermost Enterprise Edition. Enterprise features on this server have been unlocked with a license key and a valid subscription.'
      if (upgradedFromTE) {
        eelicense = this.renderEELicenseText()
      }
      licenseType = (
        <div>
          ...
          <br />
          {'See also '}
          <a rel="noopener noreferrer" target="_blank" href="https://about.mattermost.com/enterprise-edition-terms/">
      ...
      edition = (
        <div>
          ...
          <p className="trial">
            <button type="button" className="btn btn-primary" onClick={this.requestLicense} disabled={isDisabled}>
              <LoadingWrapper
                loading={this.state.gettingTrial}
                text={Utils.localizeMessage('admin.license.trial-request.loading', 'Getting trial')}
              >
                <FormattedMessage id="admin.license.trial-request.submit" defaultMessage="Start trial" />
              </LoadingWrapper>
            </button>
          </p>
          {gettingTrialError}
    ...
    return (
      <div className="wrapper--fixed">
        <FormattedAdminHeader id="admin.license.title" defaultMessage="Edition and License" />
            ...
            <form className="form-horizontal" role="form">
              ...
              <div className="form-group">
                <label className="control-label col-sm-4">
                  <FormattedMessage id="admin.license.edition" defaultMessage="Edition: " />
                </label>
                <div className="col-sm-8">{edition}</div>
              </div>
              <div className="form-group">
                <label className="control-label col-sm-4">
                  <FormattedMessage id="admin.license.type" defaultMessage="License: " />
                </label>
                <div className="col-sm-8">{licenseType}</div>
              </div>
              {licenseContent && <div className="form-group">{licenseContent}</div>}
              {eelicense && <div className="form-group">{eelicense}</div>}
            </form>
          </div>
        </div>
      </div>
  ...
  renderE10E20Content = () => {
    let removeButtonText = (
      <FormattedMessage id="admin.license.keyRemove" defaultMessage="Remove Enterprise License and Downgrade Server" />
    )
    if (this.state.removing) {
      removeButtonText = <FormattedMessage id="admin.license.removing" defaultMessage="Removing License..." />
    }

    return (
      <>
        <label className="control-label col-sm-4">
          <FormattedMessage id="admin.license.key" defaultMessage="License Key: " />
        </label>
  ...
  renderE0Content = () => {
    ...
    if (this.state.fileName) {
      ...
      fileName = <FormattedMessage id="admin.license.noFile" defaultMessage="No file uploaded" />
    }

    let uploadButtonText = <FormattedMessage id="admin.license.upload" defaultMessage="Upload" />
    if (this.state.uploading) {
      uploadButtonText = <FormattedMessage id="admin.license.uploading" defaultMessage="Uploading License..." />
    }
    return (
      <>
        <label className="control-label col-sm-4">
        ...
        <div className="col-sm-8">
          <div className="file__upload">
            <button type="button" className="btn btn-primary">
              <FormattedMessage id="admin.license.choose" defaultMessage="Choose File" />
            </button>
            <input
