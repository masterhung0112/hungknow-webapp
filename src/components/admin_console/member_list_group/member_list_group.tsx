...
import { GroupStats } from 'hkclient-ts/lib/types/groups'
...
export default class MemberListGroup extends React.PureComponent<Props, State> {
  ...
  async componentDidMount() {
    ...
    await Promise.all([actions.getProfilesInGroup(groupID, 0, USERS_PER_PAGE * 2), actions.getGroupStats(groupID)])
    this.loadComplete()
  }
  ...
  private getColumns = (): Column[] => {
    return [
      {
        name: <FormattedMessage id="admin.member_list_group.name" defaultMessage="Name" />,
        field: 'name',
      },
    ]
  }
