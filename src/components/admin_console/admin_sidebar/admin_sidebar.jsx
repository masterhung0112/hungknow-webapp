...
const renderScrollView = (props) => <div {...props} className="scrollbar--view" />

const renderScrollThumbHorizontal = (props) => <div {...props} className="scrollbar--horizontal" />

const renderScrollThumbVertical = (props) => <div {...props} className="scrollbar--vertical" />
...
class AdminSidebar extends React.PureComponent {
      ...
      if (section.isHidden) {
        isSectionHidden =
          typeof section.isHidden === 'function'
            ? section.isHidden(config, this.state, license, buildEnterpriseReady, consoleAccess, cloud)
        ...
        Object.entries(section).forEach(([subKey, item]) => {
          ...
          if (item.isHidden) {
            if (
              typeof item.isHidden === 'function'
                ? item.isHidden(config, this.state, license, buildEnterpriseReady, consoleAccess, cloud)
          ...
          sidebarItems.push(
            <AdminSidebarSection
              ...
              name={item.url}
              tag={tag}
              title={<FormattedMessage id={item.title} defaultMessage={item.title_default} />}
            />
          )
        })
        ...
        if (sidebarItems.length || moreSidebarItems.length) {
          sidebarSections.push(
            <AdminSidebarCategory
              ...
              icon={section.icon}
              sectionClass=""
              title={<FormattedMessage id={section.sectionTitle} defaultMessage={section.sectionTitleDefault} />}
            >
              {sidebarItems}
              {moreSidebarItems}
            </AdminSidebarCategory>
  ...
  renderPluginsMenu = () => {
    if (this.props.config.PluginSettings.Enable) {
      return Object.values(this.props.plugins)
        .sort((a, b) => {
        ...
        >
          <div className="nav-pills__container">
            <Highlight filter={this.state.filter}>
              <ul className="nav nav-pills nav-stacked">{this.renderRootMenu(this.props.adminDefinition)}</ul>
            </Highlight>
          </div>
        </Scrollbars>
      </div>
