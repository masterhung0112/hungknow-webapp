...
describe('components/admin_console/permission_schemes_settings/permission_tree', () => {
  ...
  test('should match snapshot on read only', () => {
    const wrapper = shallow(<PermissionsTree {...defaultProps} readOnly={true} />)
    expect(wrapper).toMatchSnapshot()
  })

  test('should match snapshot on team scope', () => {
    const wrapper = shallow(<PermissionsTree {...defaultProps} scope={'team_scope'} />)
    expect(wrapper).toMatchSnapshot()
  })

  test('should match snapshot on system scope', () => {
    const wrapper = shallow(<PermissionsTree {...defaultProps} scope={'system_scope'} />)
    expect(wrapper).toMatchSnapshot()
  })

  test('should match snapshot on license without LDAPGroups', () => {
    const wrapper = shallow(<PermissionsTree {...defaultProps} license={{}} />)
    expect(wrapper).toMatchSnapshot()
  })

  test('should match snapshot with parentRole with permissions', () => {
    const wrapper = shallow(
      <PermissionsTree {...defaultProps} parentRole={{ permissions: 'invite_user' }} scope={'system_scope'} />
    )
    expect(wrapper).toMatchSnapshot()
  })
  ...
  test('should ask to toggle on row toggle', () => {
    ...
    const wrapper = shallow(<PermissionsTree {...defaultProps} onToggle={onToggle} />)
