...
describe('components/admin_console/permission_schemes_settings/permission_row', () => {
  ...
  test('should match snapshot on editable and inherited', () => {
    const wrapper = shallow(<PermissionRow {...defaultProps} inherited={{ name: 'test', displayName: 'Test' }} />)
    expect(wrapper).toMatchSnapshot()
  })

  test('should match snapshot on read only and not inherited', () => {
    const wrapper = shallow(<PermissionRow {...defaultProps} readOnly={true} />)
    expect(wrapper).toMatchSnapshot()
  })
  ...
  test('should call onChange function on click', () => {
    ...
    const wrapper = shallow(<PermissionRow {...defaultProps} onChange={onChange} />)
  ...
  test("shouldn't call onChange function on click when is read-only", () => {
    const onChange = jest.fn()
    const wrapper = shallow(<PermissionRow {...defaultProps} readOnly={true} onChange={onChange} />)
    wrapper.find('div').first().simulate('click')
