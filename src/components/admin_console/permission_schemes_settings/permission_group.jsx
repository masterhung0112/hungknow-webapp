...
export default class PermissionGroup extends React.PureComponent {
  ...
  getStatus = (permissions) => {
    ...
    for (const permission of permissions) {
      if (typeof permission === 'string') {
        ...
        anyUnchecked =
          anyUnchecked || (!this.fromParent(permission) && this.props.role.permissions.indexOf(permission) === -1)
      } else {
  ...
  render = () => {
    ...
    if (root) {
      return (
        <div className={'permission-group-permissions ' + (this.state.expanded ? 'open' : '')}>{permissionsRows}</div>
      )
    }
    ...
    return (
      <div className="permission-group">
        {!root && (
          <div className={'permission-group-row ' + classes} onClick={this.toggleSelectGroup} id={uniqId}>
            {!combined && (
              <div
            ...
            <PermissionCheckbox value={this.getStatus(this.props.permissions)} id={`${uniqId}-checkbox`} />
