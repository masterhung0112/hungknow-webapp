...
describe('components/admin_console/permission_schemes_settings/permission_group', () => {
  ...
  test('should match snapshot on editable without every permission out of the scope', () => {
    const wrapper = shallow(<PermissionGroup {...defaultProps} scope={'system_scope'} />)
    expect(wrapper).toMatchSnapshot()
  })

  test('should match snapshot on editable with some permissions', () => {
    const wrapper = shallow(<PermissionGroup {...defaultProps} role={{ permissions: ['invite_user'] }} />)
    expect(wrapper).toMatchSnapshot()
  })

  test('should match snapshot on editable with all permissions', () => {
    const wrapper = shallow(
      <PermissionGroup {...defaultProps} role={{ permissions: ['invite_user', 'add_user_to_team'] }} />
    )
    expect(wrapper).toMatchSnapshot()
  })

  test('should match snapshot on editable without permissions and read-only', () => {
    const wrapper = shallow(<PermissionGroup {...defaultProps} readOnly={true} />)
    expect(wrapper).toMatchSnapshot()
  })

  test('should match snapshot on editable with some permissions and read-only', () => {
    const wrapper = shallow(
      <PermissionGroup {...defaultProps} role={{ permissions: ['invite_user'] }} readOnly={true} />
    )
    expect(wrapper).toMatchSnapshot()
  })
  ...
  test('should match snapshot on editable with all permissions and read-only', () => {
    const wrapper = shallow(
      <PermissionGroup {...defaultProps} role={{ permissions: ['invite_user', 'add_user_to_team'] }} readOnly={true} />
    )
    expect(wrapper).toMatchSnapshot()
  })

  test('should match snapshot on editable with some permissions from parentRole', () => {
    const wrapper = shallow(<PermissionGroup {...defaultProps} parentRole={{ permissions: ['invite_user'] }} />)
    expect(wrapper).toMatchSnapshot()
  })

  test('should match snapshot on editable with all permissions from parentRole', () => {
    const wrapper = shallow(
      <PermissionGroup {...defaultProps} parentRole={{ permissions: ['invite_user', 'add_user_to_team'] }} />
    )
    expect(wrapper).toMatchSnapshot()
  })
  ...
  test('should call correctly onChange function on click without permissions', () => {
    ...
    const wrapper = shallow(<PermissionGroup {...defaultProps} onChange={onChange} />)
  ...
  test('should call correctly onChange function on click with some permissions', () => {
    const onChange = jest.fn()
    const wrapper = shallow(
      <PermissionGroup {...defaultProps} role={{ permissions: ['invite_user'] }} onChange={onChange} />
    )
    wrapper.find('.permission-group-row').first().simulate('click')
  ...
  test("shouldn't call onChange function on click when is read-only", () => {
    const onChange = jest.fn()
    const wrapper = shallow(<PermissionGroup {...defaultProps} readOnly={true} onChange={onChange} />)
    wrapper.find('.permission-group-row').first().simulate('click')
  ...
  test('should collapse when toggle to all permissions and expand otherwise', () => {
    let wrapper = shallow(<PermissionGroup {...defaultProps} role={{ permissions: ['invite_user'] }} />)
    expect(wrapper.state().expanded).toBe(true)
    wrapper.instance().toggleSelectGroup()
    expect(wrapper.state().expanded).toBe(false)

    wrapper = shallow(<PermissionGroup {...defaultProps} role={{ permissions: ['invite_user', 'add_user_to_team'] }} />)
    wrapper.setState({ expanded: false })
    wrapper.instance().toggleSelectGroup()
    expect(wrapper.state().expanded).toBe(true)

    wrapper = shallow(<PermissionGroup {...defaultProps} role={{ permissions: [] }} />)
  ...
  test('should toggle correctly between states', () => {
    ...
    onChange = jest.fn()
    wrapper = shallow(<PermissionGroup {...defaultProps} role={{ permissions: [] }} onChange={onChange} />)
    wrapper.setState({ prevPermissions: ['invite_user'] })
    wrapper.instance().toggleSelectGroup()
