...
export default class UploadButton extends React.PureComponent {
  ...
  render() {
    ...
    if (status === UploadStatuses.COMPLETE) {
      buttonContent = [<SuccessIcon key="icon" />, ' ', Utils.localizeMessage('admin.team.uploaded', 'Uploaded!')]
    } else {
      buttonContent = Utils.localizeMessage('admin.team.upload', 'Upload')
    }

    return (
      <button type="button" className={primaryClass} disabled={disabled} onClick={onClick} id="upload-button">
        <LoadingWrapper
