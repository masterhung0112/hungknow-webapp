...
export default class GeneratedSetting extends React.PureComponent<Props> {
  public static get defaultProps() {
    return {
      disabled: false,
      regenerateText: <FormattedMessage id="admin.regenerate" defaultMessage="Regenerate" />,
    }
  }
  ...
  public render() {
    ...
    if (this.props.disabled && this.props.disabledText) {
      disabledText = <div className="admin-console__disabled-text">{this.props.disabledText}</div>
    }

    let regenerateHelpText = null
    if (this.props.regenerateHelpText) {
      regenerateHelpText = <div className="help-text">{this.props.regenerateHelpText}</div>
    }

    let text: React.ReactNode = this.props.value
    if (!text) {
      text = <span className="placeholder-text">{this.props.placeholder}</span>
    }

    return (
      <div className="form-group">
        <label className="control-label col-sm-4" htmlFor={this.props.id}>
          {this.props.label}
        </label>
