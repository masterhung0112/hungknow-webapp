...
describe('components/admin_console/team_channel_settings/group/UsersToRemoveGroups', () => {
  ...
  test('should match snapshot with 0 groups', () => {
    const wrapper = shallow(<UsersToRemoveGroups user={userWithGroups(user, [])} />)
    expect(wrapper).toMatchSnapshot()
  })

  test('should match snapshot with 1 group', () => {
    const wrapper = shallow(<UsersToRemoveGroups user={userWithGroups(user, [group1])} />)
    expect(wrapper).toMatchSnapshot()
  })

  test('should match snapshot with 3 groups', () => {
    const wrapper = shallow(<UsersToRemoveGroups user={userWithGroups(user, [group1, group2, group3])} />)
    expect(wrapper).toMatchSnapshot()
  })
})
