...
import { ChannelMembership } from 'hkclient-ts/lib/types/channels'
import { RelationOneToOne } from 'hkclient-ts/lib/types/utilities'
import GeneralConstants from 'hkclient-ts/lib/constants/general'
...
export default class UsersToRemove extends React.PureComponent<Props, State> {
  ...
  async componentDidMount() {
    ...
    while (membershipsLoaded < total) {
      promises.push(
        this.loadMembersForProfilesList(members.slice(membershipsLoaded, membershipsLoaded + MEMBERSHIPS_TO_LOAD_COUNT))
      )
    ...
    return usersToDisplay.map((user) => {
      return {
        cells: {
          ...
          role: <GroupUsersRole key={user.id} user={user} membership={memberships[user.id]} scope={scope} />,
  ...
  private getColumns = (): Column[] => {
    return [
      {
        name: <FormattedMessage id="admin.team_channel_settings.user_list.nameHeader" defaultMessage="Name" />,
        field: 'name',
        width: 5,
      },
      {
        name: <FormattedMessage id="admin.team_channel_settings.user_list.roleHeader" defaultMessage="Role" />,
        field: 'role',
        width: 2,
      },
      {
        name: <FormattedMessage id="admin.team_channel_settings.user_list.groupsHeader" defaultMessage="Groups" />,
        field: 'groups',
        width: 3,
      },
    ]
  ...
  private getFilterOptions = (): FilterOptions => {
    const filterOptions: FilterOptions = {
      role: {
        name: <FormattedMessage id="admin.user_grid.role" defaultMessage="Role" />,
        values: {
          [GeneralConstants.SYSTEM_GUEST_ROLE]: {
            name: <FormattedMessage id="admin.user_grid.guest" defaultMessage="Guest" />,
            value: false,
          },
          [GeneralConstants.SYSTEM_ADMIN_ROLE]: {
            name: <FormattedMessage id="admin.user_grid.system_admin" defaultMessage="System Admin" />,
            value: false,
          },
        },
    ...
    if (this.props.scope === 'channel') {
      filterOptions.role.values = {
        ...
        [GeneralConstants.CHANNEL_USER_ROLE]: {
          name: <FormattedMessage id="admin.user_item.member" defaultMessage="Member" />,
          value: false,
        },
        [GeneralConstants.CHANNEL_ADMIN_ROLE]: {
          name: <FormattedMessage id="admin.user_grid.channel_admin" defaultMessage="Channel Admin" />,
          value: false,
        },
      }
      ...
      filterOptions.role.values = {
        ...
        [GeneralConstants.TEAM_ADMIN_ROLE]: {
          name: <FormattedMessage id="admin.user_grid.team_admin" defaultMessage="Team Admin" />,
          value: false,
        },
      }
      filterOptions.role.keys = [
  ...
  public render = (): JSX.Element => {
    ...
    const placeholderEmpty: JSX.Element = (
      <FormattedMessage id="admin.member_list_group.notFound" defaultMessage="No users found" />
    )
