...
import { TeamMembership, Team } from 'hkclient-ts/lib/types/teams'
  ...
  public async componentDidUpdate(prevProps: Props) {
    ...
    if (filtersModified || searchTermModified) {
      ...
      if (searchTerm === '') {
        this.searchTimeoutId = 0
        if (filtersModified) {
          await prevProps.actions.loadProfilesAndReloadTeamMembers(0, PROFILE_CHUNK_SIZE * 2, prevProps.teamId, {
            active: true,
            ...filters,
          })
        }
      ...
      const searchTimeoutId = window.setTimeout(async () => {
        await prevProps.actions.searchProfilesAndTeamMembers(searchTerm, {
          ...filters,
    ...
    return (
      <AdminPanel
        ...
        button={
          ...
          >
            <FormattedMessage id="admin.team_settings.team_details.add_members" defaultMessage="Add Members" />
          </ToggleModalButton>
        }
      >
        <UserGrid
