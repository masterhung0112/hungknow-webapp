...
import { Team, TeamSearchOpts, TeamsWithCount } from 'hkclient-ts/lib/types/teams'
...
export default class TeamList extends React.PureComponent<Props, State> {
  ...
  getColumns = (): Column[] => {
    const name = <FormattedMessage id="admin.team_settings.team_list.nameHeader" defaultMessage="Name" />
    const management = <FormattedMessage id="admin.team_settings.team_list.mappingHeader" defaultMessage="Management" />
  ...
  renderManagementMethodText = (team: Team) => {
    if (team.group_constrained) {
      return (
        <FormattedMessage id="admin.team_settings.team_row.managementMethod.groupSync" defaultMessage="Group Sync" />
      )
    ...
    return (
      <FormattedMessage id="admin.team_settings.team_row.managementMethod.inviteOnly" defaultMessage="Invite Only" />
    )
  }
          ...
          name: (
            <div className="TeamList_nameColumn">
              <div className="TeamList__lowerOpacity">
                <TeamIcon size="sm" url={Utils.imageURLForTeam(team)} content={team.display_name} />
              </div>
              <div className="TeamList_nameText">
                <b data-testid="team-display-name">{team.display_name}</b>
                {team.description && <div className="TeamList_descriptionText">{team.description}</div>}
              </div>
            </div>
          ),
          management: (
            <span data-testid={`${team.name}Management`} className="TeamList_managementText">
              {this.renderManagementMethodText(team)}
            </span>
          ),
          edit: (
            <span data-testid={`${team.display_name}edit`} className="group-actions TeamList_editText">
              <Link to={`/admin_console/user_management/teams/${team.id}`}>
                <FormattedMessage id="admin.team_settings.team_row.configure" defaultMessage="Edit" />
              </Link>
            </span>
          ),
        },
  ...
  render() {
    ...
    let placeholderEmpty = (
      <FormattedMessage id="admin.team_settings.team_list.no_teams_found" defaultMessage="No teams found" />
    )
    ...
    const filterOptions = {
      management: {
        name: <FormattedMessage id="admin.team_settings.team_list.mappingHeader" defaultMessage="Management" />,
        values: {
          allow_open_invite: {
            name: (
              <FormattedMessage
