...
import { debounce } from 'hkclient-ts/lib/actions/helpers'
...
export default class ChannelList extends React.PureComponent<ChannelListProps, ChannelListState> {
  ...
  searchChannels = async (page = 0, term = '', filters = {}) => {
    ...
    const response = await this.props.actions.searchAllChannels(term, {
      ...filters,
      page,
      per_page: PAGE_SIZE,
  ...
  searchChannelsDebounced = debounce(
    (page, term, filters = {}) => this.searchChannels(page, term, filters),
    300,
    false,
    () => {}
  )
  ...
  getColumns = (): Column[] => {
    const name: JSX.Element = (
      <FormattedMessage id="admin.channel_settings.channel_list.nameHeader" defaultMessage="Name" />
    )
    const team: JSX.Element = (
      <FormattedMessage id="admin.channel_settings.channel_list.teamHeader" defaultMessage="Team" />
    )
    const management: JSX.Element = (
      <FormattedMessage id="admin.channel_settings.channel_list.managementHeader" defaultMessage="Management" />
    )
  ...
  getRows = (): Row[] => {
    ...
    return channelsToDisplay.map((channel) => {
      ...
      if (isArchivedChannel(channel)) {
        iconToDisplay = <ArchiveIcon className="channel-icon" data-testid={`${channel.name}-archive-icon`} />
      }
        ...
        cells: {
          ...
          name: (
            <span className="group-name overflow--ellipsis row-content" data-testid="channel-display-name">
              {iconToDisplay}
              <span className="TeamList_channelDisplayName">{channel.display_name}</span>
            </span>
          ),
          team: <span className="group-description row-content">{channel.team_display_name}</span>,
          management: (
            <span className="group-description adjusted row-content">
              <FormattedMessage
                id={`admin.channel_settings.channel_row.managementMethod.${
                  channel.group_constrained ? 'group' : 'manual'
                }`}
          ...
          edit: (
            <span className="group-actions TeamList_editRow" data-testid={`${channel.name}edit`}>
              <Link to={`/admin_console/user_management/channels/${channel.id}`}>
                <FormattedMessage id="admin.channel_settings.channel_row.configure" defaultMessage="Edit" />
              </Link>
            </span>
          ),
        },
  ...
  onFilter = (filterOptions: FilterOptions) => {
    ...
    const {
      ...
      exclude_group_constrained: excludeGroupConstrained,
    } = filterOptions.management.values
    ...
    if (
      ...
      deleted.value ||
      groupConstrained.value ||
      excludeGroupConstrained.value ||
  ...
  render = (): JSX.Element => {
    ...
    let placeholderEmpty: JSX.Element = (
      <FormattedMessage id="admin.channel_settings.channel_list.no_channels_found" defaultMessage="No channels found" />
    )
    ...
    const filterOptions: FilterOptions = {
      teams: {
        ...
        values: {
          team_ids: {
            name: <FormattedMessage id="admin.team_settings.title" defaultMessage="Teams" />,
            value: [],
          },
        },
        keys: ['team_ids'],
      ...
      management: {
        ...
        values: {
          group_constrained: {
            name: <FormattedMessage id="admin.channel_list.group_sync" defaultMessage="Group Sync" />,
            value: false,
          },
          exclude_group_constrained: {
            name: <FormattedMessage id="admin.channel_list.manual_invites" defaultMessage="Manual Invites" />,
            value: false,
          },
        },
      ...
      channels: {
        ...
        values: {
          public: {
            name: <FormattedMessage id="admin.channel_list.public" defaultMessage="Public" />,
            value: false,
          },
          private: {
            name: <FormattedMessage id="admin.channel_list.private" defaultMessage="Private" />,
            value: false,
          },
          deleted: {
            name: <FormattedMessage id="admin.channel_list.archived" defaultMessage="Archived" />,
            value: false,
          },
        },
