...
import { Groups, Permissions } from 'hkclient-ts/lib/constants'
...
import { Scheme } from 'hkclient-ts/lib/types/schemes'
import { ChannelModerationRoles } from 'hkclient-ts/lib/types/roles'
import { SyncablePatch, Group, SyncableType } from 'hkclient-ts/lib/types/groups'
import {
  Channel,
  ChannelModeration as ChannelPermissions,
  ChannelModerationPatch,
} from 'hkclient-ts/lib/types/channels'
...
export default class ChannelDetails extends React.PureComponent<ChannelDetailsProps, ChannelDetailsState> {
  ...
  private restrictChannelMentions() {
    ...
    const currentCreatePostRoles: any = channelPermissions!.find(
      (element) => element.name === Permissions.CHANNEL_MODERATED_PERMISSIONS.CREATE_POST
    )?.roles
    for (const channelRole of Object.keys(currentCreatePostRoles)) {
      channelPermissions = channelPermissions!.map((permission) => {
        if (
          permission.name === Permissions.CHANNEL_MODERATED_PERMISSIONS.USE_CHANNEL_MENTIONS &&
          (!currentCreatePostRoles[channelRole].value || !currentCreatePostRoles[channelRole].enabled)
        ) {
          return {
  ...
  async processGroupsChange(groups: Group[]) {
    ...
    if (this.state.isSynced) {
      try {
        if (groups.length === 0) {
          serverError = <NeedGroupsError warning={true} isChannel={true} />
        } else {
          if (!channelID) {
            return
          }
  ...
  private channelPermissionsChanged = (name: string, channelRole: ChannelModerationRoles) => {
    ...
    if (name === Permissions.CHANNEL_MODERATED_PERMISSIONS.CREATE_POST) {
      const originalObj = this.props.channelPermissions!.find(
        (element) => element.name === Permissions.CHANNEL_MODERATED_PERMISSIONS.USE_CHANNEL_MENTIONS
      )?.roles![channelRole]
      channelPermissions = channelPermissions.map((permission) => {
  ...
  private handleSubmit = async () => {
    ...
    const patchChannelSyncable = groups
      ...
      .map((g) =>
        actions.patchGroupSyncable(g.id, channelID, Groups.SYNCABLE_TYPE_CHANNEL, { scheme_admin: g.scheme_admin })
      )
    ...
    const link = groups
      ...
      .map((g) =>
        actions.linkGroupSyncable(g.id, channelID, Groups.SYNCABLE_TYPE_CHANNEL, {
          auto_add: true,
    ...
    if (groupActions.length > 0) {
      ...
      if (resultWithError && 'error' in resultWithError) {
        ...
        if (unlink.length > 0) {
          trackEvent('admin_channel_config_page', 'groups_removed_from_channel', {
            count: unlink.length,
            channel_id: channelID,
          })
        }
        if (link.length > 0) {
          trackEvent('admin_channel_config_page', 'groups_added_to_channel', {
            count: link.length,
            channel_id: channelID,
          })
        }
        ...
        if (isPrivacyChanging) {
          ...
          actionsToAwait.push(
            actions.getChannelModerations(channelID).then(() => {
              const manageMembersIndex = channelPermissions!.findIndex(
                (element) => element.name === Permissions.CHANNEL_MODERATED_PERMISSIONS.MANAGE_MEMBERS
              )
              if (channelPermissions) {
                const updatedManageMembers = this.props.channelPermissions!.find(
                  (element) => element.name === Permissions.CHANNEL_MODERATED_PERMISSIONS.MANAGE_MEMBERS
                )
    ...
    return (
      <div className="wrapper--fixed">
        <div className="admin-console__header with-back">
          <div>
            <BlockableLink to="/admin_console/user_management/channels" className="fa fa-angle-left back" />
            <FormattedMessage
