...
import { ChannelModeration as ChannelPermissions } from 'hkclient-ts/lib/types/channels'
import { Permissions, Roles } from 'hkclient-ts/lib/constants'
...
export const ChannelModerationTableRow: React.FunctionComponent<RowProps> = (props: RowProps): JSX.Element => {
  ...
  return (
    <tr>
      <td>
        <label data-testid={formattedMessages[props.name].title.id.replace(PERIOD_TO_SLASH_REGEX, '-')}>
          <FormattedMessage
            id={formattedMessages[props.name].title.id}
            defaultMessage={formattedMessages[props.name].title.defaultMessage}
          />
        </label>
        <div data-testid={formattedMessages[props.name].description.id.replace(PERIOD_TO_SLASH_REGEX, '-')}>
          <FormattedMessage id={descriptionId} defaultMessage={descriptionDefaultMessage} />
        </div>
        {props.errorMessages}
      </td>
...
export default class ChannelModeration extends React.PureComponent<Props> {
  private errorMessagesToDisplay = (entry: ChannelPermissions): JSX.Element[] => {
    ...
    const isGuestsDisabled =
      !isNil(entry.roles.guests?.enabled) && !entry.roles.guests?.enabled && this.props.guestAccountsEnabled
    ...
    if (entry.name === Permissions.CHANNEL_MODERATED_PERMISSIONS.USE_CHANNEL_MENTIONS) {
      const createPostsObject =
        ...
        this.props.channelPermissions!.find(
          (permission) => permission.name === Permissions.CHANNEL_MODERATED_PERMISSIONS.CREATE_POST
        )
      if (
        ...
        errorMessages.push(
          <div
            data-testid={formattedMessages[entry.name].disabledBothDueToCreatePosts.id.replace(
      ...
      <AdminPanel
        ...
        subtitleDefault={
          guestAccountsEnabled
            ? formattedMessages.subtitle.defaultMessage
            : formattedMessages.subtitleMembers.defaultMessage
        }
      >
        <div className="channel-moderation">
          <div className="channel-moderation--body">
            <table id="channel_moderation_table" className="channel-moderation--table">
              <thead>
                <tr>
                  <th>
                    <FormattedMessage
