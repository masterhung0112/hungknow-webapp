...
const features = [
  ...
  localizeMessage(
    'admin.billing.subscription.planDetails.features.selfServiceDocumentation',
    'Self-Service documentation and forum support'
  ),
  localizeMessage(
    'admin.billing.subscription.planDetails.features.mfaAuthentication',
...
const seatsAndSubscriptionDates = (
  locale: string,
  userCount: number,
  ...
  return (
    <div className="PlanDetails__seatsAndSubscriptionDates">
      ...
      <div className="PlanDetails__subscriptionDate">
        <FormattedMessage id="admin.billing.subscription.planDetails.startDate" defaultMessage="Start Date: " />
        <FormattedDate value={startDate} day="numeric" month={getMonthLong(locale)} year="numeric" />
      </div>
      <div className="PlanDetails__subscriptionDate">
        <FormattedMessage id="admin.billing.subscription.planDetails.endDate" defaultMessage="End Date: " />
        <FormattedDate value={endDate} day="numeric" month={getMonthLong(locale)} year="numeric" />
      </div>
    </div>
  )
}
...
const PlanDetails: React.FC = () => {
  ...
  const userLimit = parseInt(
    useSelector((state: GlobalState) => getConfig(state).ExperimentalCloudUserLimit) || '0',
    10
  )
  ...
  switch (subscription.is_paid_tier) {
    case 'false':
      planPricing = (
        <div className="PlanDetails__plan">
          <div className="PlanDetails__planName">
            <FormattedMessage id="admin.billing.subscription.planDetails.tiers.free" defaultMessage="Free" />
          </div>
      ...
      planDetailsDescription = (
        <div className="PlanDetails__description">
          <div className="PlanDetails__planDetailsName">{`$${product.price_per_seat.toFixed(2)}`}</div>
    ...
    case 'true':
      planPricing = (
        <div className="PlanDetails__plan">
          <div className="PlanDetails__planName">{`$${product.price_per_seat.toFixed(2)}`}</div>
          <div className="PlanDetails__planCaveat">
            <FormattedMessage
  ...
  const featureList = features.map((feature, i) => (
    <div key={`PlanDetails__feature${i}`} className="PlanDetails__feature">
  ...
  return (
    <div className="PlanDetails">
      <div className="PlanDetails__top">
        <div className="PlanDetails__productName">{product.name}</div>
        {userCountDisplay}
      </div>
      {planPricing}
      {showSeatsAndSubscriptionDates &&
      ...
      <div className="PlanDetails__currentPlan">
        ...
        <FormattedMessage id="admin.billing.subscription.planDetails.currentPlan" defaultMessage="Current Plan" />
      </div>
    </div>
  )
}
