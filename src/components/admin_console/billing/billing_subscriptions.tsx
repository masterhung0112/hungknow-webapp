...
import { getCloudSubscription, getCloudProducts, getCloudCustomer } from 'hkclient-ts/lib/actions/cloud'
...
import { Preferences, CloudBanners, ModalIdentifiers, TELEMETRY_CATEGORIES } from 'utils/constants'
...
const BillingSubscriptions: React.FC<Props> = () => {
  ...
  const preferences = useSelector<GlobalState, PreferenceType[]>((state) =>
    getCategory(state, Preferences.ADMIN_CLOUD_UPGRADE_PANEL)
  )

  const contactSalesLink = useSelector((state: GlobalState) => getCloudContactUsLink(state, InquiryType.Sales))
  const cancelAccountLink = useSelector((state: GlobalState) =>
    getCloudContactUsLink(state, InquiryType.Sales, InquiryIssue.CancelAccount)
  )
  ...
  const shouldShowInfoBanner = (): boolean => {
    if (
      ...
      !preferences ||
      !subscription ||
      subscription.is_paid_tier === 'true' ||
      preferences.some((pref: PreferenceType) => pref.name === CloudBanners.HIDE && pref.value === 'true')
    ) {
      return false
    }

    if (
      userLimit - Number(analytics.TOTAL_USERS) <= WARNING_THRESHOLD &&
      userLimit - Number(analytics.TOTAL_USERS) > 0
    ) {
      return true
    }
  ...
  const handleHide = async () => {
    trackEvent(TELEMETRY_CATEGORIES.CLOUD_ADMIN, 'click_close_banner_user_limit_warning')
    dispatch(
  ...
  const upgradeMattermostCloud = () => (
    <div className="UpgradeMattermostCloud">
      ...
      <button type="button" onClick={onUpgradeMattermostCloud} className="UpgradeMattermostCloud__upgradeButton">
        <FormattedMessage
  ...
  return (
    <div className="wrapper--fixed BillingSubscriptions">
      <FormattedAdminHeader id="admin.billing.subscription.title" defaultMessage="Subscriptions" />
