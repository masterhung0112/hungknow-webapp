...
import { getCloudProducts, getCloudSubscription, getInvoices } from 'hkclient-ts/lib/actions/cloud'
import { Client4 } from 'hkclient-ts/lib/client'
import { Invoice } from 'hkclient-ts/lib/types/cloud'
...
const noBillingHistorySection = (
  <div className="BillingHistory__noHistory">
    <img className="BillingHistory__noHistory-graphic" src={noBillingHistoryGraphic} />
    ...
    >
      <FormattedMessage id="admin.billing.history.seeHowBillingWorks" defaultMessage="See how billing works" />
    </a>
  </div>
)
    ...
    case 'failed':
      return (
        <div className="BillingHistory__paymentStatus failed">
          ...
          <FormattedMessage id="admin.billing.history.paymentFailed" defaultMessage="Payment failed" />
        </div>
      )
    case 'paid':
      return (
        <div className="BillingHistory__paymentStatus paid">
          ...
          <FormattedMessage id="admin.billing.history.paid" defaultMessage="Paid" />
        </div>
      )
    default:
      return (
        <div className="BillingHistory__paymentStatus pending">
          <i className="icon icon-check-circle-outline" />
          <FormattedMessage id="admin.billing.history.pending" defaultMessage="Pending" />
        </div>
      )
  }
}
...
const BillingHistory: React.FC<Props> = () => {
  ...
  const paging = (
    <div className="BillingHistory__paging">
      ...
      />
      <button onClick={previousPage} disabled={firstRecord <= PAGE_LENGTH}>
        <i className="icon icon-chevron-left" />
      </button>
      <button onClick={nextPage} disabled={!invoices || firstRecord + PAGE_LENGTH >= Object.values(invoices).length}>
    ...
    <>
      <table className="BillingHistory__table">
        <tr className="BillingHistory__table-header">
          <th>
            <FormattedMessage id="admin.billing.history.date" defaultMessage="Date" />
          </th>
          <th>
            <FormattedMessage id="admin.billing.history.description" defaultMessage="Description" />
          </th>
          <th className="BillingHistory__table-headerTotal">
            <FormattedMessage id="admin.billing.history.total" defaultMessage="Total" />
          </th>
          <th>
            <FormattedMessage id="admin.billing.history.status" defaultMessage="Status" />
          </th>
          <th>{''}</th>
        </tr>
        {billingHistory.map((invoice: Invoice) => {
          ...
          const partialUsers = invoice.line_items
            .filter((item) => item.type === 'partial')
            .reduce((val, item) => val + item.quantity, 0)

          return (
            <tr className="BillingHistory__table-row" key={invoice.id}>
              <td>
                <FormattedDate
              ...
              <td className="BillingHistory__table-invoice">
                <a target="_new" rel="noopener noreferrer" href={Client4.getInvoicePdfUrl(invoice.id)}>
  ...
  return (
    <div className="wrapper--fixed BillingHistory">
      <FormattedAdminHeader id="admin.billing.history.title" defaultMessage="Billing History" />
        ...
        <div className="admin-console__content">
          <div className="BillingHistory__card">
            <div className="BillingHistory__cardHeader">
              <div className="BillingHistory__cardHeaderText">
                <div className="BillingHistory__cardHeaderText-top">
                  <FormattedMessage id="admin.billing.history.transactions" defaultMessage="Transactions" />
                </div>
            ...
            <div className="BillingHistory__cardBody">
              {invoices != null && <>{billingHistory ? billingHistoryTable : noBillingHistorySection}</>}
              {invoices == null && (
