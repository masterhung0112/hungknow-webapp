...
export default class SystemUsers extends React.PureComponent<Props, State> {
  ...
  loadDataForTeam = async (teamId: string, filter: string | undefined) => {
    const { getProfiles, loadProfilesWithoutTeam, loadProfilesAndTeamMembers, getTeamStats } = this.props.actions
    ...
    if (teamId === SearchUserTeamFilter.ALL_USERS) {
      await Promise.all([getProfiles(0, Constants.PROFILE_CHUNK_SIZE, options), getStandardAnalytics()])
  ...
  nextPage = async (page: number) => {
    ...
    const { getProfiles, loadProfilesWithoutTeam, loadProfilesAndTeamMembers } = this.props.actions
  ...
  doSearch = debounce(
    async (term, teamId = this.props.teamId, filter = this.props.filter) => {
      if (!term) {
        return
      }
  ...
  renderRevokeAllUsersModal = () => {
    ...
    const revokeAllButton = (
      <FormattedMessage id="admin.system_users.revoke_all_sessions_button" defaultMessage="Revoke All Sessions" />
    )
      ...
      <div className="system-users__filter-row">
        ...
        <label>
          <span className="system-users__team-filter-label">
            <FormattedMessage id="filtered_user_list.team" defaultMessage="Team:" />
          </span>
          <select
        ...
        <label>
          <span className="system-users__filter-label">
            <FormattedMessage id="filtered_user_list.userStatus" defaultMessage="User Status:" />
          </span>
          <select
            id="selectUserStatus"
          ...
          >
            ...
            <option value={UserFilters.SYSTEM_ADMIN}>
              {Utils.localizeMessage('admin.system_users.system_admin', 'System Admin')}
            </option>
            <option value={UserFilters.SYSTEM_GUEST}>
              {Utils.localizeMessage('admin.system_users.guest', 'Guest')}
            </option>
            <option value={UserFilters.INACTIVE}>
              {Utils.localizeMessage('admin.system_users.inactive', 'Inactive')}
            </option>
          </select>
        </label>
      </div>
          ...
          <div className="admin-console__content">
            ...
            <SystemPermissionGate permissions={[Permissions.REVOKE_USER_ACCESS_TOKEN]}>
              ...
              <div className="pt-3 pb-3">
                ...
                >
                  <FormattedMessage id="admin.system_users.revokeAllSessions" defaultMessage="Revoke All Sessions" />
                </button>
              </div>
