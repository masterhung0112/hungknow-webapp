...
export default class ClusterTable extends React.PureComponent {
  ...
  render() {
    var versionMismatch = <img alt="version mismatch" className="cluster-status" src={statusGreen} />

    var configMismatch = <img alt="config mismatch" className="cluster-status" src={statusGreen} />
    ...
    this.props.clusterInfos.map((clusterInfo) => {
      if (clusterInfo.version !== version) {
        versionMismatch = <img alt="version mismatch" className="cluster-status" src={statusYellow} />
      }

      if (clusterInfo.config_hash !== configHash) {
        configMismatch = <img alt="config mismatch" className="cluster-status" src={statusYellow} />
      }
    ...
    var items = this.props.clusterInfos.map((clusterInfo) => {
      ...
      if (singleItem) {
        status = <img alt="Cluster status" className="cluster-status" src={statusYellow} />
      } else {
        status = <img alt="Cluster status" className="cluster-status" src={statusGreen} />
      }
      ...
      <div className="cluster-panel__table" style={style.clusterTable}>
        <div className="text-right">
          <button type="submit" className="btn btn-link" onClick={this.props.reload}>
            <ReloadIcon />
            <FormattedMessage id="admin.cluster.status_table.reload" defaultMessage=" Reload Cluster Status" />
          </button>
        </div>
        <table className="table">
          <thead>
            <tr>
              ...
              <th>
                <FormattedMessage id="admin.cluster.status_table.config_hash" defaultMessage="Config File MD5" />
              </th>
              <th>
