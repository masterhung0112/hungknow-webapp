...
import { AdminConfig, EnvironmentConfig, ClientLicense } from 'hkclient-ts/lib/types/config'
...
import { CloudState } from 'hkclient-ts/lib/types/cloud'
...
export default class AdminConsole extends React.PureComponent<Props, State> {
  ...
  public render(): JSX.Element | null {
    const { license, config, environmentConfig, showNavigationPrompt, roles } = this.props
    ...
    const discardChangesModal: JSX.Element = (
      <DiscardChangesModal show={showNavigationPrompt} onConfirm={confirmNavigation} onCancel={cancelNavigation} />
    )
    ...
    return (
      <div className="admin-console__wrapper" id="adminConsoleWrapper">
        ...
        <div className="admin-console">
          <Highlight filter={this.state.filter}>{this.renderRoutes(extraProps)}</Highlight>
        </div>
        {discardChangesModal}
        <ModalController />
      </div>
