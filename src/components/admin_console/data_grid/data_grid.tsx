...
class DataGrid extends React.PureComponent<Props, State> {
  ...
  private renderRows(): JSX.Element {
    ...
    if (this.props.loading) {
      if (this.props.loadingIndicator) {
        rowsToRender = <div className="DataGrid_loading">{this.props.loadingIndicator}</div>
      } else {
        rowsToRender = (
      ...
      const placeholder = this.props.placeholderEmpty || (
        <FormattedMessage id="admin.data_grid.empty" defaultMessage="No items found" />
      )
      rowsToRender = <div className="DataGrid_empty">{placeholder}</div>
    } else {
      rowsToRender = rows.map((row, index) => {
        return <DataGridRow key={index} row={row} columns={visibleColumns} />
      })
    }
  ...
  public render() {
    return (
      <div className="DataGrid" ref={this.ref}>
        {this.renderSearch()}
        {this.renderHeader()}
        {this.renderRows()}
        {this.renderFooter()}
