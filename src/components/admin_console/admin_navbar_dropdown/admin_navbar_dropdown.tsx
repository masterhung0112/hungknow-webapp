    ...
    if (teams && teams.length > 0) {
      ...
      for (const team of teamsArray) {
        teamToRender.push(
          <MenuItemBlockableLink
            ...
            to={'/' + team.name}
            text={
              formatMessage({ id: 'navbar_dropdown.switchTo', defaultMessage: 'Switch to ' }) + ' ' + team.display_name
            }
          />
        )
      }
      ...
      switchTeams = (
        <MenuItemBlockableLink
          to={'/select_team'}
          icon={
            <FormattedMessage id="select_team.icon" defaultMessage="Select Team Icon">
              {(title) => {
                return <i className="fa fa-exchange" title={title as string} />
              }}
            </FormattedMessage>
          }
