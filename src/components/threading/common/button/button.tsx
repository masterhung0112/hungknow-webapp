...
function Button({ prepend, append, children, isActive, hasDot, ...attrs }: Props & Attrs) {
  return (
    <button
      {...attrs}
      className={classNames('Button Button___transparent', { 'is-active': isActive }, attrs.className)}
    >
      {prepend && <span className="Button_prepended">{prepend}</span>}
      <span className="Button_label">
        {children}
        {hasDot && <span className="dot" />}
      </span>
      {append && <span className="Button_appended">{append}</span>}
    </button>
  )
}
