...
describe('components/threading/global_threads/thread_item', () => {
  ...
  beforeEach(() => {
    props = {
      ...
      previewText:
        'Lobortis phasellus feugiat vivamus facilisis ac suspendisse elit orci augue, metus hac libero cum diam accumsan magnis.',
      participants: users,
      totalReplies: 9,
      newReplies: 0,
      newMentions: 0,
  ...
  test('should report unread messages', () => {
    const wrapper = shallow(<ThreadItem {...props} newReplies={2} />)
    expect(wrapper).toMatchSnapshot()
  ...
  test('should report unread mentions', () => {
    const wrapper = shallow(<ThreadItem {...props} newReplies={2} newMentions={1} />)
    expect(wrapper).toMatchSnapshot()
