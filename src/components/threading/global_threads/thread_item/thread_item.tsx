  ...
  return (
    ...
    >
      <h1>
        ...
        <Timestamp className="alt-hidden" value={lastReplyAt} useTime={false} units={THREADING_TIME} />
      </h1>
      <span className="menu-anchor alt-visible">
        <ThreadMenu isSaved={isSaved} isFollowing={isFollowing} hasUnreads={Boolean(newReplies)} actions={actions} />
      </span>
      <p>{previewText}</p>
      {Boolean(totalReplies) && (
        <div className="activity">
          <Avatars users={participants} totalUsers={totalParticipants} size="xs" />
          {newReplies ? (
            <FormattedMessage
