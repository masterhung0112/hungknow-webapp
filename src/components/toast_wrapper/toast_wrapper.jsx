...
const TOAST_REL_RANGES = [RelativeRanges.TODAY_YESTERDAY]
...
class ToastWrapper extends React.PureComponent {
  ...
  static getDerivedStateFromProps(props, prevState) {
    ...
    if (
      props.channelMarkedAsUnread &&
      !props.atBottom &&
      !prevState.channelMarkedAsUnread &&
      !prevState.showUnreadToast
    ) {
  ...
  componentDidUpdate(prevProps, prevState) {
    const { showUnreadToast, showNewMessagesToast, showMessageHistoryToast } = this.state
    const { atBottom, atLatestPost, postListIds, lastViewedBottom, updateNewMessagesAtInChannel, actions } = this.props
    ...
    const toastStateChanged =
      prevState.showUnreadToast !== showUnreadToast ||
      ...
      return (
        <FormattedMessage
          ...
          values={{
            ...
            date: <Timestamp value={since} useTime={false} ranges={TOAST_REL_RANGES} />,
          }}
        />
      )
    }
  ...
  archiveToastText = () => {
    return <FormattedMessage id="postlist.toast.history" defaultMessage="Viewing message history" />
  }
  ...
  render() {
    ...
    return (
      <React.Fragment>
        <Toast {...unreadToastProps}>{this.newMessagesToastText(unreadCount, lastViewedAt)}</Toast>
        <Toast {...archiveToastProps}>{this.archiveToastText()}</Toast>
      </React.Fragment>
    )
  }
}
