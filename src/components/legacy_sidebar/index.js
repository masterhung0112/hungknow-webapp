...
import { Preferences } from 'hkclient-ts/lib/constants/index'
import {
  getCurrentChannel,
...
import { getBool as getBoolPreference, getSidebarPreferences } from 'hkclient-ts/lib/selectors/entities/preferences'
...
function mapStateToProps(state) {
  ...
  const currentTeammate =
    currentChannel && currentChannel.teammate_id && getCurrentChannel(state, currentChannel.teammate_id)
  ...
  if (currentTeam) {
    canCreatePublicChannel = haveITeamPermission(state, {
      team: currentTeam.id,
      permission: Permissions.CREATE_PUBLIC_CHANNEL,
    })
    canCreatePrivateChannel = haveITeamPermission(state, {
      team: currentTeam.id,
