...
class LegacySidebar extends React.PureComponent {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    if (
      nextProps.orderedChannelIds[0].type === SidebarChannelGroups.UNREADS &&
  ...
  updateUnreadIndicators = () => {
    ...
    if (this.firstUnreadChannel) {
      ...
      if (
        firstUnreadPosition &&
        firstUnreadPosition.top + firstUnreadElement.height() - scrollMargin < this.refs.scrollbar.getScrollTop()
      ) {
        showTopUnread = true
      }
    }
    ...
    if (this.lastUnreadChannel) {
      ...
      if (
        lastUnreadPosition &&
        lastUnreadPosition.top + scrollMargin >
          this.refs.scrollbar.getScrollTop() + this.refs.scrollbar.getClientHeight()
      ) {
  ...
  navigateChannelShortcut = (e) => {
    if (
      ...
      !e.ctrlKey &&
      !e.metaKey &&
      (Utils.isKeyPressed(e, Constants.KeyCodes.UP) || Utils.isKeyPressed(e, Constants.KeyCodes.DOWN))
    ) {
      e.preventDefault()
  ...
  renderOrderedChannels = () => {
    ...
    return (
      ...
      >
        <div id="sidebarChannelContainer" className="nav-pills__container">
            ...
            return (
              ...
              >
                <li className="sidebar-section__header">
                  <h4 role="presentation" id={sectionId}>
                    <ChannelName
  ...
  render() {
    const { channelSwitcherOption, currentTeam, currentUser, isOpen } = this.props

    const { showDirectChannelsModal, showTopUnread, showBottomUnread } = this.state
    ...
    if (showDirectChannelsModal) {
      moreDirectChannelsModal = (
        <MoreDirectChannels onModalDismissed={this.hideMoreDirectChannelsModal} isExistingChannel={false} />
      )
    }
