  ...
  render = () => {
    ...
    const customStatus =
      this.props.channelType === Constants.DM_CHANNEL ? (
        <CustomStatusEmoji userID={this.props.teammateId} showTooltip={true} />
      ) : null
    ...
    const content = (
      <React.Fragment>
        ...
        <span className="sidebar-item__name">
          <span ref={this.displayNameRef}>{this.props.displayName}</span>
        </span>
        {customStatus}
        {badge}
    ...
    if (this.props.unreadMentions === 1) {
      ...
      ariaLabel += ` ${this.props.unreadMentions} ${localizeMessage(
    ...
    if (isDesktopApp()) {
      element = (
        <div>
          <CopyUrlContextMenu link={this.props.link} menuId={this.props.channelId}>
            <button className={'btn btn-link ' + this.props.rowClass} aria-label={ariaLabel} onClick={this.handleClick}>
              {content}
            </button>
          </CopyUrlContextMenu>
        </div>
    ...
    if (this.state.showTooltip) {
      const displayNameToolTip = (
        <Tooltip id="channel-displayname__tooltip" style={style.channelTooltip}>
      ...
      element = (
        ...
        >
          <div ref={this.gmItemRef}>{element}</div>
        </OverlayTrigger>
      )
    }
    return element
