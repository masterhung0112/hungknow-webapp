    ...
    if (this.props.handleClose && !this.props.badge) {
      let removeTooltip = (
        <Tooltip id="remove-dm-tooltip">
          <FormattedMessage id="sidebar.removeList" defaultMessage="Remove from this list" />
        </Tooltip>
      )
      ...
      if (this.props.channelType === Constants.OPEN_CHANNEL || this.props.channelType === Constants.PRIVATE_CHANNEL) {
        removeTooltip = (
          <Tooltip id="remove-dm-tooltip">
            <FormattedMessage id="sidebar.leave" defaultMessage="Leave the channel" />
          </Tooltip>
        )
      }

      closeButton = (
        <OverlayTrigger delayShow={1000} placement="top" overlay={removeTooltip}>
