...
import { ChannelType, Channel } from 'hkclient-ts/lib/types/channels'
...
export default class NewChannelFlow extends React.PureComponent<Props, State> {
  ...
  onCreateChannelError = (err: ServerError) => {
    if (err.server_error_id === 'model.channel.is_valid.2_or_more.app_error') {
      ...
      this.setState({
        serverError: Utils.localizeMessage('channel_flow.alreadyExist', 'A channel with that URL already exists'),
      })
    } else {
  ...
  render() {
    ...
    switch (this.state.flowState) {
      ...
      case SHOW_EDIT_URL:
        ...
        changeURLTitle = <FormattedMessage id="channel_flow.changeUrlTitle" defaultMessage="Change Channel URL" />
        changeURLSubmitButtonText = changeURLTitle
        break
      case SHOW_EDIT_URL_THEN_COMPLETE:
        showChangeURLModal = true
        changeURLTitle = <FormattedMessage id="channel_flow.set_url_title" defaultMessage="Set Channel URL" />
        changeURLSubmitButtonText = <FormattedMessage id="channel_flow.create" defaultMessage="Create Channel" />
        break
    }
