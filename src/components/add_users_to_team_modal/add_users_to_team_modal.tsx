...
export default class AddUsersToTeamModal extends React.PureComponent<Props, State> {
  ...
  private renderOption = (
    ...
    return (
      ...
      >
        <ProfilePicture src={Client4.getProfilePictureUrl(option.id, option.last_picture_update)} size="md" />
        <div className="more-modal__details">
          <div className="more-modal__name">
            {displayEntireNameForUser(option)}
            <BotBadge show={Boolean(option.is_bot)} className="badge-popoverlist" />
            <GuestBadge show={isGuest(option)} className="popoverlist" />
          </div>
        </div>
  ...
  private handlePageChange = (page: number, prevPage: number) => {
    if (page > prevPage) {
      ...
      this.props.actions
        .getProfilesNotInTeam(this.props.team.id, false, page, USERS_PER_PAGE * 2)
        ...
        <Modal.Header closeButton={true}>
          <Modal.Title componentClass="h1">
            <FormattedMessage
              ...
              values={{
                teamName: <strong>{this.props.team.name}</strong>,
              }}
            />
          </Modal.Title>
        </Modal.Header>
