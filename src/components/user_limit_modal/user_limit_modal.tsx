...
export default function UserLimitModal(props: Props): JSX.Element {
  ...
  useEffect(() => {
    pageVisited(TELEMETRY_CATEGORIES.CLOUD_PURCHASING, 'pageview_modal_user_limit_reached')
  }, [])
  ...
  const onSubmit = () => {
    trackEvent(TELEMETRY_CATEGORIES.CLOUD_PURCHASING, 'click_modal_user_limit_upgrade')
    props.actions.closeModal(ModalIdentifiers.UPGRADE_CLOUD_ACCOUNT)
    props.actions.openModal({
  ...
  const close = () => {
    trackEvent(TELEMETRY_CATEGORIES.CLOUD_PURCHASING, 'click_modal_user_limit_not_now')
    props.actions.closeModal(ModalIdentifiers.UPGRADE_CLOUD_ACCOUNT)
  }

  const confirmBtn = props.userIsAdmin ? (
    <Button className="confirm-button" onClick={onSubmit}>
      <FormattedMessage id={'upgrade.cloud'} defaultMessage={'Upgrade Mattermost Cloud'} />
    </Button>
  ) : (
    <NotifyLink
  ...
  return (
    <>
      {props.show && (
        <Modal className={'UserLimitModal'} show={props.show} id="userLimitModal" role="dialog" onHide={close}>
          <Modal.Header closeButton={true} />
          <Modal.Body>
            <UpgradeUserLimitModalSvg />
            <div className="title">
              <FormattedMessage id={'upgrade.cloud_modal_title'} defaultMessage={'You\'ve reached the user limit"'} />
            </div>
            <div className="description">
              <FormattedMessage
                id={'upgrade.cloud_modal_body'}
                defaultMessage={'The free tier is limited to {num} users. Upgrade Mattermost Cloud for more users.'}
                values={{
            ...
            <div className="buttons">
              <Button className="dismiss-link" onClick={close}>
                <FormattedMessage id={'cloud.upgrade.notrightnow'} defaultMessage={'Not right now'} />
              </Button>
              {confirmBtn}
            </div>
          </Modal.Body>
