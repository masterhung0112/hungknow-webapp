...
import { ChannelSearchOpts, ChannelWithTeamData } from 'hkclient-ts/lib/types/channels'
...
export default class ChannelSelectorModal extends React.PureComponent<Props, State> {
    ...
    if (prevProps.searchTerm !== this.props.searchTerm) {
      ...
      if (searchTerm === '') {
        ...
        this.searchTimeoutId = window.setTimeout(async () => {
          ...
          const response = await this.props.actions.searchAllChannels(searchTerm, {
    ...
    return (
      ...
      >
        <div className="more-modal__details">
          {option.type === Constants.PRIVATE_CHANNEL && <i className="icon icon-lock-outline" />}
          {option.type === Constants.OPEN_CHANNEL && <i className="icon icon-globe" />}
  ...
  render() {
    const numRemainingText = (
      <FormattedMessage id="multiselect.selectChannels" defaultMessage="Use ↑↓ to browse, ↵ to select." />
    )

    const buttonSubmitText = localizeMessage('multiselect.add', 'Add')

    const options = this.state.channels.map(
      (i): ChannelWithTeamDataValue => ({ ...i, label: i.display_name, value: i.id })
    )
    ...
    return (
      ...
      >
        <Modal.Header closeButton={true}>
          <Modal.Title componentClass="h1" id="channelSelectorModalLabel">
