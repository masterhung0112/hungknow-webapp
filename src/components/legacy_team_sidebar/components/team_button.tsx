...
class TeamButton extends React.PureComponent<Props> {
  ...
  render() {
    ...
    const isNotCreateTeamButton: boolean =
      !this.props.url.endsWith('create_team') && !this.props.url.endsWith('select_team')
    ...
    if (!teamClass) {
      ...
      if (mentions) {
        ...
        badge = <span className={'badge badge-max-number pull-right small'}>{mentions > 99 ? '99+' : mentions}</span>
      }
    }
    ...
    if (typeof this.props.order !== 'undefined' && this.props.order < 10) {
      ...
      if (this.props.showOrder) {
        orderIndicator = <div className="order-indicator">{this.props.order}</div>
      }
    }
    ...
    const btn = (
      <OverlayTrigger
        ...
        overlay={<Tooltip id={`tooltip-${this.props.url}`}>{toolTip}</Tooltip>}
      >
    ...
    if (isDesktopApp()) {
      ...
      if (isNotCreateTeamButton) {
        teamButton = (
          <CopyUrlContextMenu link={this.props.url} menuId={this.props.url}>
            {teamButton}
          </CopyUrlContextMenu>
        )
      }
      ...
      <Draggable draggableId={teamId!} index={teamIndex!}>
        {(provided, snapshot) => {
          return (
            ...
            >
              <div className={classNames([`team-container ${teamClass}`, { isDragging: snapshot.isDragging }])}>
                {teamButton}
                {orderIndicator}
              </div>
            </div>
