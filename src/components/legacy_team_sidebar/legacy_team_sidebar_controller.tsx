...
import { GenericAction, GetStateFunc } from 'hkclient-ts/lib/types/actions'
...
type Actions = {
  ...
  updateTeamsOrderForUser: (
    teamIds: string[]
  ) => (dispatch: Dispatch<GenericAction>, getState: GetStateFunc) => Promise<void>
}
...
export default class LegacyTeamSidebar extends React.PureComponent<Props, State> {
  ...
  onDragEnd = (result: DropResult) => {
    ...
    const pushElement = (list: Team[], idx: number, itemId: string): Team[] => {
      return [...list.slice(0, idx), teams.find((team) => team.id === itemId), ...list.slice(idx, list.length)]
    }

    const newTeamsOrder = pushElement(popElement(teams, sourceIndex), destinationIndex, result.draggableId)
  ...
  render() {
    ...
    if (this.props.moreTeamsToJoin && !this.props.experimentalPrimaryTeam) {
      ...
      joinableTeams.push(
        <SystemPermissionGate permissions={[Permissions.CREATE_TEAM]} key="more_teams">
          <TeamButton
            btnClass="team-btn__add"
            url="/create_team"
            tip={<FormattedMessage id="navbar_dropdown.create" defaultMessage="Create a Team" />}
            content={<i className="icon icon-plus" />}
            switchTeam={this.props.actions.switchTeam}
          />
    ...
    plugins.push(
      <div key="team-sidebar-bottom-plugin" className="team-sidebar-bottom-plugin is-empty">
        ...
        <div className="team-wrapper" id="teamSidebarWrapper">
          ...
          >
            <DragDropContext onDragEnd={this.onDragEnd}>
              <Droppable droppableId="my_teams" type="TEAM_BUTTON">
