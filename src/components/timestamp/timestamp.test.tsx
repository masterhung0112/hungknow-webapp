...
describe('components/timestamp/Timestamp', () => {
  ...
  test('should be wrapped in SemanticTime and support passthrough className and label', () => {
    const wrapper = shallowWithIntl(<Timestamp useTime={false} className="test class" label="test label" />)
  ...
  test('should not be wrapped in SemanticTime', () => {
    const wrapper = shallowWithIntl(<Timestamp useTime={false} useSemanticOutput={false} />)
    expect(wrapper).toMatchSnapshot()
  ...
  test('should render title-case Today', () => {
    const wrapper = mountWithIntl(<Timestamp useTime={false} ranges={[RelativeRanges.TODAY_TITLE_CASE]} />)
    expect(wrapper.text()).toEqual('Today')
  })

  test('should render normal today', () => {
    const wrapper = mountWithIntl(
      <Timestamp value={daysFromNow(0)} useTime={false} ranges={[RelativeRanges.TODAY_YESTERDAY]} />
    )
    expect(wrapper.text()).toEqual('today')
  })
  ...
  test('should render title-case Yesterday', () => {
    const wrapper = mountWithIntl(
      <Timestamp value={daysFromNow(-1)} useTime={false} ranges={[RelativeRanges.YESTERDAY_TITLE_CASE]} />
    )
  ...
  test('should render normal yesterday', () => {
    const wrapper = mountWithIntl(
      <Timestamp value={daysFromNow(-1)} useTime={false} ranges={[RelativeRanges.TODAY_YESTERDAY]} />
    )
    expect(wrapper.text()).toEqual('yesterday')
  })

  test('should render normal tomorrow', () => {
    const wrapper = mountWithIntl(<Timestamp value={daysFromNow(1)} useTime={false} unit="day" />)
    expect(wrapper.text()).toEqual('tomorrow')
  })

  test('should render 3 days ago', () => {
    const wrapper = mountWithIntl(<Timestamp value={daysFromNow(-3)} useTime={false} unit="day" />)
    expect(wrapper.text()).toEqual('3 days ago')
  })

  test('should render 3 days ago as weekday', () => {
    const date = daysFromNow(-3)
    const wrapper = mountWithIntl(<Timestamp value={date} useTime={false} />)
  ...
  test('should render 2 days ago as weekday in supported timezone', () => {
    ...
    const wrapper = mountWithIntl(<Timestamp value={date} timeZone="Asia/Manila" useTime={false} />)
  ...
  test('should render time without timezone', () => {
    const wrapper = mountWithIntl(
      <Timestamp value={new Date('Fri Jan 12 2018 20:15:13 GMT+0800 (+08)').getTime()} useDate={false} />
    )
  ...
  test('should render date without timezone', () => {
    const wrapper = mountWithIntl(
      <Timestamp value={new Date('Fri Jan 12 2018 23:15:13 GMT+0800 (+08)').getTime()} useTime={false} />
    )
