...
storiesOf('Connected/Timestamp', module)
    ...
    return (
      <StoryGrid>
        {propVariations.map(([label, props], boxIndex) => (
          <StoryBox key={`kind-${boxIndex}`} label={label}>
            {[...unitDiffs].map(([unit, diffs], unitIndex) => (
              <StoryBox key={unit} label={unit}>
                <Timestamp
                  key={`start-${0}`}
                  {...props}
                ...
                {diffs.map((diff) => (
                  <Timestamp key={`start-${diff}`} units={units} {...props} value={moment().add(diff, unit).toDate()}>
                    {({ formatted }, { relative }) => (
                      <span key={`span-${diff}`}>
                        <pre
                          style={{
  ...
  .add('relative, fixed unit', () => (
    <StoryGrid>
      {units.map((unit) => (
        <StoryBox key={`fixed-box-${unit}`} label={unit}>
