...
import { getMyTeams, getJoinableTeamIds, getCurrentTeam } from 'hkclient-ts/lib/selectors/entities/teams'
import { getCurrentUser } from 'hkclient-ts/lib/selectors/entities/users'
import {
  haveITeamPermission,
  haveICurrentTeamPermission,
  haveISystemPermission,
} from 'hkclient-ts/lib/selectors/entities/roles'
...
import { showOnboarding, showNextStepsTips, showNextSteps } from 'components/next_steps_view/steps'
...
function mapStateToProps(state) {
  ...
  if (!canCreateOrDeleteCustomEmoji) {
    for (const team of getMyTeams(state)) {
      if (
        haveITeamPermission(state, { team: team.id, permission: Permissions.CREATE_EMOJIS }) ||
        haveITeamPermission(state, { team: team.id, permission: Permissions.DELETE_EMOJIS })
      ) {
  ...
  const canManageTeamIntegrations =
    haveICurrentTeamPermission(state, { permission: Permissions.MANAGE_SLASH_COMMANDS }) ||
    haveICurrentTeamPermission(state, { permission: Permissions.MANAGE_OAUTH }) ||
    haveICurrentTeamPermission(state, { permission: Permissions.MANAGE_INCOMING_WEBHOOKS }) ||
    haveICurrentTeamPermission(state, { permission: Permissions.MANAGE_OUTGOING_WEBHOOKS })
