...
export default class MoreDirectChannels extends React.PureComponent<Props, State> {
  ...
  public loadProfilesMissingStatus = (
    users: UserProfile[] = [],
    statuses: RelationOneToOne<UserProfile, string> = {}
  ) => {
  ...
  optionDisplayParts = (option: OptionValue): ReactNode => {
    if (isGroupChannel(option)) {
      return (
        <>
          <div className="more-modal__gm-icon bg-text-200">{option.profiles.length}</div>
          <div className="more-modal__details">
            <div className="more-modal__name">
              <span>{option.profiles.map((profile) => `@${profile.username}`).join(', ')}</span>
            </div>
          </div>
        </>
      )
    ...
    const { id, delete_at: deleteAt, is_bot: isBot = false, last_picture_update: lastPictureUpdate } = option
    ...
    return (
      <>
        ...
        <div className="more-modal__details">
          <div className="more-modal__name">
            {modalName}
            <BotBadge show={isBot} className="badge-popoverlist" />
            <GuestBadge show={isGuest(option)} className="badge-popoverlist" />
            <CustomStatusEmoji userID={option.id} showTooltip={true} emojiSize={15} />
          </div>
          {!isBot && <div className="more-modal__description">{option.email}</div>}
        </div>
      </>
    )
  }
  ...
  computeOptions = (): Option[] => {
    const { currentUserId, searchTerm, myDirectChannels, recentDMUsers = [] } = this.props

    const { values } = this.state

    const [activeUsers, inactiveUsers] = partition(this.props.users, ({ delete_at: deleteAt }) => deleteAt === 0)

    const groupChannelsWithAvailableProfiles = this.props.groupChannels.filter(
      ({ profiles }) => differenceBy(profiles, values, 'id').length
    )
    ...
    return [...recent, ...users, ...groupChannels]
  }
    ...
    return (
      ...
      >
        ...
        <Modal.Footer className="modal-footer--invisible">
          <button id="closeModalButton" type="button" className="btn btn-link">
            <FormattedMessage id="general_button.close" defaultMessage="Close" />
          </button>
        </Modal.Footer>
      </Modal>
    )
