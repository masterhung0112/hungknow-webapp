  ...
  if (shouldShowHintOptions) {
    try {
      newVisibleSearchHintOptions = searchHintOptions.filter((option) => {
        return (
          new RegExp(pretext, 'ig').test(option.searchTerm) && option.searchTerm.toLowerCase() !== pretext.toLowerCase()
        )
      })
    } catch {
...
const Search: React.FC<Props> = (props: Props): JSX.Element => {
  ...
  const renderMentionButton = (): JSX.Element => (
    <HeaderIconWrapper
      iconComponent={<MentionsIcon className="icon icon--standard" aria-hidden="true" />}
      ariaLabel={true}
      buttonClass={classNames('channel-header__icon', { 'channel-header__icon--active': props.isMentionSearch })}
  ...
  const renderFlagBtn = (): JSX.Element => (
    <HeaderIconWrapper
      ...
      ariaLabel={true}
      buttonClass={classNames('channel-header__icon ', { 'channel-header__icon--active': props.isFlaggedPosts })}
  ...
  if (!props.isSideBarRight) {
    return <div className="sidebar-right__table">{renderSearchBar()}</div>
  }
