...
import { PaymentMethod } from 'hkclient-ts/lib/types/cloud'
...
export default class PaymentForm extends React.PureComponent<Props, State> {
  ...
  private handleStateChange = (stateValue: string) => {
    ...
    if (this.props.onInputChange) {
      this.props.onInputChange({
        ...this.state,
        ...newStateValue,
        card: this.cardRef.current?.getCard(),
      } as BillingDetails)
    }
  }
  ...
  public render() {
    ...
    if (changePaymentMethod) {
      paymentDetails = (
        <React.Fragment>
          <div className="form-row">
            <CardInput forwardedRef={this.cardRef} required={true} onBlur={this.onBlur} />
          </div>
          <div className="form-row">
            <Input
              name="name"
          ...
          <div className="section-title">
            <FormattedMessage id="payment_form.billing_address" defaultMessage="Billing address" />
          </div>
          <DropdownInput
      ...
      if (paymentMethod) {
        let cardDetails = <FormattedMessage id="payment_form.no_credit_card" defaultMessage="No credit card added" />
        ...
        let addressDetails = (
          <i>
            <FormattedMessage id="payment_form.no_billing_address" defaultMessage="No billing address added" />
          </i>
        )
        if (this.state.state) {
          addressDetails = (
        ...
        cardContent = (
          <React.Fragment>
            <div className="PaymentForm-saved-card">{cardDetails}</div>
            <div className="PaymentForm-saved-address">{addressDetails}</div>
          </React.Fragment>
        )
      }

      paymentDetails = (
        <div id="console_payment_saved" className="PaymentForm-saved">
          <div className="PaymentForm-saved-title">
            <FormattedMessage id="payment_form.saved_payment_method" defaultMessage="Saved Payment Method" />
          </div>
          {cardContent}
          <button className="Form-btn-link PaymentForm-change" onClick={this.changePaymentMethod}>
            <FormattedMessage id="payment_form.change_payment_method" defaultMessage="Change Payment Method" />
          </button>
        </div>
      )
    }
    ...
    return (
      <form id="payment_form" className={`PaymentForm ${className}`}>
        <div className="section-title">
          <FormattedMessage id="payment_form.credit_card" defaultMessage="Credit Card" />
        </div>
        {paymentDetails}
      </form>
    )
