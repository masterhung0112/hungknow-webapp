...
class CardInput extends React.PureComponent<Props, State> {
  ...
  private renderError(error: string) {
    ...
    if (error === REQUIRED_FIELD_TEXT) {
      errorMessage = <FormattedMessage id="payment.field_required" defaultMessage="This field is required" />
    } else if (error === VALID_CARD_TEXT) {
      errorMessage = (
        <FormattedMessage id="payment.invalid_card_number" defaultMessage="Please enter a valid credit card" />
      )
    }
      ...
      <div className="Input_container">
        <fieldset className={error ? fieldsetErrorClass : fieldsetClass}>
          <legend className={showLegend ? 'Input_legend Input_legend___focus' : 'Input_legend'}>
            <FormattedMessage id="payment.card_number" defaultMessage="Card Number" />
          </legend>
          <CardElement
            {...otherProps}
...
const InjectedCardInput = (props: OwnProps) => {
  return (
    <ElementsConsumer>
      {({ elements }) => <CardInput ref={props.forwardedRef} elements={elements} {...props} />}
    </ElementsConsumer>
  )
}
