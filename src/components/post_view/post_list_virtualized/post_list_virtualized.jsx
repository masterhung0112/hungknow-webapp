...
import { isDateLine, isStartOfNewMessages } from 'hkclient-ts/lib/utils/post_list'
...
class PostList extends React.PureComponent {
  ...
  getSnapshotBeforeUpdate(prevProps) {
    if (this.postListRef && this.postListRef.current) {
      const postsAddedAtTop =
        ...
        this.props.postListIds.length !== prevProps.postListIds.length &&
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    ...
    if (snapshot) {
      ...
      const postsAddedAtTop =
        presentPostsCount !== prevPostsCount && this.props.postListIds[0] === prevProps.postListIds[0]
      const channelHeaderAdded = this.props.atOldestPost !== prevProps.atOldestPost
  ...
  getNewMessagesSeparatorIndex = (postListIds) => {
    return postListIds.findIndex((item) => item.indexOf(PostListRowListIds.START_OF_NEW_MESSAGES) === 0)
  }
  ...
  render() {
    ...
    let ariaLabel
    if (
      this.props.latestAriaLabelFunc &&
      this.props.postListIds.indexOf(PostListRowListIds.START_OF_NEW_MESSAGES) >= 0
    ) {
    ...
    return (
      ...
      >
        {this.state.isMobile && (
          <React.Fragment>
            <FloatingTimestamp isScrolling={this.state.isScrolling} isMobile={true} postId={this.state.topPostId} />
        ...
        <div role="presentation" className="post-list-holder-by-time" key={'postlist-' + channelId}>
          <div role="presentation" className="post-list__table">
