...
export default class PostMessageView extends React.PureComponent<Props, State> {
  ...
  renderEditedIndicator() {
    ...
    return (
      <span id={`postEdited_${this.props.post.id}`} className="post-edited__indicator">
  ...
  render() {
    const { post, enableFormatting, options, pluginPostTypes, compactDisplay, isRHS, theme } = this.props
    ...
    if (pluginPostTypes && pluginPostTypes.hasOwnProperty(postType)) {
      ...
      return <PluginComponent post={post} compactDisplay={compactDisplay} isRHS={isRHS} theme={theme} />
    }
    ...
    return (
      <ShowMore checkOverflow={this.state.checkOverflow} text={message}>
        <div
          aria-readonly="true"
          tabIndex={0}
          id={id}
        ...
        <Pluggable pluggableName="PostMessageAttachment" postId={post.id} onHeightChange={this.handleHeightReceived} />
      </ShowMore>
    )
  }
}
