...
import { PostAction, PostActionOption } from 'hkclient-ts/lib/types/integration_actions'
import {
  MessageAttachment as MessageAttachmentType,
  MessageAttachmentField,
} from 'hkclient-ts/lib/types/message_attachments'
...
export default class MessageAttachment extends React.PureComponent<Props, State> {
  ...
  renderPostActions = () => {
    ...
    actions.forEach((action: PostAction) => {
      ...
      switch (action.type) {
        case 'select':
          content.push(
            <ActionMenu key={action.id} postId={this.props.postId} action={action} disabled={action.disabled} />
          )
          break
        case 'button':
        default:
  ...
  getFieldsTable = () => {
    ...
    fields.forEach((field: MessageAttachmentField, i: number) => {
      ...
      headerCols.push(
        <th className="attachment-field__caption" key={'attachment__field-caption-' + i + '__' + nrTables}>
          <Markdown message={field.title} options={markdown} />
        </th>
      )

      bodyCols.push(
        <td className="attachment-field" key={'attachment__field-' + i + '__' + nrTables}>
  ...
  render() {
    ...
    if (attachment.author_link && isUrlSafe(attachment.author_link)) {
      author = [
        <a href={attachment.author_link} target="_blank" rel="noopener noreferrer" key={'attachment__author-name'}>
          {author}
        </a>,
      ]
    }
    ...
    if (attachment.text) {
      attachmentText = (
        <ShowMore checkOverflow={this.state.checkOverflow} isAttachmentText={true} text={attachment.text}>
          <Markdown message={attachment.text || ''} options={options} imageProps={this.imageProps} />
        </ShowMore>
      )
    }
    ...
    if (attachment.image_url) {
      ...
      image = (
        <div className="attachment__image-container">
          <ExternalImage src={attachment.image_url} imageMetadata={imageMetadata}>
            {(imageUrl) => (
              <SizeAwareImage
    ...
    if (attachment.footer) {
      ...
      if (attachment.footer_icon) {
        ...
        footerIcon = (
          <ExternalImage src={attachment.footer_icon} imageMetadata={footerIconMetadata}>
            {(footerIconUrl) => (
              <img
    ...
    if (attachment.thumb_url) {
      ...
      thumb = (
        <div className="attachment__thumb-container">
          <ExternalImage src={attachment.thumb_url} imageMetadata={thumbMetadata}>
            {(thumbUrl) => (
              <SizeAwareImage
    ...
    return (
      <div className={'attachment ' + preTextClass} ref="attachment" onClick={this.handleFormattedTextClick}>
        {preText}
          ...
          <div
            className={
              useBorderStyle
                ...
                : 'clearfix attachment__container attachment__container--' + attachment.color
            }
