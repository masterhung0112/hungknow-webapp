...
export default class PostFlagIcon extends React.PureComponent {
  static propTypes = {
    location: PropTypes.oneOf([Locations.CENTER, Locations.RHS_ROOT, Locations.RHS_COMMENT, Locations.SEARCH])
      .isRequired,
  ...
  render() {
    ...
    if (isFlagged) {
      flagIcon = (
        <FlagIconFilled
          className={classNames('icon', 'icon--small', 'icon--small-filled', {
    ...
    return (
      ...
      >
        <button
          ...
          aria-label={
            isFlagged
              ? localizeMessage('flag_post.unflag', 'Remove from Saved').toLowerCase()
              : localizeMessage('flag_post.flag', 'Save').toLowerCase()
          }
