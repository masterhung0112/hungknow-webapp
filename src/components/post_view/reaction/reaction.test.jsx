...
describe('components/post_view/Reaction', () => {
  ...
  const baseProps = {
    ...
    actions,
    sortedUsers: getSortedUsers(reactions, currentUserId, profiles, 'username'),
  }
  ...
  test('should match snapshot when a current user reacted to a post', () => {
    ...
    const props = {
      ...
      otherUsersCount: 1,
      sortedUsers: getSortedUsers(newReactions, currentUserId, newProfiles, 'username'),
    }
  ...
  test('should apply read-only class if user does not have permission to remove reaction', () => {
    ...
    const props = {
      ...
      sortedUsers: getSortedUsers(reactions, newCurrentUserId, profiles, 'username'),
    }
    const wrapper = shallow(<Reaction {...props} />)
    expect(wrapper).toMatchSnapshot()
  })
