...
export default class PostAddChannelMember extends React.PureComponent<Props, State> {
      ...
      if (this.state.expanded || usernames.length <= 3) {
        return (
          <span>
            {usernames
              .map((username) => {
                return <AtMention key={username} mentionName={username} channelId={this.props.post?.channel_id} />
              })
  ...
  render() {
    ...
    if (usernames.length) {
      outOfChannelMessage = (
        <p>
          ...
          <FormattedMessage id={outOfChannelMessageID} defaultMessage={outOfChannelMessageText} />
          <a className="PostBody_addChannelMemberLink" onClick={this.handleAddChannelMember}>
            <FormattedMessage id={linkId} defaultMessage={linkText} />
          </a>
          <FormattedMessage
    ...
    if (noGroupsUsernames.length) {
      outOfGroupsMessage = (
        <p>
          {outOfGroupsAtMentions} <FormattedMessage id={outOfGroupsMessageID} defaultMessage={outOfGroupsMessageText} />
        </p>
      )
    }
