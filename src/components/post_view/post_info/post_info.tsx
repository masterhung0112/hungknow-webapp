...
export default class PostInfo extends React.PureComponent<Props, State> {
  ...
  buildOptions = (post: Post, isSystemMessage: boolean, fromAutoResponder: boolean) => {
    ...
    const hover =
      this.props.hover || this.state.showEmojiPicker || this.state.showDotMenu || this.state.showOptionsMenuWithoutHover

    const showCommentIcon =
      fromAutoResponder ||
      (!isSystemMessage &&
        (isMobile || hover || (!post.root_id && Boolean(this.props.replyCount)) || this.props.isFirstReply))
    ...
    let postFlagIcon
    if (showFlagIcon) {
      postFlagIcon = <PostFlagIcon postId={post.id} isFlagged={this.props.isFlagged} />
    }

    return (
      <div ref="dotMenu" data-testid={`post-menu-${post.id}`} className={'col post-menu'}>
        {dotMenu}
        {postReaction}
        {postFlagIcon}
        {commentIcon}
    ...
    if (isLastPost) {
      ...
      if (boundingRectOfPostInfo) {
        const isPostHeaderVisibleToUser =
          boundingRectOfPostInfo.top - 65 > 0 && boundingRectOfPostInfo.bottom < window.innerHeight - 85
  ...
  render() {
    ...
    if (showPostTime) {
      ...
      postTime = <PostTime isPermalink={isPermalink} eventTime={post.create_at} postId={post.id} />
    }

    return (
      <div className="post__header--info" ref={this.postHeaderRef}>
        <div className="col">
          {postTime}
          {postInfoIcon}
          {visibleMessage}
