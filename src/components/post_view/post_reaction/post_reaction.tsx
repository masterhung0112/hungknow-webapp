...
import { CustomEmoji, Emoji, SystemEmoji } from 'hkclient-ts/lib/types/emojis'
...
export default class PostReaction extends React.PureComponent<Props, State> {
  ...
  handleAddEmoji = (emoji: Emoji): void => {
    ...
    const emojiName = (emoji as CustomEmoji).name || (emoji as SystemEmoji).aliases[0]
    this.props.actions.addReaction(this.props.postId, emojiName)
    this.props.toggleEmojiPicker()
  }

  render() {
    const { channelId, location, postId, showEmojiPicker, teamId } = this.props
    ...
    return (
      <ChannelPermissionGate channelId={channelId} teamId={teamId} permissions={[Permissions.ADD_REACTION]}>
        <React.Fragment>
          <EmojiPickerOverlay
          ...
          <OverlayTrigger
            ...
            overlay={
              <Tooltip id="reaction-icon-tooltip" className="hidden-xs">
                <FormattedMessage id="post_info.tooltip.add_reactions" defaultMessage="Add Reaction" />
              </Tooltip>
            }
          >
            <button
