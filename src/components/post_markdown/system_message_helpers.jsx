...
function renderFormattedText(value, options, post) {
  return <Markdown message={value} options={options} postId={post && post.id} postType={post && post.type} />
}
...
function renderLeaveTeamMessage(post) {
  ...
  return <FormattedMessage id="api.team.leave.left" defaultMessage="{username} left the team." values={{ username }} />
}
...
export function renderSystemMessage(post, channel, isUserCanManageMembers) {
  if (post.props && post.props.add_channel_member) {
    ...
    return <CombinedSystemMessage allUserIds={allUserIds} allUsernames={allUsernames} messageData={messageData} />
  }
