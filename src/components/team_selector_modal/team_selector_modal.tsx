...
export default class TeamSelectorModal extends React.PureComponent<Props, State> {
  ...
  search = (
    term: string,
    multiselectComponent: { state: { page: number }; setState: (arg0: { page: number }) => void }
  ) => {
  ...
  renderOption = (
    option: TeamValue,
    isSelected: boolean,
    onAdd: (value: TeamValue) => void,
    onMouseMove: (value: TeamValue) => void
  ) => {
    let rowSelected = ''
    if (isSelected) {
      ...
      >
        <div className="more-modal__details">
          <div className="team-info-block">
            <TeamIcon content={option.display_name} url={imageURLForTeam(option)} />
  ...
  renderConfirmModal(show: boolean, team: TeamValue) {
    const title = (
      <FormattedMessage id="add_teams_to_scheme.confirmation.title" defaultMessage="Team Override Scheme Change?" />
    )
    const message = (
      <FormattedMessage
    ...
    const confirmButtonText = (
      <FormattedMessage id="add_teams_to_scheme.confirmation.accept" defaultMessage="Yes, Move Team" />
    )
    return (
      <ConfirmModal
        show={show}
  ...
  render() {
    ...
    const numRemainingText = (
      <FormattedMessage id="multiselect.selectTeams" defaultMessage="Use ↑↓ to browse, ↵ to select." />
    )
    ...
    return (
      ...
      >
        <Modal.Header closeButton={true}>
          <Modal.Title componentClass="h1" id="teamSelectorModalLabel">
