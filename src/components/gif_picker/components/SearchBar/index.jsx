...
import { saveSearchScrollPosition, saveSearchBarText, searchTextUpdate } from 'hkclient-ts/lib/actions/gifs'
import { getTheme } from 'hkclient-ts/lib/selectors/entities/preferences'
import { changeOpacity, makeStyleFromTheme } from 'hkclient-ts/lib/utils/theme_utils'
...
export class SearchBar extends Component {
  ...
  render() {
    ...
    const clearSearchButton = searchBarText ? (
      <GifSearchClearIcon className="ic-clear-search" style={style.icon} onClick={this.clearSearchHandle} />
    ) : null

    return (
      <form className="gfycat-search" method="get" target="_top" noValidate="" onSubmit={this.handleSubmit}>
        <div className="search-bar" style={style.background}>
          <div className="search-input-bg" style={style.inputBackground} />
          <LocalizedInput
            ...
            value={searchBarText}
          />
          <GifSearchIcon className="ic ic-search" style={style.icon} />
          {clearSearchButton}
        </div>
