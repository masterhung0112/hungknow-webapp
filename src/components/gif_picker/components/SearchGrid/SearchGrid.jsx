...
import { saveSearchScrollPosition } from 'hkclient-ts/lib/actions/gifs'
...
export class SearchGrid extends PureComponent {
  ...
  render() {
    const { containerClassName, gifs, keyword, resultsByTerm, scrollPosition, loadMore } = this.props

    const { containerWidth } = this.state
    const { moreRemaining, items = [], isEmpty = items.length === 0, isFetching } = resultsByTerm[keyword]
    ...
    const leftPosition = Array(this.numberOfColumns)
      .fill(0)
      .map((item, index) => this.padding + (index * columnWidth - index * (this.padding / 2)))
    ...
    const emptySearch =
      !isFetching && isEmpty ? (
        <NoResultsIndicator variant={NoResultsVariant.ChannelSearch} titleValues={{ channelName: `"${keyword}"` }} />
      ) : null

    return (
      <div id="search-grid-container" className={`search-grid-container ${containerClassName}`}>
        {content}
        {emptySearch}
      </div>
    )
