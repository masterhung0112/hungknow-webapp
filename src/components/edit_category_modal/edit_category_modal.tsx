...
export default class EditCategoryModal extends React.PureComponent<Props, State> {
  ...
  getText = () => {
    ...
    if (this.props.categoryId) {
      modalHeaderText = <FormattedMessage id="rename_category_modal.renameCategory" defaultMessage="Rename Category" />
      editButtonText = <FormattedMessage id="rename_category_modal.rename" defaultMessage="Rename" />
    } else {
      modalHeaderText = (
        <FormattedMessage id="create_category_modal.createCategory" defaultMessage="Create New Category" />
      )
      editButtonText = <FormattedMessage id="create_category_modal.create" defaultMessage="Create" />
      helpText = (
        <FormattedMessage
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        />
        {Boolean(helpText) && <span className="edit-category__helpText">{helpText}</span>}
      </GenericModal>
    )
  }
}
