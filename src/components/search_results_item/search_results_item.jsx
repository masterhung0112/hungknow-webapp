...
class SearchResultsItem extends React.PureComponent {
  ...
  renderPostTime = () => {
    ...
    const isPermalink = !(Posts.POST_DELETED === post.state || ReduxPostUtils.isPostPendingOrFailed(post))

    return (
      <PostTime isPermalink={isPermalink} eventTime={post.create_at} postId={post.id} location={Locations.SEARCH} />
    )
  }
  ...
  render() {
    ...
    const profilePic = (
      <PostProfilePicture compactDisplay={this.props.compactDisplay} post={post} userId={post.user_id} />
    )
    ...
    if (post.state === Constants.POST_DELETED || post.state === Posts.POST_DELETED) {
      ...
      if (!Utils.isMobile()) {
        flagContent = <PostFlagIcon location={Locations.SEARCH} postId={post.id} isFlagged={this.props.isFlagged} />
      }
      ...
      rhsControls = (
        <div className="col__controls post-menu">
          ...
          />
          <a href="#" onClick={this.handleJumpClick} className="search-item__jump">
    ...
    return (
      <div data-testid="search-item-container" className="search-item__container">
        ...
        >
          <div className="search-channel__name" aria-hidden="true">
            {channelName}
            {channelIsArchived && (
              <span className="search-channel__archived">
                ...
                <FormattedMessage id="search_item.channelArchived" defaultMessage="Archived" />
              </span>
            )}
          </div>
          <PostPreHeader
