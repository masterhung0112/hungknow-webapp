...
export default class SidebarNextSteps extends React.PureComponent<Props, State> {
  ...
  render() {
    ...
    const complete = this.props.preferences.filter(
      (pref) => pref.name !== RecommendedNextSteps.HIDE && pref.value === 'true'
    ).length

    let header = <FormattedMessage id="sidebar_next_steps.gettingStarted" defaultMessage="Getting Started" />
    if (!this.props.showNextSteps) {
      header = <FormattedMessage id="sidebar_next_steps.tipsAndNextSteps" defaultMessage="Tips & Next Steps" />
    }
    ...
    if (!this.props.showNextSteps) {
      middleSection = (
        <FormattedMessage id="sidebar_next_steps.otherAreasToExplore" defaultMessage="A few other areas to explore" />
      )
    }
    return (
      <div
      ...
      >
        ...
        {this.props.showNextSteps && (
          <div className="SidebarNextSteps__progressBar">
            <ProgressBar current={complete} total={total} basePercentage={4} />
          </div>
        )}
      </div>
    )
