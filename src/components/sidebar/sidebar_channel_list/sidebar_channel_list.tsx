...
export default class SidebarChannelList extends React.PureComponent<Props, State> {
  ...
  updateUnreadIndicators = () => {
    ...
    if (firstUnreadChannel) {
      ...
      if (
        firstUnreadElement &&
        firstUnreadElement.offsetTop + firstUnreadElement.offsetHeight - scrollMargin - categoryHeaderHeight <
    ...
    if (lastUnreadChannel) {
      ...
      if (
        ...
        lastUnreadElement.offsetTop + scrollMargin >
          this.scrollbar.current!.getScrollTop() + this.scrollbar.current!.getClientHeight()
      ) {
  ...
  onBeforeCapture = (before: BeforeCapture) => {
    ...
    if (this.props.categories.some((category) => category.id === before.draggableId)) {
      ...
      const draggingChannels = this.props.displayedChannels.filter(
        (channel) => this.props.multiSelectedChannelIds.indexOf(channel.id) !== -1 || channel.id === before.draggableId
      )
      if (
        draggingChannels.every((channel) => channel.type === General.DM_CHANNEL || channel.type === General.GM_CHANNEL)
      ) {
  ...
  render() {
    ...
    if (this.props.isUnreadFilterEnabled) {
      channelList = <UnreadChannels getChannelRef={this.getChannelRef} setChannelRef={this.setChannelRef} />
    } else {
      let unreadsCategory
      if (this.props.showUnreadsCategory) {
        unreadsCategory = <UnreadChannels getChannelRef={this.getChannelRef} setChannelRef={this.setChannelRef} />
      }
