...
import { ChannelCategory, CategorySorting } from 'hkclient-ts/lib/types/channel_categories'
import { localizeMessage } from 'hkclient-ts/lib/utils/i18n_utils'
...
export default class SidebarCategory extends React.PureComponent<Props, State> {
    ...
    return (
      <SidebarChannel
        ...
        isAutoSortedCategory={
          category.sorting === CategorySorting.Alphabetical || category.sorting === CategorySorting.Recency
        }
      />
    )
  }
  ...
  handleSortDirectMessages = (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
    ...
    const newSorting =
      category.sorting === CategorySorting.Recency ? CategorySorting.Alphabetical : CategorySorting.Recency
  ...
  renderNewDropBox = (isDraggingOver: boolean) => {
    ...
    return (
      <React.Fragment>
        <Draggable draggableId={`NEW_CHANNEL_SPACER__${category.id}`} isDragDisabled={true} index={0}>
          {(provided) => {
        ...
        <div
          ...
          className={classNames('SidebarCategory_newDropBox', {
            collapsed:
              isCollapsed || (draggingState.type === DraggingStateTypes.CATEGORY && draggingState.id === category.id),
            isDraggingOver,
          })}
  ...
  render() {
    const { category, categoryIndex, channels, isCollapsed, isNewCategory } = this.props
      ...
      categoryMenu = (
        <React.Fragment>
          <SidebarCategorySortingMenu
            ...
            onToggleMenu={this.handleMenuToggle}
          />
          <OverlayTrigger delayShow={500} placement="top" overlay={addTooltip}>
            <button
        ...
        {(provided, snapshot) => {
          return (
            <div
              ...
              {...provided.draggableProps}
            >
              <Droppable droppableId={category.id} type="SIDEBAR_CHANNEL" isDropDisabled={this.isDropDisabled()}>
