...
export default class SidebarMenu extends React.PureComponent<Props, State> {
  ...
  componentDidUpdate(prevProps: Props) {
    if (
      prevProps.draggingState.state !== this.props.draggingState.state &&
      this.props.draggingState.state === DraggingStates.CAPTURE
    ) {
      this.closeMenu()
    }
  ...
  disableScrollbar = () => {
    const scrollbars: NodeListOf<HTMLElement> = document.querySelectorAll(
      '#SidebarContainer .SidebarNavContainer .scrollbar--view'
    )
  ...
  setMenuPosition = () => {
    if (this.menuButtonRef.current && this.menuRef) {
      ...
      menuRef.style.top = `${
        window.scrollY +
        this.menuButtonRef.current.getBoundingClientRect().top +
        this.menuButtonRef.current.clientHeight +
        openUpOffset
      }px`
    }
  }
  ...
  render() {
    const { ariaLabel, buttonAriaLabel, children, isMenuOpen, tooltipText, id, additionalClass } = this.props
    ...
    return (
      ...
      >
        <OverlayTrigger delayShow={500} placement="top" overlay={tooltip} disabled={isMenuOpen}>
          <button
        ...
        <Menu ref={this.refCallback} openUp={this.state.openUp} id={`SidebarMenu-${id}`} ariaLabel={ariaLabel}>
          {children}
        </Menu>
      </MenuWrapper>
    )
