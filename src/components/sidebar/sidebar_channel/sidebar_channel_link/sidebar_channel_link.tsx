...
export default class SidebarChannelLink extends React.PureComponent<Props, State> {
  ...
  showChannelAsUnread = (): boolean =>
    this.props.unreadMentions > 0 || (this.props.unreadMsgs > 0 && this.props.showUnreadForMsgs)
  ...
  render(): JSX.Element {
    ...
    let labelElement: JSX.Element = (
      <span
        className={classNames('SidebarChannelLinkLabel', {
    ...
    if (this.state.showTooltip) {
      const displayNameToolTip = <Tooltip id="channel-displayname__tooltip">{label}</Tooltip>
      labelElement = (
        <OverlayTrigger
        ...
        >
          <div ref={this.gmItemRef}>{labelElement}</div>
        </OverlayTrigger>
      )
    }
    ...
    const content = (
      <>
        <SidebarChannelIcon channel={channel} icon={icon} />
        <div className="SidebarChannelLinkLabel_wrapper" ref={this.labelRef}>
          {labelElement}
          {customStatus}
        </div>
        <ChannelMentionBadge unreadMentions={unreadMentions} />
        <SidebarChannelMenu
          channel={channel}
