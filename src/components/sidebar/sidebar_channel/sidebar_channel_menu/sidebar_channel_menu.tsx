...
export class SidebarChannelMenu extends React.PureComponent<Props, State> {
  ...
  renderDropdownItems = () => {
    ...
    const selectedChannels =
      multiSelectedChannelIds.indexOf(channel.id) === -1
        ? [channel]
        : displayedChannels.filter((c) => multiSelectedChannelIds.indexOf(c.id) !== -1)
    const allChannelsAreDMs = selectedChannels.every(
      (selectedChannel) =>
        selectedChannel.type === Constants.DM_CHANNEL || selectedChannel.type === Constants.GM_CHANNEL
    )
    ...
    let addMembers
    if (
      (channel.type === Constants.PRIVATE_CHANNEL && this.props.managePrivateChannelMembers) ||
      (channel.type === Constants.OPEN_CHANNEL && this.props.managePublicChannelMembers)
    ) {
      addMembers = (
        <Menu.ItemAction
    ...
    let leaveChannelText = intl.formatMessage({
      id: 'sidebar_left.sidebar_channel_menu.leaveChannel',
      defaultMessage: 'Leave Channel',
    })
    if (channel.type === Constants.DM_CHANNEL || channel.type === Constants.GM_CHANNEL) {
      leaveChannelText = intl.formatMessage({
  ...
  render() {
    const { channel, intl, isCollapsed, isMenuOpen } = this.props
