...
import { favoriteChannel, unfavoriteChannel, markChannelAsRead } from 'hkclient-ts/lib/actions/channels'
import Permissions from 'hkclient-ts/lib/constants/permissions'
import { isFavoriteChannel } from 'hkclient-ts/lib/selectors/entities/channels'
import { getMyChannelMemberships, getCurrentUserId } from 'hkclient-ts/lib/selectors/entities/common'
import { getCategoryInTeamWithChannel } from 'hkclient-ts/lib/selectors/entities/channel_categories'
...
function mapStateToProps(state: GlobalState, ownProps: OwnProps) {
  ...
  if (currentTeam) {
    managePublicChannelMembers = haveIChannelPermission(state, {
      channel: ownProps.channel.id,
      team: currentTeam.id,
      permission: Permissions.MANAGE_PUBLIC_CHANNEL_MEMBERS,
    })
    managePrivateChannelMembers = haveIChannelPermission(state, {
      channel: ownProps.channel.id,
      team: currentTeam.id,
