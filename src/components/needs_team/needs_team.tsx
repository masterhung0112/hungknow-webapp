...
import { UserStatus } from 'hkclient-ts/lib/types/users'
...
type Props = {
  ...
  actions: {
    fetchMyChannelsAndMembers: (
      teamId: string
    ) => Promise<{ data: { channels: Channel[]; members: ChannelMembership[] } }>
...
export default class NeedsTeam extends React.PureComponent<Props, State> {
  ...
  static getDerivedStateFromProps(nextProps: Props, state: State) {
    if (state.prevTeam !== nextProps.match.params.team) {
      const team = nextProps.teamsList
        ? nextProps.teamsList.find((teamObj: Team) => teamObj.name === nextProps.match.params.team)
        : null
      return {
  ...
  initTeam = (team: Team) => {
    ...
    if (this.props.license && this.props.license.IsLicensed === 'true' && this.props.license.LDAPGroups === 'true') {
      if (this.props.currentUser) {
  ...
  render() {
    ...
    return (
      <Switch>
        <Route path={'/:team/integrations'} component={BackstageController} />
        <Route path={'/:team/emoji'} component={BackstageController} />
        {this.props.plugins?.map((plugin: any) => (
          <Route
            key={plugin.id}
            path={'/:team/' + plugin.route}
            render={() => <Pluggable pluggableName={'NeedsTeamComponent'} pluggableId={plugin.id} />}
          />
        ))}
        <Route
          render={(renderProps) => (
