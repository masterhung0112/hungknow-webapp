...
import { getTeammateNameDisplaySetting, getMyPreferences } from 'hkclient-ts/lib/selectors/entities/preferences'
...
import { searchChannels } from 'hkclient-ts/lib/actions/channels'
import { logError } from 'hkclient-ts/lib/actions/errors'
import { getLastPostPerChannel } from 'hkclient-ts/lib/selectors/entities/posts'
import {
...
class SwitchChannelSuggestion extends Suggestion {
  ...
  render() {
    ...
    if (channelIsArchived) {
      ...
      icon = (
        <div className="pull-left">
          <Avatar size="sm" url={userImageUrl} />
        </div>
      )
      ...
      if (userItem.firstName || userItem.last_name) {
        displayName = (
          <React.Fragment>
            ...
            <StatusIcon className={`${status}--icon`} status={status} button={false} />
            <div className="mentions__fullname">{userItem.username}</div>
          </React.Fragment>
        )
      } else {
        displayName = (
    ...
    if (channel.type === Constants.DM_CHANNEL) {
      ...
      tag = (
        <React.Fragment>
          <BotBadge show={Boolean(teammate && teammate.is_bot)} className="badge-autocomplete" />
          <GuestBadge show={Boolean(teammate && Utils.isGuest(teammate))} className="badge-autocomplete" />
        </React.Fragment>
      )
    }
    ...
    return (
      ...
      >
        {icon}
        <span className="suggestion-list__info_user">{displayName}</span>
        {tag}
        {badge}
      </div>
    )
...
const ConnectedSwitchChannelSuggestion = connect(mapStateToPropsForSwitchChannelSuggestion, null, null, {
  forwardRef: true,
...
export default class SwitchChannelProvider extends Provider {
  ...
  async fetchUsersAndChannels(channelPrefix, resultsCallback) {
    ...
    const combinedTerms = [
      ...
      ...remoteFormattedData.terms.filter((term) => !localFormattedData.terms.includes(term)),
    ]
    const combinedItems = [
      ...localFormattedData.items,
      ...remoteFormattedData.items.filter(
        (item) => !localFormattedData.terms.includes(item.channel.userId || item.channel.id)
      ),
    ]
      ...
      if (channelFilter(channel)) {
        ...
        if (!viewArchivedChannels && channelIsArchived) {
          ...
          const isGMVisible = isGroupChannelVisible(
            ...
            isUnreadChannel(getMyChannelMemberships(state), channel)
          )
