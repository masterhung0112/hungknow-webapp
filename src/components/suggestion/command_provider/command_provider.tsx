...
import { getChannel, getCurrentChannel } from 'hkclient-ts/lib/selectors/entities/channels'
import { CommandAutocompleteSuggestion } from 'hkclient-ts/lib/types/integrations'
...
export class CommandSuggestion extends Suggestion {
  render() {
    ...
    if (item.iconData && item.iconData !== EXECUTE_CURRENT_COMMAND_ITEM_ID) {
      icon = (
        <div className="slash-command__icon" style={{ backgroundColor: 'transparent' }}>
    ...
    return (
      ...
      >
        ...
        <div className="slash-command__info">
          <div className="slash-command__title">{item.suggestion.substring(1) + ' ' + item.hint}</div>
...
export default class CommandProvider extends Provider {
  ...
  handleMobile(pretext: string, resultCallback: ResultsCallback) {
    ...
    Client4.getCommandsList(getCurrentTeamId(this.store.getState()))
      .then((data) => {
  ...
  handleWebapp(pretext: string, resultCallback: ResultsCallback) {
    ...
    const channel =
      this.isInRHS && selectedPost?.channel_id
        ? getChannel(this.store.getState(), selectedPost.channel_id)
        : getCurrentChannel(this.store.getState())
