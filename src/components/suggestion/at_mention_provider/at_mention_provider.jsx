...
import { getSuggestionsSplitBy, getSuggestionsSplitByMultiple } from 'hkclient-ts/lib/utils/user_utils'
...
export default class AtMentionProvider extends Provider {
  ...
  items() {
    ...
    const remoteMembers = this.remoteMembers().filter(
      (member) => !localUserIds[member.id] && !priorityProfilesIds[member.id]
    )
