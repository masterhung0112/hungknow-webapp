...
export default class SuggestionBox extends React.PureComponent {
  ...
  nonDebouncedPretextChanged = (pretext, complete = false) => {
    ...
    for (const provider of this.props.providers) {
      ...
      if (handled) {
        if (
          this.state.suggestionBoxAlgn.pixelsToMoveX === undefined &&
  ...
  render() {
    const { listComponent, dateComponent, listStyle, renderNoResults, ...props } = this.props
    ...
    return (
      <div ref={this.setContainerRef} className={this.props.containerClass}>
        <div ref={this.suggestionReadOut} aria-live="polite" role="alert" className="sr-only" />
        <QuickInput
          ref={this.inputRef}
          autoComplete="off"
          {...props}
