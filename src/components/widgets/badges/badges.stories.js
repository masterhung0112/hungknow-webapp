...
storiesOf('Widgets/Badges', module)
  ...
  .add('regular badge, clickable', () => {
    const content = text('Text', 'BADGE')
    return (
      <Badge show={boolean('Show', true)} onClick={action('click!')}>
        {content}
      </Badge>
    )
  })
  .add('guest badge', () => <GuestBadge show={boolean('Show', true)} />)
  .add('bot badge', () => <BotBadge show={boolean('Show', true)} />)
