...
function Avatars({
  ...
  return (
    <div className={`Avatars Avatars___${size}`} onMouseLeave={() => setImmediate(false)}>
      {displayUsers.map(({ name, ...user }) => (
        <SimpleTooltip key={user.url} id={`name-${user.username}`} content={name} {...overlayProps}>
          <Avatar size={size} tabIndex={0} {...user} />
        </SimpleTooltip>
      ))}
