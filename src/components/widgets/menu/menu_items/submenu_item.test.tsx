...
describe('components/widgets/menu/menu_items/submenu_item', () => {
  test('empty subMenu should match snapshot', () => {
    const wrapper = mount(
      <SubMenuItem key={'_pluginmenuitem'} id={'1'} text={'test'} subMenu={[]} action={jest.fn()} root={true} />
    )

    expect(wrapper.html()).toEqual(
      format(`<li class="SubMenuItem MenuItem" role="menuitem" id="1_menuitem">
  ...
  test('should show/hide submenu based on keyboard commands', () => {
    const wrapper = mount<SubMenuItem>(
      <SubMenuItem key={'_pluginmenuitem'} id={'1'} text={'test'} subMenu={[]} root={true} direction={'right'} />
    )
