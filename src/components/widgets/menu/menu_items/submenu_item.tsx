  ...
  private onClick = (event: React.SyntheticEvent<HTMLElement>) => {
    ...
    if (isMobile) {
      if (subMenu && subMenu.length) {
        // if contains a submenu, call openModal with it
        if (!root) {
          //required to close only the original menu
          event.stopPropagation()
        }
        showMobileSubMenuModal(subMenu)
      } else if (action) {
        // leaf node in the tree handles action only
        action(postId)
      }
    } else if (
      ...
      action(postId)
    } else if (!path && !event.nativeEvent.composedPath && action) {
        ...
        >
          {hasSubmenu
            ? subMenu!.map((s) => {
                ...
                return (
                  <div className={classNames(['SubMenuItemContainer', { hasDivider }])} key={s.id}>
                    <SubMenuItem
                      id={s.id}
                      postId={postId}
                      text={s.text}
                    ...
                    />
                    {s.text === selectedValueText && (
                      <span className="sorting-menu-checkbox">
        ...
        >
          {id !== 'SidebarChannelMenu-moveToDivider' && (
            <span
              ...
              className={classNames([
                `fa fa-angle-left SubMenu__icon-left${hasSubmenu && direction === 'left' ? '' : '-empty'}`,
                { mobile: isMobile },
              ])}
          ...
          {id !== 'SidebarChannelMenu-moveToDivider' && (
            <span
              ...
              className={classNames([
                `fa fa-angle-right SubMenu__icon-right${hasSubmenu && direction === 'right' ? '' : '-empty'}`,
                { mobile: isMobile },
              ])}
              aria-label={Utils.localizeMessage('post_info.submenu.icon', 'submenu icon').toLowerCase()}
            />
