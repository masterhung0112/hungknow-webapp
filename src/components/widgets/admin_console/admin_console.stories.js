...
storiesOf('Widgets/Admin Console', module)
  ...
  .add('admin header', () => {
    const title = text('Title', 'title')
    return <AdminHeader>{title}</AdminHeader>
  })
  .add('formatted admin header', () => {
    const markdown = text('Markdown', '**Markdown** text')
    return <FormattedAdminHeader id="not-valid-id" defaultMessage={markdown} />
  })
