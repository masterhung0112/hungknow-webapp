...
describe('components/AddGroupsToChannelModal', () => {
  ...
  test('should match state when handleSubmit is called', async () => {
    ...
    expect(actions.linkGroupSyncable).toBeCalledWith('id_1', baseProps.currentChannelId, Groups.SYNCABLE_TYPE_CHANNEL, {
      auto_add: true,
    })
    expect(actions.linkGroupSyncable).toBeCalledWith('id_2', baseProps.currentChannelId, Groups.SYNCABLE_TYPE_CHANNEL, {
      auto_add: true,
    })
  ...
  test('should match when renderValue is called', () => {
    ...
    expect((wrapper.instance() as AddGroupsToChannelModal).renderValue({ data: { display_name: 'foo' } })).toEqual(
      'foo'
    )
  })
})
