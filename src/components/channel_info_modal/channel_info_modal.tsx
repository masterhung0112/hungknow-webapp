...
export default class ChannelInfoModal extends React.PureComponent<Props, State> {
  ...
  getHeaderMarkdownOptions = memoizeResult((channelNamesMap: ChannelNamesMap) => ({
  ...
  render() {
    ...
    if (channelIsArchived) {
      channelIcon = <ArchiveIcon className="icon icon__archive svg-text-color" />
    ...
    if (channel.header) {
      channelHeader = (
        <div className="form-group">
          ...
          <div onClick={this.handleFormattedTextClick} className="info__value">
            <Markdown message={channel.header} options={this.getHeaderMarkdownOptions(channelNamesMap)} />
          </div>
        </div>
      )
    }
