...
import { Preferences, General } from 'hkclient-ts/lib/constants'
import { CategoryTypes } from 'hkclient-ts/lib/constants/channel_categories'
import { getPreferenceKey } from 'hkclient-ts/lib/utils/preference_utils'
...
jest.mock('hkclient-ts/lib/actions/users', () => {
  const original = jest.requireActual('hkclient-ts/lib/actions/users')
  return {
    ...original,
...
jest.mock('hkclient-ts/lib/selectors/entities/channels', () => {
  const GeneralTypes = jest.requireActual('hkclient-ts/lib/constants').General
  const original = jest.requireActual('hkclient-ts/lib/selectors/entities/channels')
...
jest.mock('hkclient-ts/lib/actions/teams', () => {
  const original = jest.requireActual('hkclient-ts/lib/actions/teams')
  return {
    ...original,
...
jest.mock('hkclient-ts/lib/actions/channels', () => {
  const original = jest.requireActual('hkclient-ts/lib/actions/channels')
  return {
    ...original,
...
jest.mock('hkclient-ts/lib/actions/preferences', () => {
  const original = jest.requireActual('hkclient-ts/lib/actions/preferences')
  return {
    ...original,
...
describe('Actions.User', () => {
  ...
  test('loadProfilesAndTeamMembersAndChannelMembers', async () => {
    const expectedActions = [
      { type: 'MOCK_GET_PROFILES_IN_CHANNEL', args: ['current_channel_id', 0, 60, '', undefined] },
    ]

    let testStore = await mockStore(initialState)
    await testStore.dispatch(
      UserActions.loadProfilesAndTeamMembersAndChannelMembers(0, 60, 'team_1', 'current_channel_id')
    )
    let actualActions = testStore.getActions()
    expect(actualActions[0].args).toEqual(expectedActions[0].args)
    expect(actualActions[0].type).toEqual(expectedActions[0].type)
  ...
  test('loadTeamMembersAndChannelMembersForProfilesList', async () => {
    ...
    testStore = await mockStore(initialState)
    await testStore.dispatch(
      UserActions.loadTeamMembersForProfilesList([{ id: 'current_user_id' }], 'team_1', 'current_channel_id')
    )
    expect(testStore.getActions()).toEqual([])
  ...
  test('loadChannelMembersForProfilesList', async () => {
    ...
    let testStore = await mockStore(initialState)
    await testStore.dispatch(
      UserActions.loadChannelMembersForProfilesList([{ id: 'other_user_id' }], 'current_channel_id')
    )
    expect(testStore.getActions()).toEqual(expectedActions)
    ...
    testStore = await mockStore(initialState)
    await testStore.dispatch(
      UserActions.loadChannelMembersForProfilesList([{ id: 'current_user_id' }], 'current_channel_id')
    )
    expect(testStore.getActions()).toEqual([])
