...
jest.mock('actions/team_actions', () => ({
  addUsersToTeam: () => ({
    // since we are using addUsersToTeamGracefully, this call will always succeed
...
jest.mock('hkclient-ts/lib/actions/teams', () => ({
  ...
  sendEmailInvitesToTeamGracefully: (team, emails) => {
    ...
    if (emails.length > 21) {
      return {
        type: 'MOCK_RECEIVED_ME',
        data: emails.map((email) => ({ email, error: { message: 'Invite emails rate limit exceeded.' } })),
      }
    }
    return {
      type: 'MOCK_RECEIVED_ME',
      data: emails.map((email) => ({
        email,
        error: team === 'correct' ? undefined : { message: 'Unable to add the user to the team.' },
      })),
    }
  },
  sendEmailGuestInvitesToChannelsGracefully: (team, channels, emails) => {
    ...
    return {
      ...
      data: emails.map((email) => ({
        email,
        error: team === 'correct' ? undefined : { message: 'Unable to add the guest to the channels.' },
      })),
    }
  },
}))
