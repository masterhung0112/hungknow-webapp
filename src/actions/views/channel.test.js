...
import { General, Posts, RequestStatus } from 'hkclient-ts/lib/constants'
import { leaveChannel, markChannelAsRead } from 'hkclient-ts/lib/actions/channels'
...
jest.mock('hkclient-ts/lib/actions/channels', () => ({
  ...jest.requireActual('hkclient-ts/lib/actions/channels'),
  markChannelAsRead: jest.fn(() => ({ type: '' })),
  leaveChannel: jest.fn(() => ({ type: '' })),
}))

jest.mock('hkclient-ts/lib/actions/posts')

jest.mock('selectors/local_storage', () => ({
  getLastViewedChannelName: () => 'channel1',
}))

jest.mock('hkclient-ts/lib/selectors/entities/utils', () => ({
    ...
    entities: {
      users: {
        ...
        profiles: {
          ...
          userid2: { id: 'userid2', username: 'username2', roles: 'system_user' },
        },
        profilesInChannel: {},
      },
      teams: {
  ...
  describe('increasePostVisibility', () => {
    ...
    test('should return more to load when enough posts are received', async () => {
      ...
      const result = await store.dispatch(
        Actions.loadPosts({ channelId, postId: 'oldest_post_id', type: PostRequestTypes.BEFORE_ID })
      )
