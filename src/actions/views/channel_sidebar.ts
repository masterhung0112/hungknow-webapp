...
import { getCategory, makeGetChannelsForCategory } from 'hkclient-ts/lib/selectors/entities/channel_categories'
...
import { insertMultipleWithoutDuplicates } from 'hkclient-ts/lib/utils/array_utils'
    ...
    if (multiSelectedChannelIds.length && multiSelectedChannelIds.indexOf(draggableChannelId) !== -1) {
      ...
      channelsToMove = multiSelectedChannelIds.filter((channelId) => {
        ...
        return (
          ...
          (isDMGM && targetCategory?.type === CategoryTypes.DIRECT_MESSAGES) ||
          (!isDMGM && targetCategory?.type !== CategoryTypes.DIRECT_MESSAGES)
        )
      })
...
export function adjustTargetIndexForMove(
  ...
  const removedChannelsAboveInsert = filteredChannelIds.filter(
    (channel, index) => channel !== draggableChannelId && channelIds.indexOf(channel) !== -1 && index <= targetIndex
  )
