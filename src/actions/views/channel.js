...
import { getCurrentUserId, getUserByUsername } from 'hkclient-ts/lib/selectors/entities/users'
import { getMostRecentPostIdInChannel, getPost } from 'hkclient-ts/lib/selectors/entities/posts'
import { makeAddLastViewAtToProfiles } from 'hkclient-ts/lib/selectors/entities/utils'

import { getChannelByName } from 'hkclient-ts/lib/utils/channel_utils'
...
export function leaveChannel(channelId) {
  return async (dispatch, getState) => {
    ...
    if (
      getMyChannels(getState()).filter((c) => c.type === Constants.OPEN_CHANNEL || c.type === Constants.PRIVATE_CHANNEL)
        .length === 0
    ) {
      LocalStorageStore.removePreviousChannelName(currentUserId, currentTeam.id, state)
      dispatch(selectTeam(''))
...
export function loadPostsAround(channelId, focusedPostId) {
  return async (dispatch) => {
    const { data, error } = await dispatch(
      PostActions.getPostsAround(channelId, focusedPostId, Posts.POST_CHUNK_SIZE / 2)
    )
    if (error) {
      return {
        error,
