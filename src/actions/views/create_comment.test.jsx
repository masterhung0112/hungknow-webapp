...
import { removeReaction, addMessageIntoHistory, moveHistoryIndexBack } from 'hkclient-ts/lib/actions/posts'
...
describe('rhs view actions', () => {
  ...
  describe('submitPost', () => {
    ...
    test('it call PostActions.createPost with post', async () => {
      ...
      expect(lastCall(PostActions.createPost.mock.calls)[0]).toEqual(expect.objectContaining(post))
  ...
  describe('submitCommand', () => {
    ...
    test('it does not call executeComaand when hooks fail', async () => {
      HookActions.runSlashCommandWillBePostedHooks.mockImplementation(() => () => ({
        error: { message: 'An error occurred' },
      }))
      ...
      const expectedActions = [
        {
          args: [
            '/test msg',
            { channel_id: '4j5j4k3k34j4', parent_id: 'fc234c34c23', root_id: 'fc234c34c23', team_id: '4j5nmn4j3' },
          ],
  ...
  describe('makeOnSubmit', () => {
    ...
    test('it submits a regular post when options.ignoreSlash is true', () => {
      ...
      const testStore = mockStore(initialState)
      testStore.dispatch(
        submitPost(channelId, rootId, { message: '/fakecommand', fileInfos: [], uploadsInProgress: [] })
      )
  ...
  describe('makeOnEditLatestPost', () => {
    ...
    test('it dispatches the correct actions', () => {
      ...
      expect(store.getActions()).toEqual([
        PostActions.setEditingPost(latestPostId, 0, 'reply_textbox', 'Comment', true),
      ])
    })
  })
})
