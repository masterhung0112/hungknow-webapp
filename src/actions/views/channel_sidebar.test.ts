...
import { insertWithoutDuplicates } from 'hkclient-ts/lib/utils/array_utils'
  ...
  describe('should be able to move channels backwards', () => {
    ...
    for (let i = 0; i < testCases.length; i++) {
      ...
      test('at ' + i, async () => {
        ...
        const newIndex = Actions.adjustTargetIndexForMove(
          store.getState(),
          'category1',
          ['seven'],
          targetIndex,
...
describe('multiSelectChannelTo', () => {
  ...
  test('should select group of channels in ascending order', () => {
    ...
    expect(store.getState().views.channelSidebar.multiSelectedChannelIds).toEqual([
      'category1_two',
      'category1_three',
      'category1_four',
      'category1_five',
