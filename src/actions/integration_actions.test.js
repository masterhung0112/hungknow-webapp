...
import { getProfilesByIds } from 'hkclient-ts/lib/actions/users'
...
describe('actions/integration_actions', () => {
  const initialState = {
    entities: {
      ...
      users: {
        ...
        profiles: {
          current_user_id: { id: 'current_user_id', username: 'current_user' },
  ...
  describe('loadProfilesForIncomingHooks', () => {
    test('load profiles for hooks including user we already have', () => {
      ...
      testStore.dispatch(
        Actions.loadProfilesForIncomingHooks([{ user_id: 'current_user_id' }, { user_id: 'user_id2' }])
      )
  ...
  describe('loadProfilesForOutgoingHooks', () => {
    test('load profiles for hooks including user we already have', () => {
      const testStore = mockStore(initialState)
      testStore.dispatch(
        Actions.loadProfilesForOutgoingHooks([{ creator_id: 'current_user_id' }, { creator_id: 'user_id2' }])
      )
      expect(getProfilesByIds).toHaveBeenCalledWith(expect.arrayContainingExactly(['user_id2']))
    })
  ...
  describe('loadProfilesForCommands', () => {
    test('load profiles for commands including user we already have', () => {
      const testStore = mockStore(initialState)
      testStore.dispatch(
        Actions.loadProfilesForCommands([{ creator_id: 'current_user_id' }, { creator_id: 'user_id2' }])
      )
      expect(getProfilesByIds).toHaveBeenCalledWith(expect.arrayContainingExactly(['user_id2']))
    })
  ...
  describe('loadProfilesForOAuthApps', () => {
    test('load profiles for apps including user we already have', () => {
      const testStore = mockStore(initialState)
      testStore.dispatch(
        Actions.loadProfilesForOAuthApps([{ creator_id: 'current_user_id' }, { creator_id: 'user_id2' }])
      )
      expect(getProfilesByIds).toHaveBeenCalledWith(expect.arrayContainingExactly(['user_id2']))
    })
