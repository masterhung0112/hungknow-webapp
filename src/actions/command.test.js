...
import * as Channels from 'hkclient-ts/lib/selectors/entities/channels'
import * as Teams from 'hkclient-ts/lib/selectors/entities/teams'
  ...
  describe('leave', () => {
    ...
    test('should use user id as name if channel is dm', async () => {
      ...
      expect(store.getActions()[0].data).toEqual([
        { category: 'direct_channel_show', name: 'userId', user_id: 'user123', value: 'false' },
      ])
    ...
    test('should use channel id as name if channel is gm', async () => {
      ...
      const result = await store.dispatch(executeCommand('/leave', {}))
      expect(store.getActions()[0].data).toEqual([
        { category: 'group_channel_show', name: 'channelId', user_id: 'user123', value: 'false' },
      ])
