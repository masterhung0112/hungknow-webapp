...
} from 'hkclient-ts/lib/actions/channel_categories'
import {
...
import { getCloudSubscription, getSubscriptionStats } from 'hkclient-ts/lib/actions/cloud'
import { loadRolesIfNeeded } from 'hkclient-ts/lib/actions/roles'
import { setServerVersion } from 'hkclient-ts/lib/actions/general'
import {
...
import { removeNotVisibleUsers } from 'hkclient-ts/lib/actions/websocket'
...
import { getPost, getMostRecentPostIdInChannel } from 'hkclient-ts/lib/selectors/entities/posts'
import { haveISystemPermission, haveITeamPermission } from 'hkclient-ts/lib/selectors/entities/roles'
...
export async function handleUserRemovedEvent(msg) {
  ...
  if (
    ...
    !haveITeamPermission(state, { permission: Permissions.VIEW_MEMBERS, team: channel.team_id })
  ) {
    dispatch(
      batchActions([
        {
...
function handleChannelViewedEvent(msg) {
  ...
  if (
    (!window.isActive || getCurrentChannelId(getState()) !== msg.data.channel_id) &&
...
function handleUserRoleUpdated(msg) {
  ...
  if (user) {
    ...
    const demoted =
      user.roles.includes(Constants.PERMISSIONS_SYSTEM_ADMIN) && !roles.includes(Constants.PERMISSIONS_SYSTEM_ADMIN)
...
function handleFirstAdminVisitMarketplaceStatusReceivedEvent(msg) {
  var receivedData = JSON.parse(msg.data.firstAdminVisitMarketplaceStatus)
